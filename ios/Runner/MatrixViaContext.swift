...
class MatrixViaContext {
    //: static let shared = AppPermissionTool()
    static let shared = MatrixViaContext()

    /// 获取麦克风权限
    //: func requestMicPermission(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
    func than(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
    ...
    func nut(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
        //: if #available(iOS 14, *) {
        if #available(iOS 14, *) {
    ...
    func modelDuring(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
