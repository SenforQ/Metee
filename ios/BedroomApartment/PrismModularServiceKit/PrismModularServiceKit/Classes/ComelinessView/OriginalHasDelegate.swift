...
class OriginalHasDelegate: UINavigationController {

    //: var tabBarType: TabBarItemType?
    var tabBarType: FirstSignedTotal?
    //: var pushing = false                             
    ...
    override func pushViewController(_ viewController: UIViewController, animated: Bool) {
        ...
        if viewControllers.count >= 1 {
            ...
            if viewController.navigationItem.leftBarButtonItem == nil {
                ...
                viewController.navigationItem.leftBarButtonItem = viewController.itemNaviPopback()
            }
        }
    ...
    func equalAppearance() {
        ...
        if #available(iOS 15.0, *) {
            ...
            barApp.backgroundImage = UIImage.magnitudeernalRepresentation(color: UIColor.white)
            //: barApp.shadowImage = UIImage.init()
            barApp.shadowImage = UIImage.init()
            //: barApp.shadowColor = UIColor.appBgColor()
            barApp.shadowColor = UIColor.objectRed()
            ...
            barApp.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.census(), NSAttributedString.Key.font: UIFont.pingfangCenter(type: .Medium, fontSize: 17)]
        ...
        //: } else {
            ...
            navigationBar.setBackgroundImage(UIImage.magnitudeernalRepresentation(color: UIColor.white), for: .any, barMetrics: .default)
            ...
            navigationBar.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.census(), NSAttributedString.Key.font: UIFont.pingfangCenter(type: .Medium, fontSize: 17)]
        }
        //: UINavigationBar.appearance().tintColor = UIColor.appTitleColor()
        UINavigationBar.appearance().tintColor = UIColor.census()
...
extension OriginalHasDelegate: UINavigationControllerDelegate {
...
extension OriginalHasDelegate: UIGestureRecognizerDelegate {
