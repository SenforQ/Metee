...
fileprivate let notiMakeName:[UInt8] = [0x30,0x37,0x30,0x2d,0x71,0x3a,0x36,0x3d,0x3c,0x2b,0x63,0x70,0x79,0x31,0x38,0x2a,0x79,0x37,0x36,0x2d,0x79,0x3b,0x3c,0x3c,0x37,0x79,0x30,0x34,0x29,0x35,0x3c,0x34,0x3c,0x37,0x2d,0x3c,0x3d]

/*: "img_home_shadow" :*/
fileprivate let user_backMainName:String = "to"
fileprivate let app_indexMsg:String = "g_homesession count else"

/*: "#9610FF" :*/
fileprivate let userValueTitle:String = "#9610FFinfo count"

/*: "#8566FF" :*/
fileprivate let constUserData:[Character] = ["#","8","5","6","6","F","F"]

/*: "Random Video" :*/
fileprivate let appViewPath:String = "Randomleading add color name"
fileprivate let const_levelTitleStr:String = "to message in Video"

/*: "icon_match_call_free" :*/
fileprivate let dataAtValue:[Character] = ["i","c","o","n","_","m","a","t"]
fileprivate let constArrayStr:String = "ch_calsuper leading manager"

/*: "Free" :*/
fileprivate let userDataFormat:String = "label net colorFree"

/*: "Scroll_upto_discove" :*/
fileprivate let userAddData:[Character] = ["S","c","r","o","l","l","_","u","p","t","o","_","d","i"]
fileprivate let notiThumbTitle:[Character] = ["s","c","o","v","e"]

/*: "icon_VectorStroke" :*/
fileprivate let main_barText:String = "icon_success label"
fileprivate let data_viewCellText:String = "talk center touchStroke"

/*: "get json error" :*/
fileprivate let app_layerUrl:String = "gadd"
fileprivate let constRawName:[Character] = ["t"," ","j","s","o","n"," ","e","r","r","o","r"]
...
class GradePointCardView: UIView {
    ...
    var maxRemoveDistance = notiEnabledRecordUrl / 4.0 /// 最大移除距离(默认屏幕的1/4)
    //: var reusableCells = [TalkingClubCardViewCell]()
    var reusableCells = [PathView]() /// 重用卡片数组
    //: var isAlpha = true
    ...
    var dataSource: PlaceOfOriginObjectProtocol? /// 数据源
    //: var delegate: TalkingClubCardViewDelegate?
    var delegate: CountObjectProtocol?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiMakeName.map{$0^89}, encoding: .utf8)!)
    }
    ...
    override func didMoveToSuperview() {
        ...
        addData()
            ...
            if (freeImag.contentMode == .top) && (freeImag.layer.position.y == 33.76) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let streamAge = PlayerVideoView(frame: freeImag.bounds.union(CGRect(x: CGFloat(203.87), y: CGFloat(530.48), width: CGFloat(611.41), height: CGFloat(69))))
            streamAge.messageTitleTotal = maxAngle
            streamAge.resourceContent = identifier
            
                freeImag.addSubview(streamAge)
            }
    ...
    private lazy var leftbackImageView: UIImageView = {
        ...
        imag.image = UIImage.addBar(name: (user_backMainName.replacingOccurrences(of: "to", with: "im") + String(app_indexMsg.prefix(6)) + "_shadow"))
    ...
    private lazy var randomBtn: UIButton = {
        ...
        btn.setBackgroundImage(UIImage.constraintWindow(colors: [UIColor(hex: (String(userValueTitle.prefix(7))))!.cgColor, UIColor(hex: (String(constUserData)))!.cgColor], size: CGSize(width: notiEnabledRecordUrl - 30, height: 54)), for: .normal)
        //: btn.layer.cornerRadius = 27
        btn.layer.cornerRadius = 27
        //: btn.layer.masksToBounds = true
        btn.layer.masksToBounds = true
        //: btn.setTitle("Random Video".localized, for: .normal)
        btn.setTitle((String(appViewPath.prefix(6)) + String(const_levelTitleStr.suffix(6))).localized, for: .normal)
        //: btn.addTarget(self, action: #selector(randomBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(snapVideoClick), for: .touchUpInside)
        //: self.addSubview(btn)
        self.addSubview(btn)
    ...
    lazy var freeImag: UIButton = {
        ...
        img.setBackgroundImage(UIImage.addBar(name: (String(dataAtValue) + String(constArrayStr.prefix(6)) + "l_free")), for: .normal)
        //: img.setTitle("Free".localized, for: .normal)
        img.setTitle((String(userDataFormat.suffix(4))).localized, for: .normal)
...
extension GradePointCardView {
    ...
    private func guideRecord() {
        ...
        image.image = UIImage.addBar(name: (String(userAddData) + String(notiThumbTitle)))
        ...
        image2.image = UIImage.addBar(name: (String(main_barText.prefix(5)) + "Vector" + String(data_viewCellText.suffix(6))))
        ...
        panoramicViewPlayer(player: player)
    }

    //: private func likeplayer(player: SVGAPlayer) {
    private func panoramicViewPlayer(player: SVGAPlayer) {
        //: do {
        do {
            //: let url = SVGAEffectTool.default.getZipEffectPath(type: .Club_guidePage)
            let url = ShadowEffectTool.default.varietyButton(type: .Club_guidePage)
            ...
            printLog(message: (app_layerUrl.replacingOccurrences(of: "add", with: "e") + String(constRawName)))
        }
    }

    //: @objc private func randomBtnClick() {
    @objc private func snapVideoClick() {
        //: self.delegate?.clubCardRandowBtnClick?()
        self.delegate?.appClick?()
    }

    //: func reloadDataAnimated(anim: Bool) {
    func effectOff(anim: Bool) {
        //: moveCount = 0
        moveCount = 0
        //: self.currentIndex = 0
        self.currentIndex = 0
        ...
        let maxCount = dataSource?.lastPunchCardRemove(cardView: self) ?? 0
    ...
    func futurismItem(index: Int) {
        //: let cell = dataSource?.cardViewCellForRowAtIndex(cardView: self, index: index)
        let cell = dataSource?.blockSerration(cardView: self, index: index)
...
//        let showCount = Double(self.visibleCount - 1)
        ...
        containerView.insertSubview(cell ?? PathView(), at: 0)
    ...
    func addMedium(anim: Bool) {
        ...
        for i in 0 ... count - 1 {
            ...
            let cell: PathView = self.visibleCells[i] as! PathView
            ...
            if isVisbleLast {
                ...
                self.delegate?.rowEqualIndex?(cardViewCell: cell, forRowAtIndex: self.currentIndex - i)
            }

            //: if anim {
            if anim {
                //: updateConstraintsCell(cell: cell, transform: transform)
                addTransform(cell: cell, transform: transform)
                //: } else {
            } else {
    ...
    func addTransform(cell: PathView, transform: CGAffineTransform) {
    ...
    func destroyEnroll(cellClass: AnyClass, forellIdentifier: String) {
    ...
    func streetSmartDetail(identifier: String) -> PathView? {
        ...
        if self.cellClass != nil {
            ...
            let cellclass = self.cellClass as! CardItemCell.Type
            //: let cell: TalkingClubCardItemCell = cellclass.init(reuseIdentifier: identifier )
            let cell: CardItemCell = cellclass.init(reuseIdentifier: identifier)
...
extension GradePointCardView: DescriptionCellDelegete {
    ...
    func guest(cell: PathView) {
        ...
        self.delegate?.backIndex?(cardView: self, cardViewCell: cell, forRowAtIndex: self.currentFirstIndex - 1)

        //: let count = self.dataSource?.numberOfCountInCardView(cardView: self) ?? 0
        let count = self.dataSource?.lastPunchCardRemove(cardView: self) ?? 0
        // 移除后的卡片是最后一张(没有更多)
        ...
        if self.visibleCells.count == 0 { // 只有最后一张卡片的时候
            ...
            self.delegate?.delayIndex?(cardViewCell: cell, forRowAtIndex: self.currentIndex)
            //: return
            return
        }
        // 当前数据源还有数据 继续创建cell
        ...
        if self.currentIndex < count - 1 {
            ...
            futurismItem(index: self.currentIndex + 1)
        }
        // 更新布局
    ...
    func asBackgroundPoint(cell: PathView, forMovePoint: CGPoint) {
        //: self.delegate?.didMoveCell?(cardView: self, cardViewCell: cell, forMovePoint: forMovePoint)
        self.delegate?.tradingCard?(cardView: self, cardViewCell: cell, forMovePoint: forMovePoint)
    }
}
