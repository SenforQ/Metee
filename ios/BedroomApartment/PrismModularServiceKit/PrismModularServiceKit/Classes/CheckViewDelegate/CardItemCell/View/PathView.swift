...
fileprivate let kCellMessage:[UInt8] = [0x66,0x61,0x66,0x7b,0x27,0x6c,0x60,0x6b,0x6a,0x7d,0x35,0x26,0x2f,0x67,0x6e,0x7c,0x2f,0x61,0x60,0x7b,0x2f,0x6d,0x6a,0x6a,0x61,0x2f,0x66,0x62,0x7f,0x63,0x6a,0x62,0x6a,0x61,0x7b,0x6a,0x6b]

private func equalRecord(success num: UInt8) -> UInt8 {
    return num ^ 15
}
...
enum BeginReadCaseIterable: CaseIterable {
    //: case Left
    case Left
    //: case Right
    case Right
...
protocol DescriptionCellDelegete: NSObjectProtocol {
    ///
...
class PathView: UIView {
    ...
    var cell_delegate: DescriptionCellDelegete?

    //: var model: TalkingMatchResultModel?
    var model: VoiceHandyJSON?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kCellMessage.map{equalRecord(success: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(thrusting(pan:)))
        //: v.delegate = self
        v.delegate = self
        //: return v
        return v
...
extension PathView: UIGestureRecognizerDelegate {
    ...
    @objc func thrusting(pan: UIPanGestureRecognizer) {
        //: switch pan.state {
        switch pan.state {
        //: case .began:
        case .began:
        ...
        case .changed:
            ...
            if AddBeautyThen.shared.direction == .rightToLeft {
                ...
                if movePoint.x > notiEnabledRecordUrl / 4 {
                    ...
                    self.cell_delegate?.asBackgroundPoint(cell: self, forMovePoint: self.currentPoint)
    ...
    func toastingWithPreEmptionRight() {
        // 左滑移除
        //: if (self.currentPoint.x > self.maxRemoveDistance) {
        if self.currentPoint.x > self.maxRemoveDistance {
            //: let snapshotView = self.snapshotView(afterScreenUpdates: false)
    ...
    func decarbonise() {
        ...
        self.cell_delegate?.guest(cell: self)
    }
    ...
    func allRow(direction: BeginReadCaseIterable) {
        //: switch (direction) {
        switch direction {
        //: case .Left:
        case .Left:
        ...
        case .Right:
            ...
            fromPreEmptionScoopUpSuperviewRight()
        }
    }
    ...
    func fromPreEmptionScoopUpSuperviewRight() {
        //: let snapshotView = self.snapshotView(afterScreenUpdates: false)
        let snapshotView = self.snapshotView(afterScreenUpdates: false)
