...
/*: "position.x" :*/
fileprivate let main_profilePath:String = "pinsidesi"
fileprivate let k_objectTitle:String = "menu margin height totion.x"

/*: "slideIn" :*/
fileprivate let mainStatusDataKey:String = "slideInnumber equal start"

/*: "slideOut" :*/
fileprivate let main_hideTitle:[Character] = ["s","l","i","d","e","O","u"]
fileprivate let appAddMakePath:String = "live"
...
class MakeViewManager: NSObject {
    //: static let shared = GiftFloatViewManager()
    static let shared = MakeViewManager()
    //: private var giftArr = [SocialBroadcastModel]()
    private var giftArr = [FromHandyJSON]()
    ...
    private lazy var giftView: ActionTextThen = {
        //: let v = GiftFloatView()
        let v = ActionTextThen()
        //: ColorDelimitateMacroDefine.getWindow().addSubview(v)
        ColorDelimitateMacroDefine.beforeText().addSubview(v)
        //: return v
        return v
        //: }()
    }()
...
extension MakeViewManager {
    ...
    private func atValueAnimation(_ model: FromHandyJSON) {
        ...
        let isRTL = AddBeautyThen.shared.direction == .rightToLeft
        // 起始位置：屏幕外
        //: let startCenterX = isRTL ? -centerX : ScreenWidth + centerX
        let startCenterX = isRTL ? -centerX : notiEnabledRecordUrl + centerX
        // 目标位置：屏幕中间
        //: let targetCenterX = ScreenWidth / 2
        let targetCenterX = notiEnabledRecordUrl / 2
        // 结束位置：屏幕另一侧
        //: let endCenterX = isRTL ? ScreenWidth + centerX : -centerX
        let endCenterX = isRTL ? notiEnabledRecordUrl + centerX : -centerX
        ...
        let slideIn = CABasicAnimation(keyPath: (main_profilePath.replacingOccurrences(of: "inside", with: "o") + String(k_objectTitle.suffix(6))))
        ...
        self.giftView.layer.add(slideIn, forKey: (String(mainStatusDataKey.prefix(7))))
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) { [weak self] in
            ...
            let slideOut = CABasicAnimation(keyPath: (main_profilePath.replacingOccurrences(of: "inside", with: "o") + String(k_objectTitle.suffix(6))))
            ...
            self.giftView.layer.add(slideOut, forKey: (String(main_hideTitle) + appAddMakePath.replacingOccurrences(of: "live", with: "t")))
...
extension MakeViewManager {
    ...
    func lastTitle() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.prevalence(noti:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.indexComplete),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.pastNow),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.brunswick),
    ...
    @objc private func prevalence(noti: Notification) {
        ...
        guard let data = noti.object,
              ...
              let gift = JSONDeserializer<FromHandyJSON>.deserializeFrom(dict: data as? [String: Any], designatedPath: nil),
              //: gift.broadType != BroadcastType.Unknow.rawValue
              gift.broadType != EffectConstantTarget.Unknow.rawValue
        //: else {
        else {
            //: return
            return
        }
        //: let currUserType = RegularBeginDecisionMakerAppManager.share.appUserConfigMode.userType
        let currUserType = RegularBeginDecisionMakerAppManager.share.appUserConfigMode.userType
        // 过滤接口异常
        ...
        // 非特殊用户 不展示 不同地区用户礼物
        //: if currUserType != 0, gift.userCountryType != RegularBeginDecisionMakerAppManager.share.appUserConfigMode.userCountryType {
        if currUserType != 0, gift.userCountryType != RegularBeginDecisionMakerAppManager.share.appUserConfigMode.userCountryType {
            //: return
            return
        }
        ...
        self.startRange(gift: gift)
    }

    /// 应用退到后台
    //: @objc private func enterBackground() {
    @objc private func indexComplete() {
