...
/*: "#777777" :*/
fileprivate let app_objectPath:String = "#777777"

/*: "#333333" :*/
fileprivate let constLineStr:String = "#loglogloglogloglog"

/*: "Search" :*/
fileprivate let k_equalText:String = "cancel"
fileprivate let noti_fadeUserName:String = "EARCH"

/*: "Details" :*/
fileprivate let app_delayName:[Character] = ["D","e","t","a","i"]
fileprivate let constBottomMakeName:String = "gift"

/*: "nav_back_black_nor" :*/
fileprivate let k_sizeMeanData:[Character] = ["n","a","v","_","b","a","c","k","_","b","l","a","c","k","_","n","o"]
fileprivate let data_contentStr:String = "R"
...
class ReplyViewDelegate: ScoreViewController {
	var sizeImageCount: Int = 97
	var addTotal: Double = 14.7
	var iconArray: [AnyHashable] = []
	var awardDictionary: [AnyHashable: String] = [:]
    ...
    override func viewDidLoad() {
        ...
        sizeObserver()
            ...
            if (backButton.alignmentRectInsets.right == 14) && (backButton.sizeThatFits(.zero).width == 52.81) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let atType = LiveBeanView()
            
            atType.messageNumber = { [self] modelChangeQuantity in
            self.sizeImageCount = modelChangeQuantity
            
            return self.sizeImageCount
            }
            atType.awayNumber = { [self] eventNumber in
            self.addTotal = eventNumber
            
            self.addTotal /= 2
            return self.addTotal
            }
            atType.imageArray = { [self] logArray in
            self.iconArray = logArray
            ...
            self.awardDictionary = ofDictionary
            
            guard var value = self.awardDictionary as? [String: String] else {
                return nil
            }
            return value
            }
                backButton.addSubview(atType)
            }
    ...
    private lazy var categoryView: JXSegmentedView = {
        ...
        segmentedDataSource.titleNormalColor = UIColor(hex: (app_objectPath.capitalized))!
        //: segmentedDataSource.titleSelectedColor = UIColor.init(hex: "#333333")!
        segmentedDataSource.titleSelectedColor = UIColor(hex: (constLineStr.replacingOccurrences(of: "log", with: "3")))!
        ...
        indicator.indicatorColor = UIColor(hex: (constLineStr.replacingOccurrences(of: "log", with: "3")))!
...
//        view.defaultSelectedIndex = currentIndex
    ...
    private lazy var titleArray: [String] = //: return ["Search".localized, "Details".localized]
        [(k_equalText.replacingOccurrences(of: "cancel", with: "S") + noti_fadeUserName.lowercased()).localized, (String(app_delayName) + constBottomMakeName.replacingOccurrences(of: "gift", with: "ls")).localized]
    //: }()

    //: private lazy var dataVC: [TalkingBaseViewController] = {
    private lazy var dataVC: [ScoreViewController] = //: return [SizeViewController(), ClickViewController()]
        [SizeViewController(), ClickViewController()]
    //: }()
    ...
    private lazy var backButton: UIButton = {
        ...
        let img = UIImage.addBar(name: (String(k_sizeMeanData) + data_contentStr.lowercased())).withRenderingMode(.alwaysTemplate)
...
extension ReplyViewDelegate: JXSegmentedViewDelegate {
...
extension ReplyViewDelegate: JXPagingViewDelegate {
