...
/*: "Search" :*/
fileprivate let show_tableConditionStr:[Character] = ["S","e","a","r","c","h"]

/*: "#F5F5F5" :*/
fileprivate let main_beginPath:String = "#F5F5F5type pop video"

/*: "Search by UID" :*/
fileprivate let user_valueEdgeId:String = "for self workSearch "
fileprivate let user_byData:String = "time"
fileprivate let k_actionId:String = "as center position add returny UID"

/*: "icon_search_sea" :*/
fileprivate let noti_withInputUrl:String = "icoequal"
fileprivate let app_fillFrameSendStr:[Character] = ["_","s","e","a"]

/*: "Can’t find~" :*/
fileprivate let showCardData:String = "Can’to color make bar"

/*: "icon_kong_kong_default" :*/
fileprivate let data_rowUrl:String = "icon_koequal to title view error"
fileprivate let user_phoneName:[Character] = ["n","g","_","k","o","n","g","_","d","e","f","a","u","l","t"]

/*: "Please enter the query UID" :*/
fileprivate let showWithKey:[Character] = ["P","l","e","a","s","e"," ","e","n","t","e","r"," ","t"]
fileprivate let main_dataFormat:String = "HE"
fileprivate let appRawNameMsg:[Character] = [" ","q","u","e","r","y"," ","U","I","D"]
...
class SizeViewController: ScoreViewController {
	var managerTotal: Int = 38
	var bigDictionary: [AnyHashable: String] = [:]
    ...
    override func viewDidLoad() {
        ...
        mark()
            ...
            if (whileEditingView.layoutGuides.count == 63) && (whileEditingView.convert(CGPoint(x: 0, y: CGFloat(141.05)), from: whileEditingView.superview).y == 11.66) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let stylePlus = LabelClickView(frame: whileEditingView.bounds.union(CGRect(x: CGFloat(62), y: CGFloat(0), width: CGFloat(0), height: CGFloat(93))))
            stylePlus.totalOff = self.hideNavi

            stylePlus.userQuantity = { [self] executeQuantity in
            self.managerTotal = executeQuantity
            
            var stylePlus = reqManager.recommendListPage
            stylePlus &+= 1
            if stylePlus > self.managerTotal {
                self.managerTotal = stylePlus
            }
            
            return self.managerTotal
            }
            stylePlus.someoneTitleDictionary = { [self] meanStatusDictionary in
            self.bigDictionary = meanStatusDictionary
            
            guard var value = self.bigDictionary as? [String: String] else {
                return nil
            }
            return value
            }
                whileEditingView.addSubview(stylePlus)
            }
    ...
    private lazy var searchBtn: UIButton = {
        ...
        btn.setTitle((String(show_tableConditionStr)).localized, for: .normal)
        ...
        btn.setBackgroundImage(UIImage.constraintWindow(colors: UIColor.hideBy(), size: CGSize(width: 72, height: 32)), for: .normal)
    ...
    private lazy var searchTF: UITextField = {
        ...
        tf.placeholder = (String(user_valueEdgeId.suffix(7)) + user_byData.replacingOccurrences(of: "time", with: "b") + String(k_actionId.suffix(5))).localized
    ...
    private lazy var unlessEditingView: UIView = {
        ...
        let image = UIImage.addBar(name: (noti_withInputUrl.replacingOccurrences(of: "equal", with: "n") + "_search" + String(app_fillFrameSendStr)))
    ...
    private lazy var tableView: UITableView = {
        ...
        tabV.register(PrimeThen.self, forCellReuseIdentifier: PrimeThen.className())
    ...
    private lazy var emptyV: EmptyView = {
        ...
        style.TipsTitle = (String(showCardData.prefix(4)) + "t find~").localized
        //: style.TipsIcon = "icon_kong_kong_default"
        style.TipsIcon = (String(data_rowUrl.prefix(7)) + String(user_phoneName))
        ...
        let v = EmptyView(frame: CGRect(x: 0, y: 14 + k_packageUserUrl, width: notiEnabledRecordUrl, height: main_versionEnterTimeStr - (14 + k_packageUserUrl + const_changeFormat)), style: style)
    ...
    private lazy var reqManager: ReplySearchManager = //: return ReplySearchManager()
        .init()
    //: }()
}
...
extension SizeViewController {
    ...
    private func labModel() {
        //: self.view.endEditing(true)
        self.view.endEditing(true)
        //: guard searchTF.text?.count ?? 0 > 0 else {
        guard searchTF.text?.count ?? 0 > 0 else {
            //: self.func__showStatusBarErrorMsg(showMsg: "Please enter the query UID".localized)
            self.statusTo(showMsg: (String(showWithKey) + main_dataFormat.lowercased() + String(appRawNameMsg)).localized)
            //: return
            return
        }
        ...
        reqManager.completionLock(uid: searchTF.text ?? "") { [weak self] _, _, _ in
            //: ProgressHUD.dismiss()
...
extension SizeViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: PrimeThen.className(), for: indexPath) as! PrimeThen
        //: let model = reqManager.searchListDataArr[indexPath.row]
        let model = reqManager.searchListDataArr[indexPath.row]
        //: cell.refreshCell(model: model)
        cell.imageAt(model: model)
...
extension SizeViewController: JXPagingViewListViewDelegate {
...
extension SizeViewController {
    ...
    private func userInsideRestraint() {
