...
fileprivate let mainModelTitle:[UInt8] = [0xa3,0xa6,0x9a,0x98,0xab,0xa0,0xa6,0xa5,0x57,0x98,0xac,0xab,0x9f,0xa6,0xa9,0xa0,0xb1,0x98,0xab,0xa0,0xa6,0xa5,0x8a,0xab,0x98,0xab,0xac,0xaa,0x57,0xa0,0xaa,0x57]

fileprivate func viewManager(data num: UInt8) -> UInt8 {
...
fileprivate let showPathData:[Character] = ["g","e","t"," ","l","o","c","a","t","i","o","n"," ","f","a","i","l","e","d","."," ","e","r","r","o","r",":"]
...
// __CLOSE_PRINT__
//
//  HeadButtonLocationManager.swift
//  AbroadTalking
//
...
class HeadButtonLocationManager: NSObject {
    ...
    var locationType: PathPriorityTarget?

    // singleton
    //: private static var _instance: TalkingLocationManager?
    private static var _instance: HeadButtonLocationManager?
    //: class func shared() -> TalkingLocationManager {
    class func goAcross() -> HeadButtonLocationManager {
        //: guard _instance != nil else {
        guard _instance != nil else {
            //: _instance = TalkingLocationManager()
            _instance = HeadButtonLocationManager()
            //: return _instance!
            return _instance!
        }
        //: return _instance!
    ...
    func itemComponent(block: @escaping Locationblock) {
    ...
    func userBlock(block: @escaping Authblock) {
    ...
    func locationWithFieldService() -> Bool {
    ...
    func east() -> CLAuthorizationStatus {
        ...
        if #available(iOS 14.0, *) {
            ...
            printLog(message: String(bytes: mainModelTitle.map{viewManager(data: $0)}, encoding: .utf8)! + "\(status.rawValue)")
            //: return status
            return status
            //: } else {
        } else {
...
extension HeadButtonLocationManager: CLLocationManagerDelegate {
...
//            guard abs(loction.timestamp.timeIntervalSinceNow) > 5 else { return }
    ...
    func locationManager(_: CLLocationManager, didFailWithError error: Error) {
        ...
        printLog(message: (String(showPathData)) + "\(error.localizedDescription)")
    }
}
