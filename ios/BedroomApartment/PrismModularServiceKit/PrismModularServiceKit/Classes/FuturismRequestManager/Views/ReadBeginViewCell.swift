...
fileprivate let dataGiftId:[UInt8] = [0xe,0x9,0xe,0x13,0x4f,0x4,0x8,0x3,0x2,0x15,0x5d,0x4e,0x47,0xf,0x6,0x14,0x47,0x9,0x8,0x13,0x47,0x5,0x2,0x2,0x9,0x47,0xe,0xa,0x17,0xb,0x2,0xa,0x2,0x9,0x13,0x2,0x3]

/*: "icon_rank_coin" :*/
fileprivate let userShowData:[UInt8] = [0x14,0x1e,0x12,0x13,0x22,0xf,0x1c,0x13,0x16,0x22,0x1e,0x12,0x14,0x13]

/*: "icon_lounge" :*/
fileprivate let noti_ofName:String = "icon_llet action view color"

/*: "icon_rank_coin2" :*/
fileprivate let k_modelMsg:String = "item arrayicon_r"
...
class ReadBeginViewCell: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dataGiftId.map{$0^103}, encoding: .utf8)!)
    }
    ...
    lazy var rankLabel = UILabel().then {
        ...
        $0.textColor = UIColor.outEqual()
        //: $0.font = .pingfangFont(type: .Semibold, fontSize: 20)
        $0.font = .pingfangCenter(type: .Semibold, fontSize: 20)
    }
    ...
    lazy var coinIconBtn = QuantityervalFirstButton().then {
        //: $0.setImage(UIImage.BundleImageNamed(name: "icon_rank_coin"), for: .normal)
        $0.setImage(UIImage.addBar(name: String(bytes: userShowData.map{$0^125}, encoding: .utf8)!), for: .normal)
        //: $0.setTitleColor(UIColor.white, for: .normal)
        $0.setTitleColor(UIColor.white, for: .normal)
        //: $0.titleLabel?.font = .pingfangFont(type: .Medium, fontSize: 15)
        $0.titleLabel?.font = .pingfangCenter(type: .Medium, fontSize: 15)
        ...
        $0.setBackgroundImage(UIImage.constraintWindow(colors: UIColor.hideBy(), size: CGSize(width: actualWidth(w: 80), height: actualWidth(w: 33))), for: .normal)
    ...
    lazy var loungeImgV = UIImageView().then {
        ...
        $0.image = UIImage.addBar(name: (String(noti_ofName.prefix(6)) + "ounge"))
    }
}
...
extension ReadBeginViewCell {
    //: func updateRankCell(model: SocialRankItemModel, index: Int) {
    func cameraFile(model: NumberTransformable, index: Int) {
        ...
        headPic.step(urlStr: model.headPic)
    ...
    func shackUp(type: Int) {
        //: let imageStr = type == 0 ? "icon_rank_coin":"icon_rank_coin2"
        let imageStr = type == 0 ? String(bytes: userShowData.map{$0^125}, encoding: .utf8)! : (String(k_modelMsg.suffix(6)) + "ank_coin2")
        //: coinIconBtn.setImage(UIImage.BundleImageNamed(name: imageStr), for: .normal)
        coinIconBtn.setImage(UIImage.addBar(name: imageStr), for: .normal)
    }
}
