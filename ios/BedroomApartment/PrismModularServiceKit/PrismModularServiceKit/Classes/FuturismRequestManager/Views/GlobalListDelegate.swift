...
fileprivate let k_modelUrl:[UInt8] = [0x74,0x73,0x74,0x69,0x35,0x7e,0x72,0x79,0x78,0x6f,0x27,0x34,0x3d,0x75,0x7c,0x6e,0x3d,0x73,0x72,0x69,0x3d,0x7f,0x78,0x78,0x73,0x3d,0x74,0x70,0x6d,0x71,0x78,0x70,0x78,0x73,0x69,0x78,0x79]

/*: "Not yet~" :*/
fileprivate let showLogMsg:[Character] = ["N","o"]
fileprivate let dataImageId:[Character] = ["t"," ","y","e","t","~"]

/*: "#8C7AFF" :*/
fileprivate let noti_playerTransformMessage:[Character] = ["#","8","C","7","A","F","F"]
...
class GlobalListDelegate: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_modelUrl.map{$0^29}, encoding: .utf8)!)
    }
    ...
    override func layoutSubviews() {
        ...
        ranktableView.frame = self.bounds
            ...
            if (tableHeaderView.layer.anchorPointZ != 0) && (tableHeaderView.motionEffects.count == 17) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let tagOf = ClipBlockView()
            
            tagOf.donationMoveOff = { [self] narrowClose in
            self.infoEnable = narrowClose
            
            return self.infoEnable
            }
            tagOf.sideSlipArray = { [self] backOfArray in
            self.statusArray = backOfArray
            ...
            self.textDictionary = fromAgeDictionary
            ...
            return value
                ...
                tableHeaderView.addSubview(tagOf)
            }
    ...
    lazy var dataArray: [NumberTransformable] = [] {
        //: didSet {
        didSet {
    ...
    lazy var ranktableView = UITableView(frame: self.bounds, style: .plain).then {
        ...
        $0.register(AddRoundedCell.self, forCellReuseIdentifier: AddRoundedCell.className())
        //: $0.register(SocialRankListViewCell.self, forCellReuseIdentifier: SocialRankListViewCell.className())
        $0.register(ReadBeginViewCell.self, forCellReuseIdentifier: ReadBeginViewCell.className())
    }
    ...
    lazy var tableHeaderView = ControlTrackReactiveCompatible(frame: CGRect(x: 0, y: 0, width: notiEnabledRecordUrl, height: actualHeight(h: 220 + 14))).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.index = self.type
        $0.index = self.type
    ...
    lazy var tableBackgrourdView = ForefrontReactiveCompatible().then {
        ...
        $0.desLab.text = (String(showLogMsg) + String(dataImageId)).localized
        //: $0.desLab.textColor = UIColor(hex: "#8C7AFF")
        $0.desLab.textColor = UIColor(hex: (String(noti_playerTransformMessage)))
...
extension GlobalListDelegate: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: AddRoundedCell.className(), for: indexPath) as! AddRoundedCell
            //: return cell
            return cell
        }
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: ReadBeginViewCell.className(), for: indexPath) as! ReadBeginViewCell
        //: let model = dataArray[indexPath.row+3]
        let model = dataArray[indexPath.row + 3]
        //: cell.updateRankCell(model: model, index: indexPath.row+3)
        cell.cameraFile(model: model, index: indexPath.row + 3)
        //: cell.updateIcon(type: self.type)
        cell.shackUp(type: self.type)
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        if indexPath.section == 0 {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: AddRoundedCell.className()) as! AddRoundedCell
...
extension GlobalListDelegate: JXSegmentedListContainerViewListDelegate {
...
extension GlobalListDelegate {
    //: private func setupSubviews() {
    private func sharedRelease() {
