...
fileprivate let appArrayTitle:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "img_match_shadow" :*/
fileprivate let data_pointLastRecordStr:String = "view letimg_mat"
fileprivate let showSumervalUrl:[Character] = ["c","h","_","s","h","a","d","o","w"]

/*: "icon_home_girl" :*/
fileprivate let userEqualPhotoKey:String = "label if type add anyicon_hom"
fileprivate let user_progressKey:String = "e_girlif style model"

/*: "icon_home_v" :*/
fileprivate let kBetweenData:String = "to tag false iconicon_h"

/*: "btn_dynamic_stop_nor" :*/
fileprivate let appVideoFormat:[Character] = ["b","t","n","_","d","y"]
fileprivate let notiTempFormat:String = "namic_sinfo self time succeed"
fileprivate let app_kitMessage:[Character] = ["t","o","p","_","n","o","r"]

/*: "#2DF2FF" :*/
fileprivate let showResumeUrl:[Character] = ["#","2","D","F","2","F","F"]

/*: "· :*/
fileprivate let user_genderFormat:String = "view"

/*: "img_home_shadow" :*/
fileprivate let user_cellFormat:String = "img_hoheight to live self"
fileprivate let user_upData:String = "amakeow"

/*: "icon_home_boy" :*/
fileprivate let appToPath:[Character] = ["i","c","o","n","_","h","o"]
fileprivate let app_norId:String = "collection let interaction returnme_boy"

/*: "GlobalUserReactiveCompatible" :*/
fileprivate let notiDataTitle:String = "model model timeTUserDet"
fileprivate let user_recordImageUrl:[Character] = ["a","i","l","H","e","a","d"]
fileprivate let user_mMsg:[Character] = ["e","r","I","t","e","m","C","e","l","l"]

/*: "#FFFFFF" :*/
fileprivate let mainFailPath:String = "#FFFFFFrecord index main equal"
...
class NameDataSource: UIView {
    ...
    private var model = VoiceHandyJSON()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: appArrayTitle.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var gradientShadowImageView: UIImageView = {
        ...
        imageView.image = UIImage.addBar(name: (String(data_pointLastRecordStr.suffix(7)) + String(showSumervalUrl)))
        //: return imageView
        return imageView
        //: }()
    }()
    ...
    private lazy var sexImgV: UIImageView = {
        ...
        img.image = UIImage.addBar(name: (String(userEqualPhotoKey.suffix(8)) + String(user_progressKey.prefix(6))))
    ...
    private lazy var authImgV: UIImageView = {
        ...
        img.image = UIImage.addBar(name: (String(kBetweenData.suffix(6)) + "ome_v"))
        //: img.isUserInteractionEnabled = true
        img.isUserInteractionEnabled = true
        //: return img
        return img
    ...
    private lazy var ageLab: UILabel = {
        ...
        lab.font = UIFont.pingfangCenter(type: .Medium, fontSize: 11)
        //: lab.textColor = .white
        lab.textColor = .white
        //: return lab
        return lab
    ...
    lazy var playImageView: UIImageView = {
        ...
        let imag = UIImageView(image: UIImage.addBar(name: (String(appVideoFormat) + String(notiTempFormat.prefix(7)) + String(app_kitMessage))))
        //: imag.isHidden = true
        imag.isHidden = true
        //: return imag
        return imag
...
//        v.roundBgColor = UIColor.white
        ...
        v.progressColor = UIColor(hex: (String(showResumeUrl)))!
        //: return v
        return v
        //: }()
    }()
...
extension NameDataSource {
    /// 更新视图UI
    /// - Parameter model: 数据模型
    //: func updatePicturesView(model: TalkingMatchResultModel) {
    func fillUpModel(model: VoiceHandyJSON) {
        //: self.model = model
        self.model = model
        ...
        userImg.equalFinish(urlStr: model.headPic, placeImg: UIImage.addBar(name: (String(user_cellFormat.prefix(6)) + "me_sh" + user_upData.replacingOccurrences(of: "make", with: "d"))))
        //: if model.sex == Gender.male.rawValue {
        if model.sex == ScoreScalarLiteral.male.rawValue {
            //: sexImgV.image = UIImage.BundleImageNamed(name: "icon_home_boy")
            sexImgV.image = UIImage.addBar(name: (String(appToPath) + String(app_norId.suffix(6))))
            //: } else {
        } else {
            //: sexImgV.image = UIImage.BundleImageNamed(name: "icon_home_girl")
            sexImgV.image = UIImage.addBar(name: (String(userEqualPhotoKey.suffix(8)) + String(user_progressKey.prefix(6))))
        }
        ...
        self.countdownTimer = Timer.scheduledTimer(withTimeInterval: 0.1, block: { [weak self] _ in
            ...
            self.progressView.imageBy(totalTime: self.MatchResultProgress_Timer, currTime: time)
            //: time -= 1
            time -= 1
            //: }, repeats: true)
        }, repeats: true)
...
extension NameDataSource: JXBannerDataSource {
    ...
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        ...
        return JXBannerCellRegister(type: GlobalUserReactiveCompatible.self, reuseIdentifier: (String(notiDataTitle.suffix(8)) + String(user_recordImageUrl) + String(user_mMsg)))
    }
    ...
    func jxBanner(_: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
        ...
        if index == 0 && self.model.videoUrl.count > 1 { // 视频
            ...
            bannerCell.toTing(videoUrl: self.model.videoUrl, imgUrl: self.model.cover)
            /// 自动播放完视频后滑动到下一个
...
//                }
            ...
            bannerCell.bgImgView.equalFinish(urlStr: str, placeImg: UIImage.gnatTop())
        }
    ...
    func jxBanner(pageControl _: JXBannerType, numberOfPages _: Int, coverView: UIView, builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder {
        ...
        pageControl.activeColor = UIColor(hex: (String(mainFailPath.prefix(7))))!.withAlphaComponent(1.0)
        //: pageControl.inactiveColor = UIColor(hex: "#FFFFFF")!.withAlphaComponent(0.6)
        pageControl.inactiveColor = UIColor(hex: (String(mainFailPath.prefix(7))))!.withAlphaComponent(0.6)
...
extension NameDataSource: JXBannerDelegate {
