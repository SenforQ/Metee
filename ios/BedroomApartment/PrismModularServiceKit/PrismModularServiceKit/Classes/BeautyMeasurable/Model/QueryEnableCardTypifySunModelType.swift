...
/*: "id" :*/
fileprivate let mainEqualUrl:[Character] = ["i","d"]
...
// __CLOSE_PRINT__
//
//  QueryEnableCardTypifySunModelType.swift
//  AbroadTalking
//
...
struct QueryEnableCardTypifySunModelType: HandyJSON {
    //: var uid: String?
    var uid: String?
    //: var sex: String?
    var sex: String?
...
struct DotTransformable: HandyJSON {
    //: var thumbUrl: String?
    var thumbUrl: String?
    //: var imgUrl: String?
    var imgUrl: String?
...
enum SpecklessValueConvertible: String {
    //: case nones  = "0"
    case nones = "0"
    //: case Photo = "1"
    case Photo = "1"
...
struct TableVideoModel: HandyJSON {
    //: var videoUrl = ""
    var videoUrl = ""
    //: var imgUrl = ""
    var imgUrl = ""
    ...
    var status: ScoreEquatable?
}

///
//: enum MomentVideoStatus: String {
enum ScoreEquatable: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension QueryEnableCardTypifySunModelType {
    //: mutating func caculateItemHeight(isPost: Bool = false) {
    mutating func beginLevel(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            textHeight = Int(ReadPlayReactiveCompatible.withQuantity(text: content!, size: CGSize(width: notiEnabledRecordUrl - 54, height: CGFloat(MAXFLOAT)), font: UIFont.pingfangCenter(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == SpecklessValueConvertible.Photo.rawValue || momentType == SpecklessValueConvertible.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
