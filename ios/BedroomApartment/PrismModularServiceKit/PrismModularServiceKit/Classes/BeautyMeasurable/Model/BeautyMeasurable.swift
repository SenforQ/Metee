...
/*: "id" :*/
fileprivate let userContextMsg:[Character] = ["i","d"]

/*: "reply" :*/
fileprivate let data_makeId:[Character] = ["r","e","p","l","y"]

/*: :  :*/
fileprivate let noti_buttonStr:String = ": "

/*:  : :*/
fileprivate let constDataMessage:String = " size"
...
struct BeautyMeasurable: HandyJSON {
    //: var cid: String?
    ...
    var comment = [PriceTransformable]()
...
extension BeautyMeasurable {
    //: mutating func caculateCommentItemHeight() {
    mutating func rangePart() {
        //: var tempContent = ""
        var tempContent = ""
        ...
        if replyUser?.count ?? 0 > 0 {
            ...
            if AddBeautyThen.shared.direction == .leftToRight {
                ...
                tempContent = (String(data_makeId)).localized + "\(replyUser!): \(content!)"
                //: } else {
            } else {
                //: tempContent = "\(content!) :\(replyUser!)" + "reply".localized
                tempContent = "\(content!) :\(replyUser!)" + (String(data_makeId)).localized
            }
        ...
        let sizeheight = Int(ReadPlayReactiveCompatible.withQuantity(text: tempContent, size: CGSize(width: notiEnabledRecordUrl - 70, height: CGFloat(MAXFLOAT)), font: UIFont.pingfangCenter(type: .Regular, fontSize: 16), lineNumber: 0).height)
