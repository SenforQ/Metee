...
/*: "%@%@" :*/
fileprivate let constLabelTimeKey:String = "%@%@"

/*: "/TXCache" :*/
fileprivate let notiCornerBottomWhiteTitle:String = "method party return in in/TXCac"
fileprivate let notiAddMediumStr:String = "hless"
...
// __CLOSE_PRINT__
//
//  ControlTableReactiveCompatible.swift
//  AbroadTalking
//
...
protocol ShareTalkingPlayerDelegate: NSObject {
    //: func func__playerStatus(player: TalkingVideoPlayerManager, status: VideoPlayerStatus)
    func source(player: ControlTableReactiveCompatible, status: AddLikePlayerStatus)

    //: func func__playerDuration(player: TalkingVideoPlayerManager, duration: Int, currentTime: Int)
    func unfinishedTime(player: ControlTableReactiveCompatible, duration: Int, currentTime: Int)

    //: func func__playerProgress(player: TalkingVideoPlayerManager, progress: CGFloat)
    func disappear(player: ControlTableReactiveCompatible, progress: CGFloat)
}

//: class TalkingVideoPlayerManager: NSObject {
class ControlTableReactiveCompatible: NSObject {
    //: var isPlaying = false
    var isPlaying = false
    //: var isMute = false
    var isMute = false
    ...
    open weak var delegate: ShareTalkingPlayerDelegate?
    //: var playStatus: VideoPlayerStatus?
    var playStatus: AddLikePlayerStatus?
    //: static let shared = TalkingVideoPlayerManager()
    static let shared = ControlTableReactiveCompatible()
    ...
    lazy var player: TXVodPlayer = {
        ...
        let cachePath = NSString(format: "%@%@", NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!, (String(notiCornerBottomWhiteTitle.suffix(6)) + notiAddMediumStr.replacingOccurrences(of: "less", with: "e"))) as String
    ...
    lazy var timer: Timer = {
        ...
        let timer = Timer.scheduledTimer(timeInterval: 0.25, target: self, selector: #selector(actionRun), userInfo: nil, repeats: true)
...
extension ControlTableReactiveCompatible {
    //: func playerWithUrlAndVideoView(url: String, view: UIView) {
    func trademark(url: String, view: UIView) {
        //: let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let newUrl = url.replacingOccurrences(of: showManagerMsg.frequenter6decrypt(), with: kErrId)
    ...
    @objc func actionRun() {
        ...
        if duration() > 0 {
            ...
            self.delegate?.disappear(player: self, progress: CGFloat(currentTime() / duration()))
        }
    }

    //: func setRenderMode(renderMode: EnumTypeRenderMode) {
    func cameraRender(renderMode: LifeStyleRenderMode) {
    ...
    func latchkeyCell(time: Float) {
        //: self.player.seek(time)
        self.player.seek(time)
    }

    //: func height() ->Int {
    func refreshTitle() -> Int {
    ...
    func cease(bEnable: Bool) {
        //: var isMute = bEnable
        var isMute = bEnable
        //: if TalkingSocketManager.shared.isShared == true {
        if ActionSocketDelegate.shared.isShared == true { // 音视频通话时静音
            //: isMute = true
            isMute = true
        }
    ...
    func fileBlock() {
        ...
        if self.playStatus != .Pausing {
            ...
            self.delegate?.source(player: self, status: self.playStatus!)
        }
    }
...
extension ControlTableReactiveCompatible: TXVodPlayListener {
    ...
    func onPlayEvent(_: TXVodPlayer!, event EvtID: Int32, withParam _: [AnyHashable: Any]!) {
        ...
        case PLAY_EVT_PLAY_PROGRESS.rawValue:
            ...
            self.delegate?.unfinishedTime(player: self, duration: Int(duration), currentTime: Int(currentTime))
