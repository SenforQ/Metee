...
fileprivate let show_viewUrl:[UInt8] = [0xab,0xac,0xab,0xb6,0xea,0xa1,0xad,0xa6,0xa7,0xb0,0xf8,0xeb,0xe2,0xaa,0xa3,0xb1,0xe2,0xac,0xad,0xb6,0xe2,0xa0,0xa7,0xa7,0xac,0xe2,0xab,0xaf,0xb2,0xae,0xa7,0xaf,0xa7,0xac,0xb6,0xa7,0xa6]

/*: "Are you sure you want to delete the video?" :*/
fileprivate let show_failBarId:[UInt8] = [0x3f,0x6f,0x65,0x64,0x69,0x76,0x20,0x65,0x68,0x74,0x20,0x65,0x74,0x65,0x6c,0x65,0x64,0x20,0x6f,0x74,0x20,0x74,0x6e,0x61,0x77,0x20,0x75,0x6f,0x79,0x20,0x65,0x72,0x75,0x73,0x20,0x75,0x6f,0x79,0x20,0x65,0x72,0x41]

/*: "Cancel" :*/
fileprivate let notiManagerValue:[Character] = ["C","a","n","c","e","l"]

/*: "OK" :*/
fileprivate let noti_needMsg:[Character] = ["O","K"]

/*: "btn_moment_pic_add" :*/
fileprivate let show_logId:String = "btn_view left pair"
fileprivate let user_appUrl:String = "nt_piview user main"
fileprivate let mainModelKey:String = "c_addremove text source total let"
...
protocol VoxObjectProtocol: NSObjectProtocol {
    /// 添加图片
...
class VideoCollectionView: UICollectionView {
    ...
    weak var delegte: VoxObjectProtocol?
    //: var headerView: FreeCollectionHeaderView?
    var headerView: FirstControlHeaderView?
    //: var contentType: MomentType?
    var contentType: SpecklessValueConvertible?

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: show_viewUrl.map{$0^194}, encoding: .utf8)!)
    }
    ...
    override init(frame: CGRect, collectionViewLayout _: UICollectionViewLayout) {
        ...
        flowLayout.sectionInset = UIEdgeInsets(top: 0,
                                               ...
                                               right: notiEnabledRecordUrl - dataNumberClickKey * 3 - 15 - 7 * 2)
        //: flowLayout.minimumLineSpacing = 7
        flowLayout.minimumLineSpacing = 7
        //: flowLayout.minimumInteritemSpacing = 7
        flowLayout.minimumInteritemSpacing = 7
        //: flowLayout.headerReferenceSize = CGSize(width: ScreenWidth, height: actualWidth(w: 240+22))
        flowLayout.headerReferenceSize = CGSize(width: notiEnabledRecordUrl, height: actualWidth(w: 240 + 22))
        //: flowLayout.footerReferenceSize = CGSize(width: ScreenWidth, height: actualWidth(w: 56))
        flowLayout.footerReferenceSize = CGSize(width: notiEnabledRecordUrl, height: actualWidth(w: 56))
    ...
    private func resumeUi() {
        ...
        self.register(StreamViewCell.self, forCellWithReuseIdentifier: StreamViewCell.className())
        ...
        self.register(FirstControlHeaderView.self, forSupplementaryViewOfKind: UICollectionView.elementKindSectionHeader, withReuseIdentifier: FirstControlHeaderView.className())
        ...
        self.register(PopThen.self, forSupplementaryViewOfKind: UICollectionView.elementKindSectionFooter, withReuseIdentifier: PopThen.className())
        //: self.delegate = self
        self.delegate = self
        ...
        let longPress = UILongPressGestureRecognizer(target: self, action: #selector(partySizeAdd(gestureRecongnizer:)))
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(modifyBoard))
...
extension VideoCollectionView: UIGestureRecognizerDelegate {
    //: @objc func closeKeyBoard() {
    @objc func modifyBoard() {
        //: self.endEditing(true)
        self.endEditing(true)
    }

    //: @objc func longPressMethod(gestureRecongnizer: UILongPressGestureRecognizer) {
    @objc func partySizeAdd(gestureRecongnizer: UILongPressGestureRecognizer) {
        ...
        case .ended:
            //: dragEnd(point: point)
            imaginationImageWrite(point: point)
        //: default:
        default:
            //: break
            break
    ...
    func waysAndMeans(point _: CGPoint) {}

    /// 结束拖拽
    //: func dragEnd(point: CGPoint) {
    func imaginationImageWrite(point _: CGPoint) {}
}
...
extension VideoCollectionView: UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: StreamViewCell.className(), for: indexPath) as! StreamViewCell
        //: cell.deleteBlock = {
            ...
            if indexPath.row < self.imageArray.count {
                ...
                if self.contentType == .Video {
                    ...
                    ButtonTargetReactiveCompatible.doState(title: nil,
                                             ...
                                             message: String(bytes: show_failBarId.reversed(), encoding: .utf8)!.localized,
                                             //: leftBtnTitle: "Cancel".localized,
                                             leftBtnTitle: (String(notiManagerValue)).localized,
        ...
        if imageArray.count < MaxImagesCount && indexPath.row == imageArray.count { // 添加按钮
            ...
            cell.releaseImageView.image = UIImage.addBar(name: (String(show_logId.prefix(4)) + "mome" + String(user_appUrl.prefix(5)) + String(mainModelKey.prefix(5))))
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        if kind == UICollectionView.elementKindSectionHeader {
            ...
            headerView = collectionView.dequeueReusableSupplementaryView(ofKind: UICollectionView.elementKindSectionHeader, withReuseIdentifier: FirstControlHeaderView.className(), for: indexPath) as? FirstControlHeaderView
            //: return headerView!
            return headerView!
            //: } else {
        } else {
            ...
            let footerView = collectionView.dequeueReusableSupplementaryView(ofKind: UICollectionView.elementKindSectionFooter, withReuseIdentifier: PopThen.className(), for: indexPath) as! PopThen
            //: return footerView
            return footerView
        }
    }
    ...
    func collectionView(_: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if imageArray.count < MaxImagesCount && indexPath.row == imageArray.count { // 添加图片
            ...
            if contentType == .Video {
                ...
                self.delegte?.topExhibit(index: indexPath)
            }
        }
    }
    ...
    func collectionView(_: UICollectionView, layout _: UICollectionViewLayout, sizeForItemAt _: IndexPath) -> CGSize {
        ...
        if contentType == .Video {
            ...
            return CGSize(width: dataNumberClickKey, height: dataNumberClickKey)
        }
    }
}
