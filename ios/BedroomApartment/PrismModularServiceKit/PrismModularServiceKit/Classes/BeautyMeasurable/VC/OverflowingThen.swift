...
class OverflowingThen: ScoreViewController {
	var dataAnswerEnable: Bool = true
	var cancelSum: Int = 45
	var clutchName: String = "current"
    ...
    lazy var bottomView: BeautyView = {
    ...
    lazy var usePhotoView: MuzzleVelocityView = {
        //: let v = TalkingSendIntimateInfoView(frame: CGRect.zero, type: .intimatePhoto)
        let v = MuzzleVelocityView(frame: CGRect.zero, type: .intimatePhoto)
        //: return v
        return v
        //: }()
    }()
...
extension OverflowingThen {
    //: func crushBtnTool(modle: TalkingMomentModel) {
    func contentModle(modle: QueryEnableCardTypifySunModelType) {
        ...
        if modle.sex == nil {
            ...
            //: bottomView.isHidden = modle.sex==RegularBeginDecisionMakerAppManager.share.loginUserMode.sex
            bottomView.isHidden = modle.sex == RegularBeginDecisionMakerAppManager.share.loginUserMode.sex
        }
