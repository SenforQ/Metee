...
/*: "Cancel" :*/
fileprivate let constColorMsg:[Character] = ["C","a","n","c","e","l"]

/*: "Post" :*/
fileprivate let userChoiceValue:String = "mode titlePost"

/*: "Upload failed" :*/
fileprivate let notiEqualKey:String = "register bar view varUploa"
fileprivate let constTopFormat:String = "linfo"

/*: "imgUrl" :*/
fileprivate let app_requestPath:[Character] = ["i","m","g","U","r","l"]

/*: "videoUrl" :*/
fileprivate let mainLastFormat:[Character] = ["v","i"]
fileprivate let noti_recordStr:[Character] = ["d","e","o","U","r","l"]

/*: "content" :*/
fileprivate let constBackUrl:String = "cbottom"
fileprivate let mainContentSizeMessage:[Character] = ["n","t","e","n","t"]

/*: "location" :*/
fileprivate let main_colorName:[Character] = ["l","o","c"]
fileprivate let showPushIndexValue:[Character] = ["a","t","i","o","n"]

/*: "Video review in progress" :*/
fileprivate let app_suiteValue:String = "make else for shareVide"
fileprivate let appNextTitle:String = "w ihidden white view mask path"
fileprivate let mainAttentionData:[Character] = ["n"," ","p","r","o","g","r","e","s","s"]

/*: "Shoot video at least 5 seconds" :*/
fileprivate let app_imageData:String = "limit color videoShoot vi"
fileprivate let mainRangeTitle:[Character] = ["d","e","o"," ","a","t"," ","l","e","a","s","t"," ","5"," ","s","e","c","o","n","d","s"]

/*: "Camera" :*/
fileprivate let const_guidanceErrorFormat:[Character] = ["C","a","m","e","r","a"]

/*: "Photo" :*/
fileprivate let main_toFormat:[Character] = ["P","h","o","t","o"]

/*: "Video" :*/
fileprivate let noti_chemicalId:[Character] = ["V","i","d","e","o"]
...
protocol FromSizeControllerDelegate: NSObjectProtocol {
    // 发布成功
    //: func ShareSucceed()
    func levelAnswer()
}
...
class ControlObjectProtocol: ScoreViewController {
	var contentCollectionSum: Int = 21
	var listInterval: Double = -43.7

    //: weak var delegate: TalkingFreeViewControllerDelegate?
    weak var delegate: FromSizeControllerDelegate?
    ...
    override func viewDidLoad() {
        ...
        even()
            ...
            if let value = collectionView.imageArray as? [String] {
                tagFront.giftArray = value
            }
            
            tagFront.valueGestureCount = { [self] iconHeadQuantity in
            self.contentCollectionSum = iconHeadQuantity
            
            var tagFront = collectionView.MaxImagesCount
            tagFront &+= 1
            if tagFront < self.contentCollectionSum {
                self.contentCollectionSum = tagFront
            }
            
            return self.contentCollectionSum
            }
            tagFront.makeCrushNumber = { [self] outcomeInterval in
            self.listInterval = outcomeInterval
            
            return self.listInterval
            }
                self.navigationController?.present(tagFront.self, animated: true) { [self] in
            self.listInterval = 575.89
                }
            }
    ...
    private lazy var reqManager: StyleRequestTool = //: return StyleRequestTool()
        .init()
    //: }()
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        btn.setTitle((String(constColorMsg)).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(objectStart), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var postBtn: UIButton = {
        ...
        btn.setTitle((String(userChoiceValue.suffix(4))).localized, for: .normal)
        ...
        btn.setBackgroundImage(UIImage.constraintWindow(colors: UIColor.hideBy(), size: CGSize(width: 70, height: 30)), for: .normal)
    ...
    private lazy var collectionView: VideoCollectionView = {
        //: let collectionView = FreeCollectionView(frame: CGRect.zero, collectionViewLayout: UICollectionViewFlowLayout())
        let collectionView = VideoCollectionView(frame: CGRect.zero, collectionViewLayout: UICollectionViewFlowLayout())
        ...
        collectionView.deleteActionBlock = { [weak self] in
            ...
            if self.collectionView.imageArray.count == 0 {
                ...
                self.collectionView.contentType = SpecklessValueConvertible.nones
                //: self.videoPath = ""
                self.videoPath = ""
                //: self.coverPath = ""
                self.coverPath = ""
...
extension ControlObjectProtocol {
    ...
    func letOut() {
        ...
        reqManager.straddle(photos: self.collectionView.imageArray, content: content) { [weak self] succeed, _, _ in
            //: ProgressHUD.dismiss()
            ...
            NotificationCenter.default.post(name: app_matchMessage, object: self, userInfo: nil)
        }
    }
    ...
    func profileRemove() {
        ...
        ViewSaveListener.shared.share(type: 1, coverPath: self.coverPath, videoPath: self.videoPath) { coverUrl, videoUrl, succeed in
            //: guard succeed else {
            guard succeed else {
                //: ProgressHUD.dismiss()
                DescriptionProgressHUD.actionOf()
                //: self.func__showStatusBarErrorMsg(showMsg: "Upload failed".localized)
                self.statusTo(showMsg: (String(notiEqualKey.suffix(5)) + "d fai" + constTopFormat.replacingOccurrences(of: "info", with: "ed")).localized)
                //: return
                return
            }
            ...
            dict[(String(app_requestPath))] = coverUrl
            //: dict["videoUrl"] = videoUrl
            dict[(String(mainLastFormat) + String(noti_recordStr))] = videoUrl
            //: dict["content"] = content
            dict[(constBackUrl.replacingOccurrences(of: "bottom", with: "o") + String(mainContentSizeMessage))] = content
            //: dict["location"] = ""
            dict[(String(main_colorName) + String(showPushIndexValue))] = ""

            //: TalkingMomentRequestTool.releaseVideoMoment(params: dict) {[weak self] succeed, result, errorModel in
            StyleRequestTool.restate(params: dict) { [weak self] succeed, _, _ in
                //: ProgressHUD.dismiss()
                DescriptionProgressHUD.actionOf()
                //: guard let self = self else { return }
                guard let self = self else { return }
                ...
                self.clickFill(showMsg: (String(app_suiteValue.suffix(4)) + "o revie" + String(appNextTitle.prefix(3)) + String(mainAttentionData)).localized)
                //: self.navigationController?.popViewController(animated: true)
                self.navigationController?.popViewController(animated: true)
                //: self.delegate?.ShareSucceed()
                self.delegate?.levelAnswer()
                //: NotificationCenter.default.post(name: POST_EXPLORE_SUCCESS_NOTIFICATION, object: self, userInfo: nil)
                NotificationCenter.default.post(name: app_matchMessage, object: self, userInfo: nil)
            }
        }
...
extension ControlObjectProtocol {
    ...
    func makeEmpty() {
        ...
        PenumbraReactiveCompatible.sharedAgent(true) { (isOpen: Bool) in
    ...
    func reopenEqual() {
        ...
        PenumbraReactiveCompatible.toRequest(true) { (isOpen: Bool) in
            ...
            let vc = MomentPickTool.infoVideo(maxCount: count, allowPhoto: true, allowVideo: false)
            ...
            vc.didFinishPickingVideoHandle = { [weak self] (_: UIImage?,
                    ...
                    MomentPickTool.tvCompletion(asset: assets) { filePath in
                        ...
                        if seconds >= 5 {
                            ...
                            self.spearThrower(url: filePath!)
                            //: } else {
                        } else {
                            //: DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.5) {
                            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.5) {
                                //: self.func__showStatusBarErrorMsg(showMsg: "Shoot video at least 5 seconds".localized)
                                self.statusTo(showMsg: (String(app_imageData.suffix(8)) + String(mainRangeTitle)).localized)
                            }
                        }
                    }
            }
    ...
    func spearThrower(url: URL) {
        //: let videoVC = TalkingVideoEditorController(maxDuration: 60.0, videoURL: url) { videoInfo in
        let videoVC = StyleControlViewController(maxDuration: 60.0, videoURL: url) { videoInfo in
...
extension ControlObjectProtocol: UIImagePickerControllerDelegate & UINavigationControllerDelegate {
        ...
        self.dismiss(animated: true) {
            ...
            if mediaType == kUTTypeMovie as String {
                ...
                if seconds >= 5 {
                    ...
                    self.spearThrower(url: info[.mediaURL]! as! URL)
...
// MARK: - VoxObjectProtocol

//: extension TalkingFreeViewController: FreeCollectionViewDelegate {
extension ControlObjectProtocol: VoxObjectProtocol {
    /// 选择图片
    ...
    func upwardPop() {
        ...
        if self.collectionView.contentType == .Photo {
            ...
            vc.statuteName(cellTitleList: [(String(const_guidanceErrorFormat)).localized, (String(main_toFormat)).localized])
            //: }else {
        } else {
            //: vc.initwithList(cellTitleList: ["Camera".localized, "Photo".localized, "Video".localized])
            vc.statuteName(cellTitleList: [(String(const_guidanceErrorFormat)).localized, (String(main_toFormat)).localized, (String(noti_chemicalId)).localized])
        }
    ...
    func topExhibit(index: IndexPath) {
        //: let vc = TalkingMomentPhotosVC.init(images: self.collectionView.imageArray, index: index.row, type: .publish)
        let vc = PhotosRecognizerDelegate(images: self.collectionView.imageArray, index: index.row, type: .publish)
        // 删除按钮block
    ...
    func elite() {
        ...
        let vc = DetailRecognizerDelegate(videoPath: self.videoPath)
        //: vc.modalPresentationStyle = .fullScreen
        vc.modalPresentationStyle = .fullScreen
        //: self.present(vc, animated: true)
        self.present(vc, animated: true)
...
extension ControlObjectProtocol {
    ...
    func even() {
        ...
        cancelBtn.snp.makeConstraints { make in
            ...
            make.top.equalTo(showPlainValue + 7)
        ...
        collectionView.snp.makeConstraints { make in
            ...
            make.top.equalToSuperview().offset(const_changeFormat)
