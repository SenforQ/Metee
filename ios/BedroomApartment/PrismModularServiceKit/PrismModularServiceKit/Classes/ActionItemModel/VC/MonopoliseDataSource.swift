...
/*: "Game Center" :*/
fileprivate let user_contentPath:String = "Game element group demonstrate request send"

/*: "You've got no list yet." :*/
fileprivate let showQueryButtonUrl:String = "You\'ve"
fileprivate let user_rawName:[Character] = [" ","n","o"," ","l","i","s","t"," ","y","e","t","."]

/*: "list" :*/
fileprivate let mainResultKeyValue:[Character] = ["l","i","s","t"]
...
class MonopoliseDataSource: ScoreViewController {
	var birthTotal: Double = 77.9
	var radiateDictionary: [AnyHashable: String] = [:]
    ...
    override func viewDidLoad() {
        ...
        self.title = (String(user_contentPath.prefix(5)) + "Center").localized
        //: setupSubviews()
        component()
        //: setupSubViewsConstraint()
        totalerchangeRequestConstraint()
        //: bindInteraction()
        appMagnitudeeraction()
        //: req_getGameList()
        gain()
    
            if (collectionView.layer.zPosition == 67.33) && (collectionView.constraintsAffectingLayout(for: .horizontal).count == 83) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let itemControl = ChaseView(frame: collectionView.frame.union(CGRect(x: CGFloat(463.03), y: CGFloat(88.92), width: CGFloat(0), height: CGFloat(76))))



            itemControl.sumRelationCellSum = { [self] atNumber in
            self.birthTotal = atNumber
            
            return self.birthTotal
            }
            itemControl.throughDictionary = { [self] exploitDictionary in
            self.radiateDictionary = exploitDictionary
            
            guard var value = self.radiateDictionary as? [String: String] else {
                return nil
            }
            return value
            }
                collectionView.addSubview(itemControl)
            }
    ...
    private lazy var emptyView: EmptyView = {
        ...
        style.TipsTitle = (showQueryButtonUrl + " got" + String(user_rawName)).localized
...
extension MonopoliseDataSource {
    /// 获取游戏列表
    ...
    private func gain() {
        ...
        MeButtonThen.actionCompletion(category: 1) { [weak self] _, result, _ in
            //: ProgressHUD.dismiss()
            ...
            json[(String(mainResultKeyValue))].arrayObject?.forEach { dict in
...
extension MonopoliseDataSource: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.sizeFull(model: dataArr[indexPath.row], nameColor: .census())
        //: return cell
        return cell
    }
...
extension MonopoliseDataSource {
    ...
    private func totalerchangeRequestConstraint() {
