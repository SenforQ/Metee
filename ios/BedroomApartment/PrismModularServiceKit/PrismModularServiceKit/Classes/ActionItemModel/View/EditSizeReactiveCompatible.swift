...
fileprivate let const_indexAddLabTitle:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Edit Title" :*/
fileprivate let app_intimateData:[Character] = ["E","d","i","t"," ","T","i","t"]
fileprivate let appRangeUrl:[Character] = ["l","e"]

/*: "0/ :*/
fileprivate let app_eventFormat:String = "0/"

/*: "Finish" :*/
fileprivate let kToStr:String = "path else up superFinis"
fileprivate let appScaleMsg:String = "label"

/*: "btn_delete" :*/
fileprivate let k_cancelData:String = "btn_delcenter self input data app"
fileprivate let notiPureMsg:String = "esex"

/*: / :*/
fileprivate let show_headKey:String = "event"

/*: "\n" :*/
fileprivate let main_modelReloadName:String = "\n"
...
// __CLOSE_PRINT__
//
//  EditSizeReactiveCompatible.swift
//  AbroadTalking
//
...
class EditSizeReactiveCompatible: UIView {
    ...
    override init(frame: CGRect) {
        ...
        self.afterLoadConstraint()
        //: let tapGesture = UITapGestureRecognizer(target: self, action: #selector(dismissView))
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(thanLaughAwayTap))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: const_indexAddLabTitle.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var titleLabel: UILabel = {
        ...
        lab.text = (String(app_intimateData) + String(appRangeUrl)).localized
    ...
    lazy var inputTView: SmallNameView = {
    ...
    private lazy var finishBtn: UIButton = {
        ...
        btn.setTitle((String(kToStr.suffix(5)) + appScaleMsg.replacingOccurrences(of: "label", with: "h")).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(colorFinish), for: .touchUpInside)
        //: btn.isEnabled = false
        btn.isEnabled = false
        //: return btn
        return btn
    ...
    private lazy var closeBtn: UIButton = {
        ...
        btn.setImage(UIImage.addBar(name: (String(k_cancelData.prefix(7)) + notiPureMsg.replacingOccurrences(of: "sex", with: "te"))), for: .normal)
        //: btn.addTarget(self, action: #selector(closeBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(application), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension EditSizeReactiveCompatible {
    //: @objc func dismissView() {
    @objc func thanLaughAwayTap() {}

    //: @objc func finishBtnClick() {
    @objc func colorFinish() {
        //: dismiss()
        changeForm()
        //: if textUpload != nil {
        if textUpload != nil {
...
extension EditSizeReactiveCompatible: UITextViewDelegate {
    ...
    public func textViewDidChange(_ textView: UITextView) {
        ...
        if textView.text.count > lengthLimit {
            ...
            if textNum > lengthLimit && lengthLimit > 0 {
                ...
                textView.text = addAt(index: lengthLimit, text: textContent)
            }
        }
    ...
    private func addAt(index: Int, text: String) -> String {
        //: if text.count <= index {
        if text.count <= index {
            //: return text
            return text
...
extension EditSizeReactiveCompatible {
    ...
    private func afterLoadConstraint() {
