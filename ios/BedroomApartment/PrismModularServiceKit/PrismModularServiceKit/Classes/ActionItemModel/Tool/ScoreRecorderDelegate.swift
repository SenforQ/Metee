...
fileprivate let noti_languageName:[UInt8] = [0x9,0x35,0x34,0x2e,0x7d,0x3b,0x38,0x3c,0x29,0x28,0x2f,0x38,0x7d,0x34,0x2e,0x7d,0x33,0x32,0x29,0x7d,0x3c,0x2b,0x3c,0x34,0x31,0x3c,0x3f,0x31,0x38,0x7d,0x39,0x28,0x2f,0x34,0x33,0x3a,0x7d,0x3c,0x7d,0x3e,0x3c,0x31,0x31]

private func sessionAppear(gift num: UInt8) -> UInt8 {
    return num ^ 93
}

/*: "wav" :*/
fileprivate let data_appFormat:String = "wabar"
...
protocol TitleReactiveCompatible: NSObject {
    /// 录音时间
...
class ScoreRecorderDelegate: NSObject {
    //: open weak var delegate: TalkingAudioToolDelegate?
    open weak var delegate: TitleReactiveCompatible?
    ...
    static let shared = ScoreRecorderDelegate()
    ...
    func header(path: NSString) -> String {
...
extension ScoreRecorderDelegate: AVAudioRecorderDelegate, AVAudioPlayerDelegate {
    /// 录制、播放过程中中断
    ...
    @objc func notification(notification: NSNotification) {
        ...
        if changeReason?.intValue ?? 0 == AVAudioSession.InterruptionType.began.rawValue {
            ...
            self.statusTo(showMsg: String(bytes: noti_languageName.map{sessionAppear(gift: $0)}, encoding: .utf8)!.localized)
            //: if self.isRecording {
            if self.isRecording {
                //: self.delegate?.func__recordFinishWithFile(filePath: self.filePath)
                self.delegate?.straightAndNarrow(filePath: self.filePath)
...
//                destroyRecordTimer()
                ...
                self.delegate?.targetSystem(status: .PlayerInterruption)
            }
        }
    }
    ...
    func audioPlayerDidFinishPlaying(_: AVAudioPlayer, successfully _: Bool) {
        ...
        self.delegate?.targetSystem(status: .PlayerFinish)
    }
}

//: extension TalkingAudioTool {
extension ScoreRecorderDelegate {
    //: func BeginRecord() {
    func familyNameRecord() {
        //: deleteFile()
        enter()
        ...
        let path = header(path: (data_appFormat.replacingOccurrences(of: "bar", with: "v")) as NSString)
    ...
    func totalimateBy() {
        ...
        recordTimer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(toTime(time:)), userInfo: nil, repeats: true)
    ...
    @objc func toTime(time _: Timer) {
        ...
        self.delegate?.cardTo(currentTime: interval)
    }
    ...
    func postItem() {
        ...
        self.delegate?.voiceStart(currentTime: currentTime)
    }
    ...
    func rangeTimer() {
        ...
        playTimer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(center(time:)), userInfo: nil, repeats: true)
        //: if let curTime: Timer = recordTimer {
        if let curTime: Timer = recordTimer {
            //: RunLoop.main.add(curTime, forMode: .common)
            RunLoop.main.add(curTime, forMode: .common)
