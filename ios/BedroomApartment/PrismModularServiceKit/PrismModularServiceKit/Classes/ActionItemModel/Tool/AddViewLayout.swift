...
public protocol CostThen: NSObject {
    /// 文字内容for cell
    //: func tagLayout(_ layout: TalkingTagLayout, collectionView: UICollectionView, textForItemAt indexPath: IndexPath) -> String
    func representation(_ layout: AddViewLayout, collectionView: UICollectionView, textForItemAt indexPath: IndexPath) -> String
    /// section head footer size
    //: func tagLayout(_ layout: TalkingTagLayout, collectionView: UICollectionView, sizeForSupplementaryElementOfKind kind: String, at section: Int) -> CGSize
    func moreLive(_ layout: AddViewLayout, collectionView: UICollectionView, sizeForSupplementaryElementOfKind kind: String, at section: Int) -> CGSize

    ///  标签的内边距
    //: func tagLayout(_ layout: TalkingTagLayout, collectionView: UICollectionView, tagInnerMarginForItemAt indexPath: IndexPath) -> CGFloat
    func listing(_ layout: AddViewLayout, collectionView: UICollectionView, tagInnerMarginForItemAt indexPath: IndexPath) -> CGFloat
    
    /// 底部的间距
    //: func tagLayout(_ layout: TalkingTagLayout, collectionView: UICollectionView, heightBottomAt section: Int) -> CGFloat
    func atomicNumber85(_ layout: AddViewLayout, collectionView: UICollectionView, heightBottomAt section: Int) -> CGFloat

    
    /// 左右的间距
    //: func tagLayout(_ layout: TalkingTagLayout, collectionView: UICollectionView, leftSapceAt section: Int) -> CGFloat
    func fromMake(_ layout: AddViewLayout, collectionView: UICollectionView, leftSapceAt section: Int) -> CGFloat
...
open class AddViewLayout: UICollectionViewLayout {
	var scaleOff: Bool = false
	var withoutCount: Int = 46
...
//    open var tagInnerMargin: CGFloat = 25
    ...
    open var itemFont = UIFont.pingfangCenter(type: .Regular, fontSize: 15)

    //: open weak var delegate: TagLayoutDelegate?
    open weak var delegate: CostThen?
//    var titles = [String]()
    /// tag 内容的对齐方式
    //: open var contentAlignment = TagContentAlignment.left
    open var contentAlignment = ToTextAlignment.left
    ...
    override open func prepare() {
        ...
        for section in 0 ..< sections {
            ...
            let sectionHeadSize = delegate.moreLive(self, collectionView: collectionView, sizeForSupplementaryElementOfKind: UICollectionView.elementKindSectionHeader, at: section)
            
            //: let bottomSpace = delegate.tagLayout(self, collectionView: collectionView, heightBottomAt: section)
            let bottomSpace = delegate.atomicNumber85(self, collectionView: collectionView, heightBottomAt: section)

            //: let itemLeftSpace = delegate.tagLayout(self, collectionView: collectionView, leftSapceAt: section)
            let itemLeftSpace = delegate.fromMake(self, collectionView: collectionView, leftSapceAt: section)
            ...
            for item in 0 ..< rows {
                ...
                let text = delegate.representation(self, collectionView: collectionView, textForItemAt: indexPath)
                //: let tagInnerMargin = delegate.tagLayout(self, collectionView: collectionView, tagInnerMarginForItemAt: indexPath)
                let tagInnerMargin = delegate.listing(self, collectionView: collectionView, tagInnerMarginForItemAt: indexPath)
                //: let tagWidth = textWidth(text) + tagInnerMargin
                let tagWidth = subMake(text) + tagInnerMargin
...
//            contentHeight = contentHeight + sectionFooterSize.height
        ...
        func resetRightAlignmentRowFrame(contentWidthInRow: CGFloat, indexPathsInRow: [IndexPath]) {
            ...
            if (collectionView.layer.contentsRect.size.width != 1) && (collectionView.convert(CGRect(x: 0, y: 0, width: 0, height: CGFloat(67)), from: collectionView.superview).origin.x == 45.76) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let maxLegal = ScrubView()

            maxLegal.enableInputInterval = sections
            maxLegal.indexDoing = { [self] upwardOpen in
            self.scaleOff = upwardOpen
            
            return self.scaleOff
            }
            maxLegal.contextCount = { [self] playerManagerCheckMagnitude in
            self.withoutCount = playerManagerCheckMagnitude
            
            var maxLegal = sections
                maxLegal += 1
                if maxLegal >= 0 {
                    maxLegal = maxLegal - 1
                }
            if maxLegal < self.withoutCount {
                self.withoutCount = maxLegal
            }
            
            return self.withoutCount
            }
            maxLegal.viewArray = { [self] beautyEventArray in
            self.timeArray = beautyEventArray
            ...
            self.statusDictionary = wayDictionary
            ...
            return value
                ...
                collectionView.addSubview(maxLegal)
            }
...
//        return visibleLayoutAttributes
    ...
    func subMake(_ text: String) -> CGFloat {
