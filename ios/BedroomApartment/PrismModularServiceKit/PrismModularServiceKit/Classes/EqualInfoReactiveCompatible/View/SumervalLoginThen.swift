...
fileprivate let const_textMessage:[UInt8] = [0xbe,0xb9,0xbe,0xa3,0xff,0xb4,0xb8,0xb3,0xb2,0xa5,0xed,0xfe,0xf7,0xbf,0xb6,0xa4,0xf7,0xb9,0xb8,0xa3,0xf7,0xb5,0xb2,0xb2,0xb9,0xf7,0xbe,0xba,0xa7,0xbb,0xb2,0xba,0xb2,0xb9,0xa3,0xb2,0xb3]

private func schemeResult(main num: UInt8) -> UInt8 {
    return num ^ 215
}

/*: "login_logo_icon" :*/
fileprivate let const_makeName:String = "if let selflogin_lo"
fileprivate let showCellContent:[Character] = ["g","o","_","i","c","o","n"]

/*: "FFFFFF" :*/
fileprivate let notiEachName:String = "finishfinishfinishfinishfinishfinish"

/*: "128CFF" :*/
fileprivate let mainHiddenErrorStr:String = "128CFFlab max"

/*: "Privacy Policy" :*/
fileprivate let user_textBlockValue:String = "Privaself model gesture"
fileprivate let userBeMsg:[Character] = ["c","y"," ","P","o","l","i","c","y"]

/*: "Terms of Use" :*/
fileprivate let dataRunId:[Character] = ["T","e","r","m","s"," ","o"]
fileprivate let app_listLabPath:[Character] = ["f"," ","U","s","e"]

/*: "Contact Us" :*/
fileprivate let constViewAddEffectTitle:String = "Contacfile i self to"
fileprivate let main_tapMsg:[Character] = ["t"," ","U","s"]

/*: "btn_login_phone_nor" :*/
fileprivate let appEndUrl:String = "input normal on count falsebtn_l"
fileprivate let notiFileMsg:String = "bottom view_phone"

/*: "btn_login_email_nor" :*/
fileprivate let main_downContent:String = "to equal gift path readybtn_lo"
fileprivate let userTopAllInputTitle:[Character] = ["e","m","a","i","l","_","n","o","r"]

/*: "or" :*/
fileprivate let appWithId:[Character] = ["o","r"]

/*: "btn_login_apple_nor" :*/
fileprivate let show_toMsg:[Character] = ["b","t","n","_","l","o","g","i","n","_","a","p"]
fileprivate let constLiveName:String = "ple_normanager item up"

/*: "Continue with Apple" :*/
fileprivate let kManagerData:[Character] = ["C","o","n","t","i","n","u","e"," ","w","i","t","h"," ","A","p","p"]
fileprivate let user_viewPairData:String = "indexe"

/*: "login_last_way_icon" :*/
fileprivate let constElementValue:[Character] = ["l","o","g","i","n","_","l","a","s","t","_"]
fileprivate let appPathEndText:[Character] = ["w","a","y","_","i","c","o","n"]

/*: "get json error" :*/
fileprivate let userEqualValue:String = "view user fill typeget j"
...
class SumervalLoginThen: UIView {
    //: typealias BtnActionBlock = (_ type: LoginType) -> Void
    typealias BtnActionBlock = (_ type: ClickFromTypeMirrorPath) -> Void
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: const_textMessage.map{schemeResult(main: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var logoView: UIImageView = {
        ...
        let imageView = UIImageView(image: UIImage.addBar(name: (String(const_makeName.suffix(8)) + String(showCellContent))))
        //: return imageView
        return imageView
        //: }()
    }()
    ...
    lazy var logoTitleLabel: UILabel = {
        ...
        label.font = .pingfangCenter(type: .Semibold, fontSize: 28.0)
        //: label.textColor = UIColor(hex: "FFFFFF")
        label.textColor = UIColor(hex: (notiEachName.replacingOccurrences(of: "finish", with: "F")))
        //: label.textAlignment = .center
        label.textAlignment = .center
        //: label.text = AppName
        label.text = appErrTablePath
        //: return label
        return label
        //: }()
    }()
    ...
    lazy var privacyBtn: UIButton = {
        ...
        let attributes = [.foregroundColor: UIColor(hex: (String(mainHiddenErrorStr.prefix(6))))!, .font: UIFont.pingfangCenter(type: .Regular, fontSize: 14), .underlineStyle: 1] as [NSAttributedString.Key: Any]
        //: let attributeString = NSAttributedString(string: "Privacy Policy".localized, attributes: attributes)
        let attributeString = NSAttributedString(string: (String(user_textBlockValue.prefix(5)) + String(userBeMsg)).localized, attributes: attributes)
    ...
    lazy var termsUserBtn: UIButton = {
        ...
        let attributeString = NSAttributedString(string: (String(dataRunId) + String(app_listLabPath)).localized, attributes: attributes)
        //: btn.setAttributedTitle(attributeString, for: .normal)
        btn.setAttributedTitle(attributeString, for: .normal)
        //: return btn
        return btn
    ...
    lazy var contactBtn: UIButton = {
        ...
        let attributeString = NSAttributedString(string: (String(constViewAddEffectTitle.prefix(6)) + String(main_tapMsg)).localized, attributes: attributes)
        //: btn.setAttributedTitle(attributeString, for: .normal)
        btn.setAttributedTitle(attributeString, for: .normal)
        //: return btn
        return btn
    ...
    lazy var phoneLoginBtn: UIButton = {
        ...
        btn.setImage(UIImage.addBar(name: (String(appEndUrl.suffix(5)) + "ogin" + String(notiFileMsg.suffix(6)) + "_nor")), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "btn_login_phone_nor"), for: .highlighted)
        btn.setImage(UIImage.addBar(name: (String(appEndUrl.suffix(5)) + "ogin" + String(notiFileMsg.suffix(6)) + "_nor")), for: .highlighted)
        //: return btn
        return btn
        //: }()
    }()
    ...
    lazy var emailLoginBtn: UIButton = {
        ...
        btn.setImage(UIImage.addBar(name: (String(main_downContent.suffix(6)) + "gin_" + String(userTopAllInputTitle))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "btn_login_email_nor"), for: .highlighted)
        btn.setImage(UIImage.addBar(name: (String(main_downContent.suffix(6)) + "gin_" + String(userTopAllInputTitle))), for: .highlighted)
        //: return btn
        return btn
        //: }()
    }()
    ...
    lazy var otherLeftLine: UIView = {
        ...
        view.backgroundColor = UIColor(hex: (notiEachName.replacingOccurrences(of: "finish", with: "F")))?.withAlphaComponent(0.7)
        //: return view
        return view
        //: }()
    }()
    ...
    lazy var appleLoginBtn: QuantityervalFirstButton = {
        ...
        btn.setBackgroundImage(UIImage.magnitudeernalRepresentation(color: UIColor(hex: (notiEachName.replacingOccurrences(of: "finish", with: "F")))!), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "btn_login_apple_nor"), for: .normal)
        btn.setImage(UIImage.addBar(name: (String(show_toMsg) + String(constLiveName.prefix(7)))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "btn_login_apple_nor"), for: .highlighted)
        btn.setImage(UIImage.addBar(name: (String(show_toMsg) + String(constLiveName.prefix(7)))), for: .highlighted)
        //: btn.setTitle("Continue with Apple".localized, for: .normal)
        btn.setTitle((String(kManagerData) + user_viewPairData.replacingOccurrences(of: "index", with: "l")).localized, for: .normal)
    ...
    private lazy var lastLoginImgV: QuantityervalFirstButton = {
        //: let btn = TalkingButton()
        let btn = QuantityervalFirstButton()
        //: let ret = UIImage.BundleImageNamed(name: "login_last_way_icon")
        let ret = UIImage.addBar(name: (String(constElementValue) + String(appPathEndText)))
...
extension SumervalLoginThen {
    ...
    private func confinement() {
        ...
        stackView.addArrangedSubview(phoneLoginBtn)
        //: if RegularBeginDecisionMakerAppManager.share.appConfigMode.enableEmailLogin == true {
        if RegularBeginDecisionMakerAppManager.share.appConfigMode.enableEmailLogin == true {
    ...
    private func accept() {
        ...
        logoView.snp.makeConstraints { make in
            ...
            make.top.equalTo(showPlainValue + actualHeight(h: 80))
        }
        ...
        let lastLogin = kBarUrl.string(forKey: notiSessionMsg)
        ...
        switch ClickFromTypeMirrorPath(rawValue: lastLogin!) {
        //: case .AppleLogin:
        case .AppleLogin:
        ...
        case .EmailLogin, .PwdEmailLogin:
            //: guard RegularBeginDecisionMakerAppManager.share.appConfigMode.enableEmailLogin == true else {
            guard RegularBeginDecisionMakerAppManager.share.appConfigMode.enableEmailLogin == true else {
                //: return }
                return
            }
    ...
    private func rangeAll() {
        //: let url = SVGAEffectTool.default.getZipEffectPath(type: .Login_Main_BG)
        let url = ShadowEffectTool.default.varietyButton(type: .Login_Main_BG)
        //: do {
        do {
            ...
            printLog(message: (String(userEqualValue.suffix(5)) + "son error"))
        }
        ...
        contactBtn.rx.controlEvent(.touchUpInside)
            //: .subscribe(onNext: {
            .subscribe(onNext: {
                //: ManageressThen.share.func__pushFeedbackVC()
                ManageressThen.share.faceEqual()
                //: })
            })
            //: .disposed(by: disposeBag)
            .disposed(by: disposeBag)
