...
fileprivate let userWhoUrl:[UInt8] = [0x7a,0x7d,0x7a,0x67,0x3b,0x70,0x7c,0x77,0x76,0x61,0x29,0x3a,0x33,0x7b,0x72,0x60,0x33,0x7d,0x7c,0x67,0x33,0x71,0x76,0x76,0x7d,0x33,0x7a,0x7e,0x63,0x7f,0x76,0x7e,0x76,0x7d,0x67,0x76,0x77]

private func totaleractionLet(draft num: UInt8) -> UInt8 {
    return num ^ 19
}
...
@objc protocol SubViewDelegate: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func successScroll()

    //: @objc optional func endScroll()
    @objc optional func recessScroll()
}

//: class TalkingVideoDanmuMsgListTableView: UIView {
class EqualConnectTableView: UIView {
	var valueOff: Bool = false
	var designateArray: [AnyHashable] = []
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userWhoUrl.map{totaleractionLet(draft: $0)}, encoding: .utf8)!)
    }
    ...
    override func layoutSubviews() {
        ...
        super.layoutSubviews()
            ...
            if (msgTableView.layer.anchorPointZ != 0) && (msgTableView.motionEffects.count == 17) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let tagOf = ClipBlockView()
            
            tagOf.donationMoveOff = { [self] narrowClose in
            self.valueOff = narrowClose
            
            var tagOf = inPending
            tagOf = true
            if tagOf != self.valueOff {
                self.valueOff = tagOf
            }
            ...
            self.designateArray = backOfArray
            
            guard var value = self.designateArray as? [String] else {
                return nil
            }
            return value
            }
            tagOf.handleDictionary = { [self] fromAgeDictionary in
            self.videoDictionary = fromAgeDictionary
...
extension EqualConnectTableView {
    //: func startTimer() {
    func titleTimer() {
        //: stopTimer()
        stallPopTableTool()
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(sumimate), userInfo: nil, repeats: true)
    }

    //: func stopTimer() {
    func stallPopTableTool() {
...
extension EqualConnectTableView {
    ...
    func towardNewCan(msgModel: PlaceTransformable?) {
    ...
    func stopBottom() {
        ...
        if AllHeight > kVideoStr {
            //: if self.msgTableView.height < VideoMsgTableViewHeight {
            if self.msgTableView.height < kVideoStr {
                ...
                self.msgTableView.height = kVideoStr
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = kVideoStr - AllHeight
        ...
        addEnableTitle(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func addEnableTitle(animated: Bool) {
    ...
    func paradigmPending(inPending: Bool) {
...
extension EqualConnectTableView: UITableViewDelegate, UITableViewDataSource {
    ...
    func countimate(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.recessScroll?()
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(show_keyTitle)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = DotBassReactiveCompatible.inheritance(tableView: tableView, msg: msgModel, indexPath: indexPath)
...
extension EqualConnectTableView {
    ...
    func eraseAddSubviews() {
        ...
        self.msgTableView.frame = CGRect(x: 0, y: 0, width: data_partyDevicePath, height: kVideoStr)
    }
}
