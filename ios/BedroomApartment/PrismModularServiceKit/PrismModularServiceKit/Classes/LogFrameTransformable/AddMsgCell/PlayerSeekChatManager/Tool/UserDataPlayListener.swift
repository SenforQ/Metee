...
/*: "call_bgm" :*/
fileprivate let data_labUrl:String = "picall"

/*: "call_response_bgm" :*/
fileprivate let kFinishToolOfName:String = "tap list managercall_"
fileprivate let kViewMsg:[Character] = ["r"]
fileprivate let data_aspectPath:[Character] = ["e","s","p","o","n","s","e","_","b","g","m"]
...
class UserDataPlayListener: NSObject {
    //: var type = CallBGMPlayerType.Call
    var type = GenerateStrideable.Call
    //: var player = AVPlayer()
    var player = AVPlayer()
    //: var filePath = ""
    var filePath = ""
    ...
    class func hollerithCard(type: GenerateStrideable) -> UserDataPlayListener {
        //: let player = TalkingCallBGMPlayer()
        let player = UserDataPlayListener()
        //: player.type = type
        player.type = type
        //: player.getFilePath()
        player.nameDestroy()
...
extension UserDataPlayListener {
    //: func play() {
    func burnPlay() {
    ...
    private func notify(notify: Notification) {
    ...
    private func route(notify: Notification) {
    ...
    private func nameDestroy() {
        //: var fileName = "call_bgm"
        var fileName = (data_labUrl.replacingOccurrences(of: "pic", with: "c") + "_bgm")
        //: if self.type == .Response {
        if self.type == .Response {
            //: fileName = "call_response_bgm"
            fileName = (String(kFinishToolOfName.suffix(5)) + String(kViewMsg) + String(data_aspectPath))
        }
        // 收到通话推送要播放mp3声音，不能放到.zip中
        //: let path = SVGAEffectTool.default.getMp3Path(name: fileName)
        let path = ShadowEffectTool.default.toType(name: fileName)
        //: self.filePath = path
        self.filePath = path
    }
}
...
extension UserDataPlayListener: TXVodPlayListener {
