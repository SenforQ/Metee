...
/*: ：" :*/
fileprivate let constFormatUrl:[Character] = ["："]

/*: "#BAFBFF" :*/
fileprivate let appPlayerAreaPath:[Character] = ["#","B","A","F","B","F","F"]
...
class SumervalSelectThen: NSObject {
    ...
    var model = PlaceTransformable()

    //: func caculateMsgHeight(model: TalkingVideoCallDanmuModel) -> TalkingVideoCallDanmuModel {
    func modelPhoto(model: PlaceTransformable) -> PlaceTransformable {
        //: self.model = model
        self.model = model
        /// 普通文本消息
    ...
    func timeMap(model: PlaceTransformable) -> PlaceTransformable {
        //: self.model = model
        self.model = model
        /// 普通文本消息
        //: if self.model.messageType == 1 {
    ...
    func equidistant() {
        ...
        let name = pastGift(text: firstStr, font: UIFont.mediumPingfangStatusSizeFont(fontSize: 14), color: UIColor(hex: (String(appPlayerAreaPath)))!)
        // 内容
        //: let content = getAttributed(text: self.model.content, font: UIFont.pingfangMediumFont(fontSize: 14), color: UIColor.white)
        let content = pastGift(text: self.model.content, font: UIFont.mediumPingfangStatusSizeFont(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        ...
        talkWarning(attribText: self.model.msgAttribText)
    }
    ...
    func closeceComment() {
        ...
        let content = pastGift(text: self.model.transContent, font: UIFont.mediumPingfangStatusSizeFont(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        //: name.yy_paragraphStyle = paraStyle
        name.yy_paragraphStyle = paraStyle
    ...
    func cognisant() -> NSMutableParagraphStyle {
    ...
    func pastGift(text: String, font: UIFont, color: UIColor) -> NSMutableAttributedString {
...
//            attribute.yy_writingDirection = [3]
        ...
        attribute.yy_paragraphStyle = cognisant()
        //: return attribute
        return attribute
    }
    ...
    func talkWarning(attribText: NSMutableAttributedString) {
        // 距离左边8  距离右边也为8
        //: let maxWidth = VideoMsgTableViewWidth - 36
        let maxWidth = data_partyDevicePath - 36
