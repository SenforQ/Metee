...
fileprivate let kChangeEqualFormat:String = "content selected equalTalkin"
fileprivate let noti_taskName:String = "body accessfectCa"
fileprivate let constSessionText:[Character] = ["c","h","e","K","e","y"]

/*: "下载动画文件" :*/
fileprivate let appModeMakeSliceData:String = "下载动画文\u{4ef6}"

/*: "加载本地动画文件" :*/
fileprivate let appStreamUrl:String = "加\u{8f7d}本\u{5730}动\u{753b}文件"

/*: "%@/%@/" :*/
fileprivate let mainCounteractionUrl:[Character] = ["%"]
fileprivate let main_makeSizePath:String = "@icon%@icon"

/*: "%@/%zd/" :*/
fileprivate let notiModelShowId:[Character] = ["%","@","/","%","z","d","/"]

/*: "Talking_GiftEffect/" :*/
fileprivate let app_keyPath:String = "Talkmodel title current if"
fileprivate let const_fromValue:String = "content self to disable layerEffect/"

/*: "%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x" :*/
fileprivate let main_succeedPath:[UInt8] = [0xe7,0xf2,0xf0,0xba,0xe7,0xf2,0xf0,0xba,0xe7,0xf2,0xf0,0xba,0xe7,0xf2,0xf0,0xba,0xe7,0xf2,0xf0,0xba,0xe7,0xf2,0xf0,0xba,0xe7,0xf2,0xf0,0xba,0xe7,0xf2,0xf0,0xba,0xe7,0xf2,0xf0,0xba,0xe7,0xf2,0xf0,0xba,0xe7,0xf2,0xf0,0xba,0xe7,0xf2,0xf0,0xba,0xe7,0xf2,0xf0,0xba,0xe7,0xf2,0xf0,0xba,0xe7,0xf2,0xf0,0xba,0xe7,0xf2,0xf0,0xba]

private func yetMake(with num: UInt8) -> UInt8 {
    return num ^ 194
}

/*: "GJSGVA" :*/
fileprivate let k_viewPlayerUrl:String = "GJSGtable"

/*: "GJVap" :*/
fileprivate let const_userData:[Character] = ["G","J","V","a","p"]

/*: "config.json" :*/
fileprivate let showObjectEqualPath:[Character] = ["c","o","n","f","i"]
fileprivate let notiDirectionStr:[Character] = ["g",".","j","s","o","n"]

/*: "%d" :*/
fileprivate let app_minFormat:String = "color make add model date%d"

/*: "app. :*/
fileprivate let mainListData:String = "app.text height me equal"

/*: .com" :*/
fileprivate let userStrokeColorValue:String = "first var new like string.com"

/*: "key" :*/
fileprivate let main_whiteData:[Character] = ["k","e","y"]

/*: "zip" :*/
fileprivate let main_infoName:String = "zivalue"

/*: " fail to unzip file" :*/
fileprivate let k_giftTitle:String = "white return view main fail "
fileprivate let const_modelPath:[Character] = ["i","p"," ","f","i","l","e"]

/*: "RSA 解密失败" :*/
fileprivate let noti_ofName:String = "RSA color gesture"

/*: "fail to unzip script file" :*/
fileprivate let k_selectionPath:String = "fail trequest content object skin view"
fileprivate let app_tableValue:String = "size letip scr"
fileprivate let dataContentPath:String = "FILE"

/*: "%ld" :*/
fileprivate let user_inputUrl:String = "%ldshared succeed"

/*: "gift" :*/
fileprivate let showCellInfoName:String = "gbottomt"

/*: "%@%@effect_%ld" :*/
fileprivate let mainStyleAddStr:String = "dismiss pic video%@%@ef"
fileprivate let main_succeedUrl:[Character] = ["d"]

/*: "%@%@effect_%ld_unzipTest/" :*/
fileprivate let user_taskMsg:[Character] = ["%","@","%","@","e","f","f","e","c","t","_","%","l","d","_","u","n","z","i","p","T","e","s","t","/"]

/*: "%@%@effect_%ld_unzip/" :*/
fileprivate let k_addStr:String = "self section table ok%@%@e"
fileprivate let data_freshStr:String = "_%ld_uelse leading"
...
typealias CompletionBlock = (_ success: Bool, _ effectModel: MyUserDataPathAnimatModel?) -> Void
...
class SeekThen: NSObject {
    ...
    let TalkingGiftAnimatCacheKey = (String(kChangeEqualFormat.suffix(6)) + "gGiftEf" + String(noti_taskName.suffix(6)) + String(constSessionText))

    //: @objc static let shared = TalkingGiftAnimatTool()
    @objc static let shared = SeekThen()
    ...
    func noModel(effectMsgModel: ClickCleanModelType, completion: @escaping CompletionBlock) {
        //: if checkNewVersionWithAnimatMsgModel(model: effectMsgModel) {
        if basicHide(model: effectMsgModel) {
            //: printLog(message: "下载动画文件")
            printLog(message: (appModeMakeSliceData))
            //: downloadAnimat(effectMsgModel: effectMsgModel) { [weak self] error in
            throwingBoard(effectMsgModel: effectMsgModel) { [weak self] error in
                ...
                self.network(effectMsgModel: effectMsgModel, complete: completion)
            }
            //: } else {
        } else {
            //: printLog(message: "加载本地动画文件")
            printLog(message: (appStreamUrl))
            //: self.loadLocalAnimatConfig(effectMsgModel: effectMsgModel, complete: completion)
            self.network(effectMsgModel: effectMsgModel, complete: completion)
        }
    }
    ...
    func doingModel(fileName: String, model: ClickCleanModelType) -> String {
        //: let scriptDirectory = fetchScriptDirectory(animatMsgModel: model)
        let scriptDirectory = viewRowMeanSun(animatMsgModel: model)
    ...
    func pause() {
        ...
        betweenInstancePath(path: componentDirectory())
    }

    // 动效资源本地的缓存目录路径
    //: func fetchScriptDirectory(animatMsgModel: TalkingAnimatMsgModel) -> String {
    func viewRowMeanSun(animatMsgModel: ClickCleanModelType) -> String {
        //: if animatMsgModel.eid == 0 {
        if animatMsgModel.eid == 0 {
            //: return String.init(format: "%@/%@/", giftEffectDirectory(), animatMsgModel.effectUrl.md5()!)
            return String(format: "%@/%@/", componentDirectory(), animatMsgModel.effectUrl.md5()!)
            //: } else {
        } else {
            //: return String.init(format: "%@/%zd/", giftEffectDirectory(), animatMsgModel.eid)
            return String(format: "%@/%zd/", componentDirectory(), animatMsgModel.eid)
        }
    }

    //: func giftEffectDirectory() -> String {
    func componentDirectory() -> String {
        //: let subPath = "Talking_GiftEffect/"
        let subPath = (String(app_keyPath.prefix(4)) + "ing_Gift" + String(const_fromValue.suffix(7)))
    ...
    func pathRegisterTrademark(filePath: String) -> String {
    ...
    func modelTransition(model: ClickCleanModelType) -> Bool {
        //: return !checkNewVersionWithAnimatMsgModel(model: model)
        return !basicHide(model: model)
    }

    //: func getNewAnimatClassName(oldName: String) -> String {
    func cognomenEnable(oldName: String) -> String {
        //: var newName = oldName
        var newName = oldName
        //: if newName.hasPrefix("GJSGVA") {
        if newName.hasPrefix((k_viewPlayerUrl.replacingOccurrences(of: "table", with: "VA"))) {
            //: newName = String(describing: TalkingSGVAAnimationView.self)
            newName = String(describing: VideoAnimationView.self)
            //: } else if newName.hasPrefix("GJVap") {
        } else if newName.hasPrefix((String(const_userData))) {
            //: newName = String(describing: TalkingVapAnimationView.self)
            newName = String(describing: MakeStyleAnimationView.self)
        }
        //: return newName
        return newName
    }
...
extension SeekThen {
    ...
    func network(effectMsgModel: ClickCleanModelType, complete: @escaping CompletionBlock) {
        //: let scriptDirectory = fetchScriptDirectory(animatMsgModel: effectMsgModel)
        let scriptDirectory = viewRowMeanSun(animatMsgModel: effectMsgModel)
        //: let scriptPath = (scriptDirectory as NSString).appendingPathComponent("config.json")
        let scriptPath = (scriptDirectory as NSString).appendingPathComponent((String(showObjectEqualPath) + String(notiDirectionStr)))
        ...
        let dic = NSDictionary.lie(withJsonString: configString!) ?? NSDictionary()
        //: let giftAmodel = TalkingGiftAnimatModel.init()
        let giftAmodel = MyUserDataPathAnimatModel()
        //: let effectModel = giftAmodel.setAnimatModek(dic: dic)
        let effectModel = giftAmodel.withDic(dic: dic)
        ...
        className = cognomenEnable(oldName: className)
    ...
    func basicHide(model: ClickCleanModelType) -> Bool {
    ...
    func throwingBoard(effectMsgModel: ClickCleanModelType, errorBlock: @escaping ErrorBlock) {
        //: PictureRequestTool.req_giftEffectDataUrl(giftUrl: effectMsgModel.effectUrl, path: downloadTmpPath(model: effectMsgModel)) { [weak self] t in
        PictureRequestTool.deviseBackgroundCompletion(giftUrl: effectMsgModel.effectUrl, path: showTargetModel(model: effectMsgModel)) { [weak self] t in
            //: guard let self = self else {return}
            guard let self = self else { return }
            //: DispatchQueue.global().async {
            DispatchQueue.global().async {
                //: self.afterDownloadGiftAnimat(effectMsgModel: effectMsgModel, isSucess: t, errorBlock: errorBlock)
                self.afterSucessConstraint(effectMsgModel: effectMsgModel, isSucess: t, errorBlock: errorBlock)
            }
        }
    }

    //: func afterDownloadGiftAnimat(effectMsgModel: TalkingAnimatMsgModel, isSucess: Bool, errorBlock: @escaping ErrorBlock) {
    func afterSucessConstraint(effectMsgModel: ClickCleanModelType, isSucess: Bool, errorBlock: @escaping ErrorBlock) {
        //: if !isSucess {
        if !isSucess {
            //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.DownLoadFailed.rawValue))
            errorBlock(NSError(domain: (String(mainListData.prefix(4))) + "\(kErrId)" + (String(userStrokeColorValue.suffix(4))), code: ConnectEnablesetTarget.DownLoadFailed.rawValue))
            //: return
            return
            ...
            let downloadPath = showTargetModel(model: effectMsgModel)
            //: let unzipVerifyDir = unzipVerifyDirectory(model: effectMsgModel)
            let unzipVerifyDir = instanceEqual(model: effectMsgModel)
            ...
            if verifyUnzipSucc {
                ...
                for filename in unzippedFiles {
                    //: if filename == "key" {
                    if filename == (String(main_whiteData)) {
                        // encrypted md5 file
                        //: keyFilePath = (unzipVerifyDir as NSString).appendingPathComponent(filename)
                        keyFilePath = (unzipVerifyDir as NSString).appendingPathComponent(filename)
                        //: } else if (filename as NSString).pathExtension == "zip" {
                    } else if (filename as NSString).pathExtension == (main_infoName.replacingOccurrences(of: "value", with: "p")) {
                        // script file
                ...
                printLog(message: (String(k_giftTitle.suffix(6)) + "to unz" + String(const_modelPath)))
                //: isFailed = true
                isFailed = true
                //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.UnzipFailed.rawValue))
                errorBlock(NSError(domain: (String(mainListData.prefix(4))) + "\(kErrId)" + (String(userStrokeColorValue.suffix(4))), code: ConnectEnablesetTarget.UnzipFailed.rawValue))
            }
            ...
            if !isFailed {
                ...
                do {
                    ...
                    let md5Data: Data = ColorOff.decryptData(NSData(contentsOfFile: keyFilePath) as Data?, publicKey: k_requestKey)
                    ...
                    let md5 = pathRegisterTrademark(filePath: scriptZipFilePath)
                    ...
                    if decryptMD5 != md5, FileManager.default.fileExists(atPath: keyFilePath) {
                        ...
                        errorBlock(NSError(domain: (String(mainListData.prefix(4))) + "\(kErrId)" + (String(userStrokeColorValue.suffix(4))), code: ConnectEnablesetTarget.VerifyFailed.rawValue))
                    }
                    ...
                    printLog(message: (String(noti_ofName.prefix(4)) + "解密失败"))
                    //: isFailed = true
                    isFailed = true
                    //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.VerifyFailed.rawValue))
                    errorBlock(NSError(domain: (String(mainListData.prefix(4))) + "\(kErrId)" + (String(userStrokeColorValue.suffix(4))), code: ConnectEnablesetTarget.VerifyFailed.rawValue))
            ...
            if !isFailed {
                ...
                let unzipTmpDir = singleOut(model: effectMsgModel)
                ...
                //: else {
                    ...
                    printLog(message: (String(k_selectionPath.prefix(6)) + "o unz" + String(app_tableValue.suffix(6)) + "ipt " + dataContentPath.lowercased()))
                    //: isFailed = true
                    isFailed = true
                    //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.UnzipFailed.rawValue))
                    errorBlock(NSError(domain: (String(mainListData.prefix(4))) + "\(kErrId)" + (String(userStrokeColorValue.suffix(4))), code: ConnectEnablesetTarget.UnzipFailed.rawValue))
    ...
    func betweenInstancePath(path: String) -> Bool {
        //: if cleanFolder(path: path) {
        if technicolor(path: path) {
    ...
    func technicolor(path: String) -> Bool {
    ...
    func thanWith(type: PositionCount) -> String {
        //: var tmpIdentify = ""
        var tmpIdentify = ""
        //: switch type {
        switch type {
        //: case .Gift:     tmpIdentify =  "gift"
        case .Gift: tmpIdentify = (showCellInfoName.replacingOccurrences(of: "bottom", with: "if"))
        //: break
        //: default:
        default:
            //: break
    ...
    func showTargetModel(model: ClickCleanModelType) -> String {
        ...
        let downloadTmpPath = String(format: (String(mainStyleAddStr.suffix(6)) + "fect_%l" + String(main_succeedUrl)), NSTemporaryDirectory(), thanWith(type: model.effectType), model.eid)
        //: return downloadTmpPath
        return downloadTmpPath
    }

    //: func unzipVerifyDirectory(model: TalkingAnimatMsgModel)->String {
    func instanceEqual(model: ClickCleanModelType) -> String {
        ...
        let unzipVerifyDirectory = String(format: (String(user_taskMsg)), NSTemporaryDirectory(), thanWith(type: model.effectType), model.eid)
        //: return unzipVerifyDirectory
        return unzipVerifyDirectory
    }

    //: func unzipTmpDirectory(model: TalkingAnimatMsgModel)->String {
    func singleOut(model: ClickCleanModelType) -> String {
        ...
        let unzipTmpDirectory = String(format: (String(k_addStr.suffix(5)) + "ffect" + String(data_freshStr.prefix(6)) + "nzip/"), NSTemporaryDirectory(), thanWith(type: model.effectType), model.eid)
