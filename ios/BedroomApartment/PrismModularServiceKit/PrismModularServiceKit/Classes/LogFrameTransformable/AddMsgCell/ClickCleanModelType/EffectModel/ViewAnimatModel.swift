...
/*: "id" :*/
fileprivate let notiContentUrl:String = "celld"

/*: "num" :*/
fileprivate let userInfoUrl:[Character] = ["n","u","m"]
...
@objc public enum CardSignedNumeric: Int {
    //: case normal  = 1
    case normal = 1 // 普通礼物
    //: case myStery = 2
    case myStery = 2 // 盲盒礼物
...
struct ViewAnimatModel: HandyJSON {
    ...
    var animationTimes = 0
    //: var showType = 0
    var showType = 0 /// CardSignedNumeric
    ...
    mutating func toSend(giftNum: Int) {
        //: self.num = giftNum
        self.num = giftNum
    }
}
...
struct TextAnimatModel: HandyJSON {
    //: init() {
    init() {}
    ...
    mutating func mapping(mapper: HelpingMapper) {
        ...
        mapper <<<
            //: self.giftNum <-- "num"
            self.giftNum <-- (String(userInfoUrl))
    }
}
