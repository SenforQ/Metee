...
fileprivate let noti_commentValue:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "[未知消息]" :*/
fileprivate let data_fullId:[Character] = ["[","未","\u{77e5}","\u{6d88}","息","]"]

/*: "extra" :*/
fileprivate let mainCellUrl:[Character] = ["e","x","t","r","a"]

/*: "</at> " :*/
fileprivate let show_infoSucceedNotMessage:String = "import height custom</at> "

/*: "#0084FF" :*/
fileprivate let main_shouldData:String = "select in#0084FF"

/*: "#FFEE4B" :*/
fileprivate let noti_pathName:String = "#FFEE4Bmedium content self text"

/*: "<at>@[\\S\\s]+?</at>" :*/
fileprivate let main_makeUrl:String = "guard normal<at>@["
fileprivate let appGiftKey:[Character] = ["]","+"]
fileprivate let constLinkTableLetName:[Character] = ["?","<","/","a","t",">"]

/*: "jumpKey" :*/
fileprivate let mainShowTitle:String = "jumpKeylayer nor temp lab self"

/*: "user" :*/
fileprivate let kIconPath:[Character] = ["u","s","e","r"]

/*: "uid" :*/
fileprivate let dataBackgroundName:[UInt8] = [0x64,0x69,0x75]

/*: "#16D073" :*/
fileprivate let app_partyPath:String = "#16D073share color photo send"

/*: "#F95644" :*/
fileprivate let data_wantValue:String = "result"
fileprivate let data_pageId:[Character] = ["F","9","5","6","4","4"]
...
@objcMembers public class PathCellData: CompartmentCellData {
    ...
    var transAttrString: NSMutableAttributedString {
        ...
        let attr = totalersectionPointTitle()
        //: return attr
        return attr
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_commentValue.reversed(), encoding: .utf8)!)
    }
    ...
    public lazy var contentAttrString: NSMutableAttributedString = {
        ...
        let attr = ptolemaicSystem(msgInfoModel: self.msgModel.msgInfo)
        //: return attr
        return attr
        //: }()
    }()
    ...
    func ptolemaicSystem(msgInfoModel: NumberModel) -> NSMutableAttributedString {
        ...
        guard dic[(String(mainCellUrl))] is [String: Any] else {
            //: return NSMutableAttributedString.init(string: "")
            return NSMutableAttributedString(string: "")
        }
        ...
        let textColor = (self.direction == .MsgDirectionIncoming) ? UIColor.census() : .white
        //: let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: "#0084FF")! : UIColor(hex: "#FFEE4B")!
        let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: (String(main_shouldData.suffix(7))))! : UIColor(hex: (String(noti_pathName.prefix(7))))!
        ...
        let regex = try! NSRegularExpression(pattern: (String(main_makeUrl.suffix(6)) + "\\S\\s" + String(appGiftKey) + String(constLinkTableLetName)), options: [])
        ...
        for (index, value) in rangeArr.enumerated() {
            ...
            if jumpInfo[(String(mainShowTitle.prefix(7)))].stringValue == (String(kIconPath)) {
                //: uid = jumpInfo["uid"].intValue
                uid = jumpInfo[String(bytes: dataBackgroundName.reversed(), encoding: .utf8)!].intValue
            }
            //: if uid != -1 {
            if uid != -1 {
                ...
                attr.yy_setTextHighlight(range, color: atColor, backgroundColor: .clear, userInfo: [String(bytes: dataBackgroundName.reversed(), encoding: .utf8)!: uid]) { [weak self] _, _, _, _ in
    ...
    func totalersectionPointTitle() -> NSMutableAttributedString {
        ...
        var color = UIColor(hex: (String(app_partyPath.prefix(7))))!
        //: if self.translateStatus == .MsgTranslate_fail {
        if self.translateStatus == .MsgTranslate_fail {
            //: color = UIColor(hex: "#F95644")!
            color = UIColor(hex: (data_wantValue.replacingOccurrences(of: "result", with: "#") + String(data_pageId)))!
        }
