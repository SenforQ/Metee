...
fileprivate let k_usedRequestMsg:[UInt8] = [0xa0,0xa7,0xa0,0xbd,0xe1,0xaa,0xa6,0xad,0xac,0xbb,0xf3,0xe0,0xe9,0xa1,0xa8,0xba,0xe9,0xa7,0xa6,0xbd,0xe9,0xab,0xac,0xac,0xa7,0xe9,0xa0,0xa4,0xb9,0xa5,0xac,0xa4,0xac,0xa7,0xbd,0xac,0xad]

private func statisticalTable(lab num: UInt8) -> UInt8 {
    return num ^ 201
}

/*: "icon_chat_translate_nor" :*/
fileprivate let k_shareSizePath:[Character] = ["i","c","o","n","_"]
fileprivate let dataDisappearMessage:String = "app"
fileprivate let k_infoKey:String = "return removeat_t"
fileprivate let dataManagerTitlePath:String = "view with tote_nor"

/*: "icon_chat_translate_pre" :*/
fileprivate let mainNameData:String = "icon_ccolor guard info"
fileprivate let constImageMessage:[Character] = ["a","n","s","l","a","t","e","_","p","r","e"]

/*: "icon_chat_miss" :*/
fileprivate let user_equalText:String = "icon_screen info hide manager"
fileprivate let appSharedFormat:String = "true gift handle add drawchat_"

/*: "icon_chat_yes" :*/
fileprivate let dataTitlePostId:String = "iperspectiven"
fileprivate let constInputKey:String = "_yeslevel name color text content"
...
class VoicePlayMsgCell: TitleMsgCell {
    //: var transBlock: (() -> Void)?
    var transBlock: (() -> Void)?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_usedRequestMsg.map{statisticalTable(lab: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var transLB: YYLabel = {
        ...
        label.font = UIFont.nogMain(fontSize: 16)
        //: return label
        return label
        //: }()
    }()
    ...
    lazy var transBtn: UIButton = {
        ...
        btn.setImage(UIImage.addBar(name: (String(k_shareSizePath) + dataDisappearMessage.replacingOccurrences(of: "app", with: "ch") + String(k_infoKey.suffix(4)) + "ransla" + String(dataManagerTitlePath.suffix(6)))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_chat_translate_pre"), for: .highlighted)
        btn.setImage(UIImage.addBar(name: (String(mainNameData.prefix(6)) + "hat_tr" + String(constImageMessage))), for: .highlighted)
...
extension VoicePlayMsgCell {
    ...
    override func revolution(with data: AggregationThen) {
        ...
        if imgPreview.count > 0, textData.msgModel.msgInfo.messageType == .MessageTypeWelcome {
            ...
            giftImagV.step(urlStr: imgPreview)
        }
        ...
        //: let maxWidth = ColorDelimitateMacroDefine.getScreenWidth() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        let maxWidth = ColorDelimitateMacroDefine.memberDisappear() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        // 翻译赋值相关
        ...
        if textData.translateStatus == .MsgTranslate_translating {
            ...
            self.transStatusImagV.image = UIImage.addBar(name: (String(user_equalText.prefix(5)) + String(appSharedFormat.suffix(5)) + "miss"))
            ...
            self.transStatusImagV.image = UIImage.addBar(name: (dataTitlePostId.replacingOccurrences(of: "perspective", with: "co") + "_chat" + String(constInputKey.prefix(4))))
        }
    ...
    func female() {
        ...
        let longPressGr = UILongPressGestureRecognizer(target: self, action: #selector(publicTransport(tap:)))
        //: self.container.addGestureRecognizer(longPressGr)
        self.container.addGestureRecognizer(longPressGr)
    }

    //: @objc func LognCellMessageAction(tap: UILongPressGestureRecognizer) {
    @objc func publicTransport(tap: UILongPressGestureRecognizer) {
        //: if tap.state == .ended {
        if tap.state == .ended {
            //: return
            return
