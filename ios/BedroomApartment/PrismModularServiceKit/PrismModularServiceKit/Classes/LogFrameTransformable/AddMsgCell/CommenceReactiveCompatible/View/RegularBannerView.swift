...
fileprivate let const_selectedMsg:[UInt8] = [0xa7,0xa0,0xa7,0xba,0xe6,0xad,0xa1,0xaa,0xab,0xbc,0xf4,0xe7,0xee,0xa6,0xaf,0xbd,0xee,0xa0,0xa1,0xba,0xee,0xac,0xab,0xab,0xa0,0xee,0xa7,0xa3,0xbe,0xa2,0xab,0xa3,0xab,0xa0,0xba,0xab,0xaa]

private func statusModel(file num: UInt8) -> UInt8 {
    return num ^ 206
}

/*: "icon_chats_banner_subtract" :*/
fileprivate let showErrorStr:String = "icon_load if model bit"
fileprivate let showDetailTitle:String = "barowrower"

/*: "icon_chats_banner_subtract_pre" :*/
fileprivate let app_bottomValue:[Character] = ["i","c","o","n","_","c","h","a"]
fileprivate let mainDayLoadKey:String = "ts_banequal view makeup image make"
fileprivate let notiSuiteFormat:String = "ract_preeffect value tool make"

/*: "bannerList" :*/
fileprivate let dataResearcherMessage:[Character] = ["b","a","n","n","e","r","L","i","s","t"]

/*: "JXBannerCellID" :*/
fileprivate let main_textUserEffectStr:[Character] = ["J","X","B","a","n","n","e","r"]
fileprivate let k_modeEqualMessage:[Character] = ["C","e","l","l","I","D"]

/*: "#FFFFFF" :*/
fileprivate let userWithBottomName:String = "make transition top text#FFFFFF"
...
protocol FromModelReactiveCompatible: NSObject {
    //: func changeBannerViewFream(isMiniView: Bool)
    func colorNow(isMiniView: Bool)
}

//: class TalkingPrivateBannerView: UIView {
class RegularBannerView: UIView {
    //: var isMiniView = false
    var isMiniView = false
    //: open weak var delegate: PrivateBannerDelegate?
    open weak var delegate: FromModelReactiveCompatible?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: const_selectedMsg.map{statusModel(file: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var bannerListData: [SeekSubMeasurable] = //: return Array<SeekSubMeasurable>()
        .init()
    //: }()
    ...
    private lazy var closeBtn: UIButton = {
        ...
        btn.setImage(UIImage.addBar(name: (String(showErrorStr.prefix(5)) + "chats_" + showDetailTitle.replacingOccurrences(of: "row", with: "n") + "_subtract")), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_chats_banner_subtract_pre"), for: .selected)
        btn.setImage(UIImage.addBar(name: (String(app_bottomValue) + String(mainDayLoadKey.prefix(6)) + "ner_subt" + String(notiSuiteFormat.prefix(8)))), for: .selected)
        //: btn.addTarget(self, action: #selector(closeBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(equalAdd), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension RegularBannerView {
    /// banner
    //: func reqBanner() {
    func starNowBanner() {
        //: SocialRequestManager.init().req_adBanner(position: 3) { [weak self] succeed, result, errorModel in
        FuturismRequestManager().adDataEnter(position: 3) { [weak self] _, result, _ in
            ...
            let array = JSON(result[(String(dataResearcherMessage))] ?? []).arrayValue
...
extension RegularBannerView {
    ...
    @objc private func equalAdd() {
        ...
        self.delegate?.colorNow(isMiniView: isMiniView)
    }

    //: func resetView(isMiniView: Bool) {
    func sizeImage(isMiniView: Bool) {
        //: if isMiniView {
        if isMiniView {
...
extension RegularBannerView: JXBannerDelegate, JXBannerDataSource {
    ...
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        ...
        return JXBannerCellRegister(type: JXBannerCell.self, reuseIdentifier: (String(main_textUserEffectStr) + String(k_modeEqualMessage)))
    }
    ...
    func jxBanner(_: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
        ...
        tempCell.imageView.equalFinish(urlStr: url, placeImg: UIImage.quadrateComment())
                  ...
                  builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder
    {
        ...
        pageControl.activeColor = UIColor(hex: (String(userWithBottomName.suffix(7))))!
