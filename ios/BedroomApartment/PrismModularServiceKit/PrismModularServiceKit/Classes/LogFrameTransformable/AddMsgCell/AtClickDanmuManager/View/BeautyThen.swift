...
/*: "99+" :*/
fileprivate let const_blockCornerName:String = "lightlight+"

/*: "init(coder:) has not been implemented" :*/
fileprivate let main_toToolValue:[UInt8] = [0x34,0x39,0x34,0x3f,0xf3,0x2e,0x3a,0x2f,0x30,0x3d,0x5,0xf4,0xeb,0x33,0x2c,0x3e,0xeb,0x39,0x3a,0x3f,0xeb,0x2d,0x30,0x30,0x39,0xeb,0x34,0x38,0x3b,0x37,0x30,0x38,0x30,0x39,0x3f,0x30,0x2f]

fileprivate func backgroundIf(leading num: UInt8) -> UInt8 {
...
/*: "#2DF2FF" :*/
fileprivate let appTableName:[Character] = ["#"]
fileprivate let noti_giftMsg:[Character] = ["2","D","F","2","F","F"]

/*: "Live" :*/
fileprivate let notiKeyEffectFormat:[Character] = ["L","i","v","e"]

/*: "img_home_shadow_icon" :*/
fileprivate let dataPicStr:[Character] = ["i","m","g","_","h","o","m","e","_","s","h","a","d","o","w","_","i","c","o","n"]

/*: "get json error" :*/
fileprivate let show_leadingUrl:String = "get jlabel user event"

/*: "#FF2348" :*/
fileprivate let notiThenId:[Character] = ["#"]
fileprivate let mainTableStr:String = "FF2send8"

/*: "#35C759" :*/
fileprivate let kReplyFrameId:String = "try if let cell string#35C759"

/*: "mfBean" :*/
fileprivate let appPlayInputTitle:String = "at icon count inmfBean"

/*: "%.1f" :*/
fileprivate let mainNoseValue:[Character] = ["%",".","1","f"]

/*: "points\n≈ $" :*/
fileprivate let kVideoFormat:String = "povia"

/*:   :*/
fileprivate let dataSizeFilterFormat:String = " "
...
public var main_buttonKey: Int {
    //: return RegularBeginDecisionMakerAppManager.share.appConfigMode.countdown1 + RegularBeginDecisionMakerAppManager.share.appConfigMode.countdown2
    return RegularBeginDecisionMakerAppManager.share.appConfigMode.countdown1 + RegularBeginDecisionMakerAppManager.share.appConfigMode.countdown2
}

//: class TalkingLiveMiniView: TalkingBaseMiniView {
class BeautyThen: SightThen {
    // 未检测出人脸时长（间隔1秒）
    ...
    var noFaceTime: Int = 0 {
        ...
        didSet {
            ...
            if noFaceTime >= RegularBeginDecisionMakerAppManager.share.appConfigMode.countdown1 { // 展示倒计时
                ...
                svgaNumLab.text = "\(main_buttonKey - noFaceTime)"
                //: } else {
            } else {
                //: refreshMiniView()
                refreshAwake()
    ...
    override init(frame: CGRect) {
        ...
        //: if Double(RegularBeginDecisionMakerAppManager.share.loginUserMode.mf_bean) > 0 {
        if Double(RegularBeginDecisionMakerAppManager.share.loginUserMode.mf_bean) > 0 {
            //: setPointText(mfBean: 0.0)
            withCheck(mfBean: 0.0)
        }
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: main_toToolValue.map{backgroundIf(leading: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var liveStatusImgV: QuantityervalFirstButton = {
        ...
        let image = UIImage.withUser(color: UIColor(hex: (String(appTableName) + String(noti_giftMsg)))!, size: CGSize(width: 8, height: 8))
        //: img.setImage(image.circle(), for: .normal)
        img.setImage(image.circle(), for: .normal)
        //: img.setTitle("Live".localized, for: .normal)
        img.setTitle((String(notiKeyEffectFormat)).localized, for: .normal)
        //: img.isHidden = true
        img.isHidden = true
        //: self.addSubview(img)
        self.addSubview(img)
    ...
    private lazy var shadowImg: UIImageView = {
        ...
        img.image = UIImage.addBar(name: (String(dataPicStr)))
        //: img.layer.cornerRadius = 8
        img.layer.cornerRadius = 8
        //: img.layer.masksToBounds = true
        img.layer.masksToBounds = true
    ...
    private lazy var svgaPlayer: SVGAPlayer = {
        ...
        do {
            ...
            printLog(message: (String(show_leadingUrl.prefix(5)) + "son error"))
        }
        //: return player
        return player
        //: }()
    ...
    private lazy var svgaNumLab: UILabel = {
        ...
        lab.font = UIFont.mediumPingfangStatusSizeFont(fontSize: 20)
        //: lab.textAlignment = .center
        lab.textAlignment = .center
        //: lab.textColor = UIColor(hex: "#FF2348")
        lab.textColor = UIColor(hex: (String(notiThenId) + mainTableStr.replacingOccurrences(of: "send", with: "34")))
        //: self.addSubview(lab)
        self.addSubview(lab)
        //: return lab
        return lab
    ...
    private lazy var msgCountLab: UILabel = {
        ...
        lab.backgroundColor = UIColor(hex: (String(kReplyFrameId.suffix(7))))
...
extension BeautyThen {
    ...
    static func administrativeDivisionView() -> BeautyThen {
        //: let view = TalkingLiveMiniView(frame: .zero)
        let view = BeautyThen(frame: .zero)
        //: let position = TalkingSocketManager.shared.liveMiniPosition
        let position = ActionSocketDelegate.shared.liveMiniPosition
    ...
    func refreshAwake() {
        ...
        liveStatusImgV.isHidden = !SearchPushListener.biologyLab().isLive
...
extension BeautyThen {
    ...
    private func concatenate() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(dataNotification(notification:)),
    ...
    @objc private func dataNotification(notification: NSNotification) {
        //: let userinfo = notification.userInfo as! [String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: guard let mfBean = userinfo["mfBean"] as? Double else { return }
        guard let mfBean = userinfo[(String(appPlayInputTitle.suffix(6)))] as? Double else { return }
        //: setPointText(mfBean: mfBean)
        withCheck(mfBean: mfBean)
    }

    //: func setPointText(mfBean: Double) {
    func withCheck(mfBean: Double) {
        //: let nowBean = RegularBeginDecisionMakerAppManager.share.loginUserMode.mf_bean + mfBean
        let nowBean = RegularBeginDecisionMakerAppManager.share.loginUserMode.mf_bean + mfBean
        //: RegularBeginDecisionMakerAppManager.share.loginUserMode.mf_bean = nowBean
        RegularBeginDecisionMakerAppManager.share.loginUserMode.mf_bean = nowBean
        ...
        mfBeanLB.text = "\(Int(nowBean))\((kVideoFormat.replacingOccurrences(of: "via", with: "in") + "ts\n≈ $").localized) \(divisor)"
