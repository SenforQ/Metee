...
/*: "live_ended" :*/
fileprivate let data_gestureName:String = "self selflive_e"

/*: "The live broadcast has ended" :*/
fileprivate let showDirectionKey:String = "The model after"
fileprivate let data_withAddText:String = " broif let true selected array"
fileprivate let data_maleName:String = "value let has e"
fileprivate let main_thumbPath:String = "nleadingd"

/*: "bth_live_male_phone_nor" :*/
fileprivate let main_modelName:String = "bth_lview mini data more"
fileprivate let const_userMakeData:[Character] = ["e"]
fileprivate let userGiftMessage:[Character] = ["_","p","h","o","n","e","_","n","o","r"]

/*: "floatingScreen" :*/
fileprivate let kDownData:[Character] = ["f","l","o","a","t","i","n","g"]
fileprivate let userStatusKey:String = "Screentag if self let"

/*: "You have been muted" :*/
fileprivate let appReadingMessageValue:String = "location add aspect button tagYou h"
fileprivate let noti_tempMessage:[Character] = ["a","v","e"]
fileprivate let dataTitleValue:String = "time icon been "

/*: "You have entered the other party's blacklist" :*/
fileprivate let user_partyLabId:[UInt8] = [0x79,0x4f,0x55,0x0,0x48,0x41,0x56,0x45,0x0,0x45,0x4e,0x54,0x45,0x52,0x45,0x44,0x0,0x54,0x48,0x45,0x0,0x4f,0x54,0x48,0x45,0x52,0x0,0x50,0x41,0x52,0x54,0x59,0x7,0x53,0x0,0x42,0x4c,0x41,0x43,0x4b,0x4c,0x49,0x53,0x54]

/*: "Live room exception, please return to retry, if multiple failures please contact the staff" :*/
fileprivate let notiSucceedData:[UInt8] = [0xba,0x9f,0x80,0x93,0xd6,0x84,0x99,0x99,0x9b,0xd6,0x93,0x8e,0x95,0x93,0x86,0x82,0x9f,0x99,0x98,0xda,0xd6,0x86,0x9a,0x93,0x97,0x85,0x93,0xd6,0x84,0x93,0x82,0x83,0x84,0x98,0xd6,0x82,0x99,0xd6,0x84,0x93,0x82,0x84,0x8f,0xda,0xd6,0x9f,0x90,0xd6,0x9b,0x83,0x9a,0x82,0x9f,0x86,0x9a,0x93,0xd6,0x90,0x97,0x9f,0x9a,0x83,0x84,0x93,0x85,0xd6,0x86,0x9a,0x93,0x97,0x85,0x93,0xd6,0x95,0x99,0x98,0x82,0x97,0x95,0x82,0xd6,0x82,0x9e,0x93,0xd6,0x85,0x82,0x97,0x90,0x90]

private func tapEnd(current num: UInt8) -> UInt8 {
    return num ^ 246
}

/*: "isCalling" :*/
fileprivate let app_hiddenValue:String = "gift manager user letisCa"
...
class PopViewDelegate: ScoreViewController {
	var textEnable: Bool = false
	var deviseSum: Int = 87
	var linguisticUnitTotal: Double = 27.5
	var tabularArrayDictionary: [AnyHashable: String] = [:]

    //: var type = LiveEnterType.userDetail
    var type = RegularStyleEnterType.userDetail // 入口
    ...
    override func viewDidLoad() {
        ...
        recordViewsSubSwaddlingClothesConstraint()
        //: bindInteraction()
        stint()
        //: playLiveVideo()
        developToTv()
		...
		if var listingValue = topView.weakVC?.hideNavi { 
			if var textTableValue = videoCallManager.uid { 
		            ...
		            withTopLet.collectionModelSwitch = self.isShowKeyboard
		            withTopLet.overObjectInterval = topView.noFaceTime
		            withTopLet.nextTableCount = commInputView.inputTextView.placeholderLeftOffset
		            withTopLet.clipKeyName = textTableValue
		            
		            withTopLet.inputSignalEqualOff = { [self] remoteOff in
		            self.textEnable = remoteOff
		            
		            var withTopLet = listingValue
		            withTopLet = !withTopLet
		            if withTopLet != self.textEnable {
		                self.textEnable = withTopLet
		            }
		            
		            return self.textEnable
		            }
		            withTopLet.showQuantity = { [self] awakeInterval in
		            self.deviseSum = awakeInterval
		            
		            var withTopLet = topView.noFaceTime
		            withTopLet &= 3
		            if withTopLet > self.deviseSum {
		                self.deviseSum = withTopLet
		            }
		            
		            return self.deviseSum
		            }
		            withTopLet.mortalSum = { [self] suiteNumber in
		            self.linguisticUnitTotal = suiteNumber
		            
		            var withTopLet = danmuView.AllHeight
		                withTopLet += 1
		                if withTopLet >= 0 {
		                    withTopLet = withTopLet - 1
		                }
		            if withTopLet < self.linguisticUnitTotal {
		                self.linguisticUnitTotal = withTopLet
		            }
		            
		            self.linguisticUnitTotal /= 5
		            return self.linguisticUnitTotal
		            }
		            withTopLet.frameDictionary = { [self] nameDictionary in
		            self.tabularArrayDictionary = nameDictionary
    ...
    private lazy var videoCallManager: RegularMatchDelegate = {
        //: let m = TalkingCallMenuManager()
        let m = RegularMatchDelegate()
        //: m.uid = String(TalkingLiveManager.shared().liveRoomModel.streamerInfo.uid)
        m.uid = String(SearchPushListener.biologyLab().liveRoomModel.streamerInfo.uid)
        //: return m
        return m
        //: }()
    }()
    ...
    private lazy var stopLiveBgView: UIView = {
        ...
        let icon = UIImageView(image: UIImage.addBar(name: (String(data_gestureName.suffix(6)) + "nded")))
        //: bg.addSubview(icon)
        bg.addSubview(icon)
        //: icon.snp.makeConstraints { make in
        icon.snp.makeConstraints { make in
            //: make.top.equalTo(StatusBarHeight+actualWidth(w: 205))
            make.top.equalTo(showPlainValue + actualWidth(w: 205))
        ...
        lab.text = (String(showDirectionKey.prefix(4)) + "live" + String(data_withAddText.prefix(4)) + "adcast" + String(data_maleName.suffix(6)) + main_thumbPath.replacingOccurrences(of: "leading", with: "de")).localized
        //: bg.addSubview(lab)
        bg.addSubview(lab)
    ...
    private lazy var bottomView: ToteUpView = {
        ...
        view.cover(SearchPushListener.biologyLab().liveRoomModel)
        //: view.delegate = self
        view.delegate = self
        //: return view
        return view
    ...
    private lazy var callBtn: UIButton = {
        ...
        btn.setImage(UIImage.addBar(name: (String(main_modelName.prefix(5)) + "ive_mal" + String(const_userMakeData) + String(userGiftMessage))), for: .normal)
        //: btn.addTarget(self, action: #selector(callButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(notGame), for: .touchUpInside)
    ...
    private lazy var topView: QuarterbackReactiveCompatible = {
        //: let v = TalkingLiveRoomTopView()
        let v = QuarterbackReactiveCompatible()
        //: v.weakVC = self
        v.weakVC = self
        //: v.refreshTopView(TalkingLiveManager.shared().liveRoomModel)
        v.appEqual(SearchPushListener.biologyLab().liveRoomModel)
...
extension PopViewDelegate {
    ...
    @objc private func developToTv() {
        ...
        self.player.startLivePlay(SearchPushListener.biologyLab().liveRoomModel.livePlayUrl, type: TX_Enum_PlayType.PLAY_TYPE_LIVE_FLV)
    }
    ...
    func tidings() {
        ...
        SelectPopThen.cellProclamation(groupId: SearchPushListener.biologyLab().liveRoomModel.chatGroupId, announcementMsg: SearchPushListener.biologyLab().liveRoomModel.notification)

        //: self.bottomView.toUid = String(TalkingLiveManager.shared().liveRoomModel.streamerInfo.uid)
        self.bottomView.toUid = String(SearchPushListener.biologyLab().liveRoomModel.streamerInfo.uid)
    }
    ...
    func ofListener() {
        ...
        V2TIMManager.sharedInstance().quitGroup(SearchPushListener.biologyLab().liveRoomModel.chatGroupId, succ: {
            //: }, fail: { code, str in
        }, fail: { _, _ in
            //: })
        })
...
extension PopViewDelegate {
    ...
    @objc private func notGame() {
        //: var videoStr = TalkingLiveManager.shared().liveRoomModel.videoPrice
        var videoStr = SearchPushListener.biologyLab().liveRoomModel.videoPrice
        //: var voiceStr = TalkingLiveManager.shared().liveRoomModel.voicePrice
        var voiceStr = SearchPushListener.biologyLab().liveRoomModel.voicePrice
        ...
        if RegularBeginDecisionMakerAppManager.share.loginUserMode.loungePlus {
            ...
            videoStr = SearchPushListener.biologyLab().liveRoomModel.videoVIPPrice
            //: voiceStr = TalkingLiveManager.shared().liveRoomModel.voiceVIPPrice
            voiceStr = SearchPushListener.biologyLab().liveRoomModel.voiceVIPPrice
        }
        //: videoCallManager.showVideoCallMenu(videoPrice: "\(videoStr)", voicePrice: "\(voiceStr)")
        videoCallManager.cellPrompt(videoPrice: "\(videoStr)", voicePrice: "\(voiceStr)")
        ...
        case .match:
            ...
            app_serverName.successOrMeanSun(eventID: const_clickMsg, toUid: String(SearchPushListener.biologyLab().liveRoomModel.streamerInfo.uid))
        ...
        case .userDetail:
            ...
            app_serverName.successOrMeanSun(eventID: dataPositionContent, toUid: String(SearchPushListener.biologyLab().liveRoomModel.streamerInfo.uid))
    ...
    func exceptGift(animated: Bool = true) {
        //: self.topView.stopTimer()
        self.topView.finishTimerImage()
        //: req_quitGroup()
        ofListener() // 退出群聊
        //: TalkingLiveManager.destroy()
        SearchPushListener.pathFor() // 销毁单例
        //: TalkingDanmuManager.shared().delegate = nil
        SelectPopThen.afterWithShared().delegate = nil
    ...
    @objc private func modelNotification(notification: NSNotification) {
        //: let userinfo = notification.userInfo as! [String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let dict = userinfo["floatingScreen"]
        let dict = userinfo[(String(kDownData) + String(userStatusKey.prefix(6)))]
...
extension PopViewDelegate: ScoreViewDelegate {
    //: func func__commentBtnClick() {
    func viewClick() {
        //: if Int(Date().timeIntervalSince1970) < TalkingLiveManager.shared().liveRoomModel.muteExpireAt {
        if Int(Date().timeIntervalSince1970) < SearchPushListener.biologyLab().liveRoomModel.muteExpireAt {
            //: self.func__showStatusBarErrorMsg(showMsg: "You have been muted".localized)
            self.statusTo(showMsg: (String(appReadingMessageValue.suffix(5)) + String(noti_tempMessage) + String(dataTitleValue.suffix(6)) + "muted").localized)
            //: return
            return
        }
        /// 拉起弹幕评论
...
extension PopViewDelegate: TalkingAtReactiveCompatible {
    //: func func__giftRecvNewModel(Msg: TalkingLiveRoomDanmuModel) {
    func funcStatusModel(Msg: StyleDanmuModel) {
        //: addGiftEffectModelArr(Msg: Msg)
        simulationTitle(Msg: Msg)
    }
...
extension PopViewDelegate: IncreaseViewDelegate {
...
extension PopViewDelegate {
    ...
    func simulationTitle(Msg: StyleDanmuModel) {
...
extension PopViewDelegate: ShadowFirstViewDelegate {
    ...
    func itemUid(msgStr: String, atUid: String?) {
        ...
        forefrontNeed(msgStr: msgStr, toUid: atUid)
    }
    ...
    func forefrontNeed(msgStr: String, toUid: String?) {
...
extension PopViewDelegate: V2TIMGroupListener {
    /// 被主播拉黑后，禁言退出房间
    ...
    func onMemberInfoChanged(_: String!, changeInfoList: [V2TIMGroupMemberChangeInfo]!) {
        //: if changeInfoList.first?.userID == RegularBeginDecisionMakerAppManager.share.loginUserMode.userID {
        if changeInfoList.first?.userID == RegularBeginDecisionMakerAppManager.share.loginUserMode.userID {
            //: let toastStr = "You have entered the other party's blacklist".localized
            let toastStr = String(bytes: user_partyLabId.map{$0^32}, encoding: .utf8)!.localized
...
extension PopViewDelegate {
    ...
    @objc func pastBar(note: Notification) {
...
extension PopViewDelegate: TXLivePlayListener {
    ...
    func onPlayEvent(_ evtID: Int32, withParam _: [AnyHashable: Any]!) {
        ...
        { // 观众侧拉流失败，退出直播间
            ...
            self.statusTo(showMsg: String(bytes: notiSucceedData.map{tapEnd(current: $0)}, encoding: .utf8)!.localized)
            //: popCurrentViewController()
            exceptGift()
        }
    }
...
extension PopViewDelegate {
    ...
    private func memoryPath() {
        ...
        self.view.backgroundColor = .census()
    ...
    private func recordViewsSubSwaddlingClothesConstraint() {
    ...
    func stint() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(pastBar(note:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(pachyteneEqual),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(modelNotification(notification:)),
        ...
        ActionSocketDelegate.shared.rx
            //: .observeWeakly(Bool.self, "isCalling")
            .observeWeakly(Bool.self, (String(app_hiddenValue.suffix(4)) + "lling"))
