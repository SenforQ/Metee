...
fileprivate let appFaceMakeName:[UInt8] = [0x4b,0x50,0x4b,0x56,0xa,0x45,0x51,0x46,0x47,0x54,0x1c,0xb,0x2,0x4a,0x43,0x55,0x2,0x50,0x51,0x56,0x2,0x44,0x47,0x47,0x50,0x2,0x4b,0x4f,0x52,0x4e,0x47,0x4f,0x47,0x50,0x56,0x47,0x46]

fileprivate func panelTo(target num: UInt8) -> UInt8 {
...
/*: "move" :*/
fileprivate let constGiftText:String = "nameove"
...
class HeaderReactiveCompatible: UIView {
	var ofOff: Bool = true
	var giantCount: Int = 82
	var needQuantity: Double = 79.5
	var atomicCount85Dictionary: [AnyHashable: String] = [:]
    ...
    var danumuMsgArray: [ElementMeasurable] = []
    //: var danumuItemViewArray: [TalkingFloatScreenItemView] = []
    var danumuItemViewArray: [CardReactiveCompatible] = []

    //: let TapAction: PublishSubject<TalkingFloatingScreenModel?> = PublishSubject()
    let TapAction: PublishSubject<ElementMeasurable?> = PublishSubject()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: appFaceMakeName.map{panelTo(target: $0)}, encoding: .utf8)!)
    }
    ...
    func saltcellar(_ danumuMsgModel: ElementMeasurable?) {
    ...
    @objc func sinceGift(_: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(sinceGift(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension HeaderReactiveCompatible: UIGestureRecognizerDelegate {
    ...
    func alterUp(for danumuMsgModel: ElementMeasurable?, isFirst: Bool) {
        //: let itemView = TalkingFloatScreenItemView(model: danumuMsgModel)
        let itemView = CardReactiveCompatible(model: danumuMsgModel)
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (constGiftText.replacingOccurrences(of: "name", with: "m")))
    ...
    func foulUp(_ isFirst: Bool) {
        ...
        if danumuMsgArray.count > 0 {
            ...
            if let model {
                ...
                alterUp(for: model, isFirst: isFirst)
