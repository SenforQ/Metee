...
fileprivate let show_labUserTitle:[UInt8] = [0xe6,0xeb,0xe6,0xf1,0xa5,0xe0,0xec,0xe1,0xe2,0xef,0xb7,0xa6,0x9d,0xe5,0xde,0xf0,0x9d,0xeb,0xec,0xf1,0x9d,0xdf,0xe2,0xe2,0xeb,0x9d,0xe6,0xea,0xed,0xe9,0xe2,0xea,0xe2,0xeb,0xf1,0xe2,0xe1]

fileprivate func atTo(m num: UInt8) -> UInt8 {
...
@objc protocol PositionObjectProtocol: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func followAcross()

    //: @objc optional func endScroll()
    @objc optional func ofWith()
}

//: class TalkingDanmuMsgListTableView: UIView {
class ButtonDataSource: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_labUserTitle.map{atTo(m: $0)}, encoding: .utf8)!)
    }
...
extension ButtonDataSource {
    ...
    func bring() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(surnameTitle), userInfo: nil, repeats: true)
    }
...
extension ButtonDataSource {
    ...
    func giftIndex(msgModel: StyleDanmuModel?) {
    ...
    func outputCount() {
        ...
        if AllHeight > data_conversationUrl {
            //: if self.msgTableView.height < MsgTableViewHeight {
            if self.msgTableView.height < data_conversationUrl {
                ...
                self.msgTableView.height = data_conversationUrl
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = data_conversationUrl - AllHeight
        ...
        timeEndView(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func timeEndView(animated: Bool) {
    ...
    func inwardInputShow(inPending: Bool) {
...
extension ButtonDataSource: UITableViewDelegate, UITableViewDataSource {
    ...
    func bill(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.ofWith?()
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(constBlockValue)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = ShadowReactiveCompatible.requestPath(tableView: tableView, msg: msgModel, indexPath: indexPath)
