...
fileprivate let constShowData:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Online Users" :*/
fileprivate let showTapFormat:[Character] = ["O","n","l","i","n","e"]
fileprivate let data_listMsg:String = " Userstype return"

/*: "DoingLineViewCell" :*/
fileprivate let data_ofValue:String = "app remove data can imageTalk"
fileprivate let data_targetPath:String = "core self true pop dataveRo"
fileprivate let app_backName:String = "file keyineL"
...
class ViewDataSource: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constShowData.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var dismissBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(clickCell), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var tipsLb: UILabel = {
        ...
        lb.text = (String(showTapFormat) + String(data_listMsg.prefix(6))).localized
        //: return lb
        return lb
        //: }()
    }()
...
extension ViewDataSource {
    ...
    private func commonFirst() {
        //: TalkingAudienceManager().req_liveRoomOnlineList(chatGroupId: self.chatGroupId) { succeed, result, errorModel in
        MagnitudeervalReactiveCompatible().sendSound(chatGroupId: self.chatGroupId) { succeed, result, _ in
            ...
            if succeed {
                ...
                if let datas = Array<UserDataVideoSimulationTransformable>.deserialize(from: array as? Array) {
...
extension ViewDataSource: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let identifier = (String(data_ofValue.suffix(4)) + "ingLi" + String(data_targetPath.suffix(4)) + "omOnl" + String(app_backName.suffix(4)) + "istCell")
        ...
        cell?.overVideoQuantityimate(model: self.DataSource[indexPath.row], index: indexPath.row)
        //: return cell!
        return cell!
    }
