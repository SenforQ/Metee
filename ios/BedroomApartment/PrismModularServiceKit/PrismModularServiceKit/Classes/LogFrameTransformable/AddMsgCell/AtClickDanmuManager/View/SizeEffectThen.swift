...
fileprivate let constInfoMiniStr:[UInt8] = [0xfa,0xfd,0xfa,0xe7,0xbb,0xf0,0xfc,0xf7,0xf6,0xe1,0xa9,0xba,0xb3,0xfb,0xf2,0xe0,0xb3,0xfd,0xfc,0xe7,0xb3,0xf1,0xf6,0xf6,0xfd,0xb3,0xfa,0xfe,0xe3,0xff,0xf6,0xfe,0xf6,0xfd,0xe7,0xf6,0xf7]

private func fieldModel(data num: UInt8) -> UInt8 {
    return num ^ 147
}

/*: "Select mute duration" :*/
fileprivate let showPlayPath:String = "Selectvar super self normal"
fileprivate let appRemoveUrl:String = "e durdismiss index guard"
fileprivate let app_mStr:[Character] = ["a","t","i","o","n"]

/*: "Cirform" :*/
fileprivate let notiLabelMessage:String = "Cirformheight vertical var"

/*: "Mute success" :*/
fileprivate let appDataMsg:[Character] = ["M","u","t","e"," ","s"]
fileprivate let show_colorFormat:String = "cellccess"

/*: "day" :*/
fileprivate let appSizeValue:String = "dcontent"

/*: "days" :*/
fileprivate let showModelStr:String = "barays"

/*:  " :*/
fileprivate let constInputFormat:String = " "
...
class SizeEffectThen: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constInfoMiniStr.map{fieldModel(data: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var dismissBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(logFor), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var tipsLb: UILabel = {
        ...
        lb.font = UIFont.mediumPingfangStatusSizeFont(fontSize: 18)
        //: lb.textColor = UIColor.appTitleColor()
        lb.textColor = UIColor.census()
        //: lb.textAlignment = .center
        lb.textAlignment = .center
        //: lb.text = "Select mute duration".localized
        lb.text = (String(showPlayPath.prefix(6)) + " mut" + String(appRemoveUrl.prefix(5)) + String(app_mStr)).localized
        //: lb.numberOfLines = 0
        lb.numberOfLines = 0
        //: return lb
        return lb
    ...
    lazy var commitBtn: UIButton = {
        ...
        btn.setTitle((String(notiLabelMessage.prefix(7))).localized, for: .normal)
...
extension SizeEffectThen {
    /// 消除视图按钮
    //: @objc private func bgDismissButtonClick() {
    @objc private func logFor() {
        //: dismiss()
        upWithEnd()
    }
    ...
    func panoramicView() {
        ...
        self.contentView.frame = CGRect(x: (notiEnabledRecordUrl - contentV_Size.width) / 2, y: main_versionEnterTimeStr, width: contentV_Size.width, height: contentV_Size.height)

        //: UIView.animate(withDuration: 0.3) {
        UIView.animate(withDuration: 0.3) {
            //: self.contentView.frame.origin.y = (ScreenHeight-self.contentV_Size.height)/2
            self.contentView.frame.origin.y = (main_versionEnterTimeStr - self.contentV_Size.height) / 2
        }
    }
    ...
    func upWithEnd() {
        ...
        UIView.animate(withDuration: 0.3) {
            ...
            self.contentView.frame.origin.y = main_versionEnterTimeStr
...
extension SizeEffectThen {
    ...
    @objc private func domino(btn: UIButton) {
    ...
    @objc private func fixingUser() {
        ...
        if self.viewType == .live {
            ...
            MagnitudeervalReactiveCompatible().searchCompletion(targetUid: targetUid, duration: index) { succeed, _, _ in
                //: ProgressHUD.dismiss()
                DescriptionProgressHUD.actionOf()
                //: if succeed {
                if succeed {
                    //: self.func__showStatusBarSuccessMsg(showMsg: "Mute success".localized)
                    self.clickFill(showMsg: (String(appDataMsg) + show_colorFormat.replacingOccurrences(of: "cell", with: "u")).localized)
                }
                //: self.dismiss()
                self.upWithEnd()
            }
            ...
            LogReqTool.andSo(targetUid: targetUid, roomID: ParlourEventHandler.tarotCard().partyModel.roomId, duration: index) { succeed, _, _ in
                //: ProgressHUD.dismiss()
                DescriptionProgressHUD.actionOf()
                //: if succeed {
                if succeed {
...
extension SizeEffectThen {
    ...
    private func distance() {
        ...
        for i in 0 ..< 3 {
            ...
            btn.dataWith(color: UIColor.outsideBecomeColor(), forState: .selected)
            ...
            let dayStr = (title == "1") ? (appSizeValue.replacingOccurrences(of: "content", with: "ay")).localized : (showModelStr.replacingOccurrences(of: "bar", with: "d")).localized
            //: btn.setTitle("\(title) " + dayStr, for: .normal)
            btn.setTitle("\(title) " + dayStr, for: .normal)
            //: btn.tag = 100+i
            btn.tag = 100 + i
            //: btn.addTarget(self, action: #selector(muteBtnClick), for: .touchUpInside)
            btn.addTarget(self, action: #selector(domino), for: .touchUpInside)
