...
fileprivate let mainBringName:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "bth_live_follow_pre" :*/
fileprivate let show_toUrl:String = "view elsebth_li"
fileprivate let data_barText:[Character] = ["o","w","_","p","r","e"]

/*: "btn_live_closed_nor" :*/
fileprivate let const_videoUrl:[Character] = ["b","t","n","_","l","i","v","e","_","c","l","o","s","e"]
fileprivate let show_backgroundMsg:[Character] = ["d","_","n","o","r"]

/*: "removeAttentionUid" :*/
fileprivate let dataViewValueMsg:String = "REMOV"
fileprivate let constTitleStr:String = "center equalntionUi"
fileprivate let showCenterUrl:String = "text"

/*: "attentionUid" :*/
fileprivate let k_representationMsg:String = "abarention"
fileprivate let userMatchUrl:String = "Uidview edit case"

/*: "source" :*/
fileprivate let dataFillUrl:String = "soubye"

/*: "num" :*/
fileprivate let constOfText:[Character] = ["n","u","m"]

/*: "99+" :*/
fileprivate let const_formatPath:[UInt8] = [0x2b,0x39,0x39]

/*: "UID: :*/
fileprivate let const_lengthName:[Character] = ["U","I","D",":"]
...
class EncephalonView: UIView {
    ...
    private var partyModel = MoveMeasurable()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainBringName.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var iconBgBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(modelIcon), for: .touchUpInside)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var followBtn: UIButton = {
        ...
        btn.setImage(UIImage.addBar(name: (String(show_toUrl.suffix(6)) + "ve_foll" + String(data_barText))), for: .normal)
        //: btn.addTarget(self, action: #selector(followButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(moveFee), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var miniBtn: UIButton = {
        ...
        btn.setImage(UIImage.addBar(name: (String(const_videoUrl) + String(show_backgroundMsg))), for: .normal)
    ...
    private lazy var onlineNumBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(hurry), for: .touchUpInside)
    ...
    private lazy var onlineListView: VideoToListView = {
        //: let v = TalkingVoiceRoomOnlineListView()
        let v = VideoToListView()
...
extension EncephalonView {
    ...
    @objc func onDown(uid: Int, isAttention: Bool, completion: ((_ succeed: Bool) -> Void)? = nil) {
        ...
        if isAttention {
            ...
            dict[(dataViewValueMsg.lowercased() + "eAtte" + String(constTitleStr.suffix(7)) + showCenterUrl.replacingOccurrences(of: "text", with: "d"))] = uid
            //: } else {
        } else {
            //: dict["attentionUid"] = uid
            dict[(k_representationMsg.replacingOccurrences(of: "bar", with: "tt") + String(userMatchUrl.prefix(3)))] = uid
            //: dict["source"] = "2"
            dict[(dataFillUrl.replacingOccurrences(of: "by", with: "rc"))] = "2"
        }
        //: PictureRequestTool.req_atationTool(isAttention: !isAttention, params: dict) { succeed, _, _ in
...
extension EncephalonView {
    ...
    @objc private func beyondNext() {
        //: TalkingVoiceRoomReqTool.req_partyOnlineNum(roomId: partyModel.roomId) { succeed, result, _ in
        LogReqTool.caseCompletion(roomId: partyModel.roomId) { succeed, result, _ in
            //: guard succeed else { return }
            guard succeed else { return }
            //: let json = JSON(result ?? [String: Any]())
            let json = JSON(result ?? [String: Any]())
            //: self.onlineNum = json["num"].intValue
            self.onlineNum = json[(String(constOfText))].intValue
    ...
    private func modeTimer() {
        ...
        onlineNunTimer = Timer.scheduledTimer(timeInterval: 60, target: self, selector: #selector(beyondNext), userInfo: nil, repeats: true)
        //: onlineNunTimer?.fire()
        onlineNunTimer?.fire()
    }
...
extension EncephalonView {
    ...
    func bestirOpeneself() {
        ...
        icon.equalFinish(urlStr: partyModel.streamerInfo.headPic, placeImg: UIImage.componentTo(sex: partyModel.streamerInfo.sex), options: optionsInfo)
        // 是否是主播
        //: let isAnchor = (String(partyModel.streamerInfo.uid) == RegularBeginDecisionMakerAppManager.share.loginUid)
        let isAnchor = (String(partyModel.streamerInfo.uid) == RegularBeginDecisionMakerAppManager.share.loginUid)
        //: if isAnchor {
        if isAnchor {
            //: refreshFollow(isAttention: true)
            refreshInsideAttention(isAttention: true)
            //: } else {
        } else {
            //: refreshFollow(isAttention: partyModel.streamerInfo.isAttention)
            refreshInsideAttention(isAttention: partyModel.streamerInfo.isAttention)
        }
        // 定时更新在线人数
        //: startTimer()
        modeTimer()
    ...
    @objc private func moveFee() {
        //: req_followRequest(uid: partyModel.streamerInfo.uid, isAttention: partyModel.streamerInfo.isAttention) { [weak self] succeed in
        onDown(uid: partyModel.streamerInfo.uid, isAttention: partyModel.streamerInfo.isAttention) { [weak self] succeed in
            //: guard let self = self else { return }
            guard let self = self else { return }
            //: guard succeed else { return }
            guard succeed else { return }
            ...
            self.refreshInsideAttention(isAttention: self.partyModel.streamerInfo.isAttention)
        }
    }
    ...
    func refreshInsideAttention(isAttention: Bool) {
    ...
    @objc private func hurry() {
        //: onlineListView.showView()
        onlineListView.airSacOpticStraddle()
    }
}
...
extension EncephalonView {
    ...
    private func underConstraint() {
        //: frame = CGRect(x: 0, y: 0, width: ScreenWidth, height: StatusBarHeight+actualWidth(w: 43))
        frame = CGRect(x: 0, y: 0, width: notiEnabledRecordUrl, height: showPlainValue + actualWidth(w: 43))
