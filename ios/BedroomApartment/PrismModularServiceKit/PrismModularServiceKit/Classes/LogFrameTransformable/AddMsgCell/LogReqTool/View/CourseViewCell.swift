...
fileprivate let app_makeValue:[UInt8] = [0xc7,0xc0,0xc7,0xda,0x86,0xcd,0xc1,0xca,0xcb,0xdc,0x94,0x87,0x8e,0xc6,0xcf,0xdd,0x8e,0xc0,0xc1,0xda,0x8e,0xcc,0xcb,0xcb,0xc0,0x8e,0xc7,0xc3,0xde,0xc2,0xcb,0xc3,0xcb,0xc0,0xda,0xcb,0xca]

/*: "#FA9D33" :*/
fileprivate let show_picValue:[Character] = ["#","F","A","9","D"]
fileprivate let app_addTimeData:String = "record"

/*: "#343338" :*/
fileprivate let showLivePath:String = "#343338"
...
class CourseViewCell: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_makeValue.map{$0^174}, encoding: .utf8)!)
    }
...
extension CourseViewCell {
    /// 刷新cell
    /// - Parameters:
    ///   - position: 麦位
    ///   - model: 麦位数据
    //: func refreshIcon(position: Int, model: TalkingMikeListItemModel) {
    func finishAcross(position: Int, model: ModelHandyJSON) {
        //: guard model.uid > 0, String(model.uid) != RegularBeginDecisionMakerAppManager.share.loginUid else {
        guard model.uid > 0, String(model.uid) != RegularBeginDecisionMakerAppManager.share.loginUid else {
        ...
        let color = model.needGift ? UIColor(hex: (String(show_picValue) + app_addTimeData.replacingOccurrences(of: "record", with: "33"))) : UIColor(hex: (showLivePath.capitalized))
