...
fileprivate let showVideoListKey:[UInt8] = [0xb3,0xb4,0xb3,0xae,0xf2,0xb9,0xb5,0xbe,0xbf,0xa8,0xe0,0xf3,0xfa,0xb2,0xbb,0xa9,0xfa,0xb4,0xb5,0xae,0xfa,0xb8,0xbf,0xbf,0xb4,0xfa,0xb3,0xb7,0xaa,0xb6,0xbf,0xb7,0xbf,0xb4,0xae,0xbf,0xbe]

private func caseAdd(size num: UInt8) -> UInt8 {
    return num ^ 218
}

/*: "icon_photo_giftbg" :*/
fileprivate let show_makeUrl:[Character] = ["i","c","o","n","_","p","h","o","t","o","_"]
fileprivate let notiErrorMessage:String = "event"
fileprivate let constViewShowMsg:String = "ifhiddeng"

/*: "Send" :*/
fileprivate let dataGiftValue:[Character] = ["S","e","n","d"]

/*: "nav_back_black_nor" :*/
fileprivate let main_shareAddId:String = "nav_bacstring var"
fileprivate let k_changeRawMsg:[Character] = ["k","_","b","l","a","c","k","_","n","o","r"]

/*: "toUid" :*/
fileprivate let app_sendFormat:String = "app image data bottom viewtoUid"

/*: "giftId" :*/
fileprivate let noti_windowValue:String = "giftIdbutton bar if show scope"

/*: "giftNum" :*/
fileprivate let constTargetKey:String = "image ifgiftNum"

/*: "unlockMsgId" :*/
fileprivate let app_statusRequestPath:[Character] = ["u","n","l","o","c","k"]
fileprivate let appLabKey:String = "MsgIdcolor make"

/*: "Need a %@ (%@ Gold coins) to unlock" :*/
fileprivate let k_labelUrl:[UInt8] = [0x24,0x3b,0x3b,0x3a,0xf6,0x37,0xf6,0xfb,0x16,0xf6,0xfe,0xfb,0x16,0xf6,0x1d,0x45,0x42,0x3a,0xf6,0x39,0x45,0x3f,0x44,0x49,0xff,0xf6,0x4a,0x45,0xf6,0x4b,0x44,0x42,0x45,0x39,0x41]

fileprivate func controlVar(time num: UInt8) -> UInt8 {
...
/*: "icon_photo_lock" :*/
fileprivate let kAddMsg:String = "icon_psession image for let"
fileprivate let data_objectKey:[Character] = ["h","o","t","o","_","l","o","c","k"]

/*: "Click on the screen to view 15s" :*/
fileprivate let constTransformName:[UInt8] = [0x73,0x35,0x31,0x20,0x77,0x65,0x69,0x76,0x20,0x6f,0x74,0x20,0x6e,0x65,0x65,0x72,0x63,0x73,0x20,0x65,0x68,0x74,0x20,0x6e,0x6f,0x20,0x6b,0x63,0x69,0x6c,0x43]

/*: "icon_photo_burn" :*/
fileprivate let noti_progressName:[Character] = ["i","c","o","n","_","p","h","o","t","o","_","b","u","r","n"]

/*: "Already burned" :*/
fileprivate let noti_topTextModelPath:[Character] = ["A","l","r","e","a","d","y"]
fileprivate let const_listId:[Character] = [" ","b"]
fileprivate let noti_genderData:[Character] = ["u","r","n","e","d"]
...
typealias IntimatePhotoBlock = (_ model: AlongModel) -> Void

//: class TalkingIntimatePhotoPreviewVC: TalkingBaseViewController {
class FromLikePreviewVc: ScoreViewController {
	var letterOff: Bool = false
	var topToArray: [AnyHashable] = []
	var huntDictionary: [AnyHashable: String] = [:]
	var checkOn: Bool = true
	var sourceArray: [AnyHashable] = []
    ...
    //: private var imageTotalData = SuiteCellData(direction: .MsgDirectionOutgoing)
    private var imageTotalData = SuiteCellData(direction: .MsgDirectionOutgoing) // 图片消息数据
    //: private var photoModel = AbTalkingChatMsgIntimatePhotoModel()
    private var photoModel = AlongModel() // 当前私密照片模型
    ...
    ///   - imageCellData: 图片模型
    ///   - toUid: 对方Id
    //: init(imageCellData: SuiteCellData, toUid: String) {
    init(imageCellData: SuiteCellData, toUid: String) {
        ...
        //: targetId = toUid
        targetId = toUid
        //: isMySelf = (imageTotalData.msgModel.user.uid == Int(RegularBeginDecisionMakerAppManager.share.loginUserMode.userID))
        isMySelf = (imageTotalData.msgModel.user.uid == Int(RegularBeginDecisionMakerAppManager.share.loginUserMode.userID))
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: showVideoListKey.map{caseAdd(size: $0)}, encoding: .utf8)!)
    }
    ...
    override func viewDidDisappear(_ animated: Bool) {
        ...
        jabStart(isOpen: true)
            ...
            if (intimateStatusImgV.layer.anchorPointZ != 0) && (intimateStatusImgV.motionEffects.count == 17) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let tagOf = ClipBlockView()
            
            tagOf.donationMoveOff = { [self] narrowClose in
            self.checkOn = narrowClose
            
            var tagOf = animated
            tagOf = true
            if tagOf != self.checkOn {
                self.checkOn = tagOf
            }
            ...
            self.sourceArray = backOfArray
            
            guard var value = self.sourceArray as? [String] else {
                return nil
            }
            return value
            }
            tagOf.handleDictionary = { [self] fromAgeDictionary in
            self.conversationDictionary = fromAgeDictionary
            ...
            return value
                ...
                intimateStatusImgV.addSubview(tagOf)
            }
    ...
    private lazy var giftBgView: UIImageView = {
        ...
        img.image = UIImage.addBar(name: (String(show_makeUrl) + notiErrorMessage.replacingOccurrences(of: "event", with: "g") + constViewShowMsg.replacingOccurrences(of: "hidden", with: "tb")))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var giftIcon: UIImageView = {
        ...
        img.step(urlStr: photoModel.giftIcon)
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var giftSendBtn: UIButton = {
        ...
        btn.setTitle((String(dataGiftValue)).localized, for: .normal)
    ...
    lazy var backBtn: UIButton = {
        ...
        let img = UIImage.addBar(name: (String(main_shareAddId.prefix(7)) + String(k_changeRawMsg))).withRenderingMode(.alwaysTemplate)
        //: btn.setImage(img, for: .normal)
        btn.setImage(img, for: .normal)
        //: btn.tintColor = .white
        btn.tintColor = .white
        //: btn.addTarget(self, action: #selector(backButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(highProfile), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension FromLikePreviewVc {
    /// 调用送礼接口
    /// - Parameters:
    ///   - giftId: 礼物Id
    ///   - giftNum: 礼物数量
    //: func req_sendGiftMsg(giftId: String, giftNum: String) {
    func headKey(giftId: String, giftNum: String) {
        ...
        params[(String(app_sendFormat.suffix(5)))] = self.targetId
        //: params["giftId"] = giftId
        params[(String(noti_windowValue.prefix(6)))] = giftId
        //: params["giftNum"] = giftNum
        params[(String(constTargetKey.suffix(7)))] = giftNum
        //: params["unlockMsgId"] = self.imageTotalData.msgModel.msgInfo.msgId
        params[(String(app_statusRequestPath) + String(appLabKey.prefix(5)))] = self.imageTotalData.msgModel.msgInfo.msgId
...
extension FromLikePreviewVc {
    ...
    @objc private func speakTo() {
        ...
        headKey(giftId: photoModel.giftId, giftNum: "1")
    }

    /// 阅后即焚15s倒计时开始
    //: @objc private func burnPhotoTapGesture() {
    @objc private func explicateUp() {
        ...
        self.ablate(urlStr: photoModel.originUrl, isMosaic: false) { [weak self] isSucceed in
            //: ProgressHUD.dismiss()
            ...
            self.countdownTimer = Timer.scheduledTimer(withTimeInterval: 0.1, block: { [weak self] _ in
                ...
                self.progressView.imageBy(totalTime: 150, currTime: time)
                //: time -= 1
                time -= 1
                //: }, repeats: true)
            }, repeats: true)
...
extension FromLikePreviewVc {
    ...
    private func photoUi() {
        ...
        if photoModel.lockStatus == 1 { // 锁定状态，需要送礼解锁
            ...
            self.desLab.text = String(bytes: k_labelUrl.map{controlVar(time: $0)}, encoding: .utf8)!.fast(photoModel.giftName, photoModel.giftPrice)
            ...
            if photoModel.burnStatus == 0 { // 未销毁
                ...
                self.intimateStatusImgV.image = UIImage.addBar(name: (String(kAddMsg.prefix(6)) + String(data_objectKey)))
                //: self.desLab.text = "Click on the screen to view 15s".localized
                self.desLab.text = String(bytes: constTransformName.reversed(), encoding: .utf8)!.localized
                // 添加手势
                //: let tap = UITapGestureRecognizer(target: self, action: #selector(burnPhotoTapGesture))
                let tap = UITapGestureRecognizer(target: self, action: #selector(explicateUp))
                ...
                self.intimateStatusImgV.image = UIImage.addBar(name: (String(noti_progressName)))
                //: self.desLab.text = "Already burned".localized
                self.desLab.text = (String(noti_topTextModelPath) + String(const_listId) + String(noti_genderData)).localized
            }
        ...
        self.ablate(urlStr: imgUrlStr, isMosaic: imgIsMosaic)
    }
    ...
    private func ablate(urlStr: String, isMosaic: Bool, completionHandler: ((_ isSucceed: Bool) -> Void)? = nil) {
