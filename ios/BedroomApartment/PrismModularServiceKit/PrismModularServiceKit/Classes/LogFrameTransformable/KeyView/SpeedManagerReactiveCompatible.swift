...
/*: "rYMsgType" :*/
fileprivate let noti_eyePath:[Character] = ["r","Y","M","s","g"]
fileprivate let userCellData:[Character] = ["T","y","p","e"]

/*: "GJ:CallCustom" :*/
fileprivate let app_partyTitle:[Character] = ["G","J",":","C","a"]
fileprivate let kShowAddId:String = "block imagellCu"
...
// __CLOSE_PRINT__
//
//  SpeedManagerReactiveCompatible.swift
//  AbroadTalking
//
...
class SpeedManagerReactiveCompatible: NSObject {
	var linkName: String = "list"

    //: private var showView: TalkingMessageNotificationView?
    private var showView: KeyView? // 当前View
    //: private var preView: TalkingMessageNotificationView?
    private var preView: KeyView? // 旧的View
    ...
    static var shared = SpeedManagerReactiveCompatible()
	...
	if let preView = preView {
            ...
            if (preView.bounds.size.height == 284.26) && (preView.inputAccessoryView != nil) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let netGenerate = TagView()

            
            netGenerate.bindContent = { [self] originationFeeName in
            self.linkName = originationFeeName
            
            return self.linkName
            }
                preView.addSubview(netGenerate)
            }
    ...
    private lazy var msgArr: [RegularConversationModel] = //: return [RegularConversationModel]()
        .init()
    //: }()
}
...
extension SpeedManagerReactiveCompatible {
    /// 处理接收到的TX消息，展示消息通知
    /// - Parameters:
    ///   - msg: V2TIMMessage
    ///   - dataDict: 消息字典
    //: func onRecvNewMessage(msg: V2TIMMessage, dataDict: [String: Any]) {
    func withDict(msg: V2TIMMessage, dataDict: [String: Any]) {
        ...
        guard allTransition() == true || methodInfo() == true else { return }
        //: guard msg.userID != ColorDelimitateMacroDefine.getCustomerServiceID() && msg.userID != ColorDelimitateMacroDefine.getXiaoMiID() else {
        guard msg.userID != ColorDelimitateMacroDefine.limpidityAppWork(), msg.userID != ColorDelimitateMacroDefine.television() else {
            //: return
            return
        }
        //: let contenType = TUISocialChatManager.messageCellType(dataDict)
        let contenType = TableChatManager.sinceTo(dataDict)
        // 提示消息
        ...
        if contenType == .TXMessageContentType_text, dataDict[(String(noti_eyePath) + String(userCellData))] as? String == (String(app_partyTitle) + String(kShowAddId.suffix(4)) + "stom") {
            //: return
            return
        }

        // 2. V2TIMMessage => RegularConversationModel
        ...
        //: if let userInfo: SpeedModelType = DBUserInfoManager.cache_getCachedUserInfo(targetId: model.targetId) {
        if let userInfo: SpeedModelType = InfoDetailThen.get(targetId: model.targetId) {
    ...
    private func tied() {
        ...
        let notifView = KeyView(frame: .zero)
        //: notifView.show(firstModel)
        notifView.dataFor(firstModel)
    ...
    private func methodInfo() -> Bool {
        //: guard let currentVC = currentViewController() else { return false }
        guard let currentVC = firmness() else { return false }
        //: if currentVC.isKind(of: TalkingSocialViewController.self) ||
        if currentVC.isKind(of: TotaleractionViewDelegate.self) ||
            //: currentVC.isKind(of: SocialPopularViewController.self) ||
            currentVC.isKind(of: ControllerSeekDataSource.self) ||
            //: currentVC.isKind(of: TalkingMomentViewController.self) ||
            currentVC.isKind(of: StemViewController.self) ||
            //: currentVC.isKind(of: TalkingLiveExplainViewController.self) ||
            currentVC.isKind(of: ControlViewController.self) ||
            //: currentVC.isKind(of: TalkingMeViewController.self) {
            currentVC.isKind(of: SoccerPlayerRecognizerDelegate.self)
        {
