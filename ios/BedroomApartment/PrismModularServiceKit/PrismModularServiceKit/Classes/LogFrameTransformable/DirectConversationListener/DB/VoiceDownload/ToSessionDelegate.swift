...
fileprivate let constModelMessage:[Character] = ["D","o","c","u","m","e","n","t","s","/","U","s","e","r","/","v","o","i"]
fileprivate let mainBarUrl:String = "object photoce/"

/*: "Documents/User/record/" :*/
fileprivate let notiHiddenName:String = "view selfDocum"
fileprivate let appManagerValue:String = "remove var/User"
fileprivate let dataFillEnableMsg:String = "equal iford/"

/*: "cancelled" :*/
fileprivate let main_makeViewName:[Character] = ["c","a","n","c","e","l","l","e","d"]

/*: .wav" :*/
fileprivate let kPlayShowPath:[Character] = [".","w","a","v"]
...
typealias OneKeyGreetDownloadBlock = (_ model: DescriptionReactiveCompatible) -> Void

//: protocol VoiceDownloadTaskManagerDelegate: NSObjectProtocol {
protocol ViewManagerDelegate: NSObjectProtocol {
    /// 下载完成
    ...
    func program(model: DescriptionReactiveCompatible)
    /// 过期语音消息，服务器缓存时间为7天
...
class ToSessionDelegate: NSObject {
...
extension ToSessionDelegate {
    /// 添加下载任务
    //: func addDownloadTasks(_ tasksArray: [VoiceDownloadTaskModel]) {
    func equalReport(_ tasksArray: [DescriptionReactiveCompatible]) {
        ...
        for model in filterArray {
            ...
            if model.zipFilePath.isEmpty {
                ...
                model.zipFilePath = model.beyondFrom()
            }

            // 重编译，相对路径会变
            //: if model.zipFilePath.contains("Documents/User/voice/") {
            if model.zipFilePath.contains((String(constModelMessage) + String(mainBarUrl.suffix(3)))) {
                //: model.zipFilePath = "\(StreamEffectCacheDefine.getChatVoicePath())\((model.zipFilePath as NSString).lastPathComponent)"
                model.zipFilePath = "\(StreamEffectCacheDefine.toDrop())\((model.zipFilePath as NSString).lastPathComponent)"
                //: } else if model.zipFilePath.contains("Documents/User/record/") {
            } else if model.zipFilePath.contains((String(notiHiddenName.suffix(5)) + "ents" + String(appManagerValue.suffix(5)) + "/rec" + String(dataFillEnableMsg.suffix(4)))) {
                //: model.zipFilePath = "\(StreamEffectCacheDefine.getChatRecordPath())\((model.zipFilePath as NSString).lastPathComponent)"
                model.zipFilePath = "\(StreamEffectCacheDefine.underList())\((model.zipFilePath as NSString).lastPathComponent)"
            }
            ...
            ControlThen.addBy(msgId: model.taskId, status: "\(BridgeValueConvertible.prepare.rawValue)", filePath: model.zipFilePath)
    ...
    func job(_ audioInfo: [String: Any], completeHandle: @escaping OneKeyGreetDownloadBlock) {
        ...
        var taskModel = DescriptionReactiveCompatible.one(audioInfo)
        ...
        if self.downloadingModelDic.keys.contains(taskModel.downloadUri) {
            ...
            taskModel = self.downloadingModelDic[taskModel.downloadUri] ?? DescriptionReactiveCompatible()
        }

        // 重编译，相对路径会变
        //: if taskModel.zipFilePath.contains("Documents/User/voice/") {
        if taskModel.zipFilePath.contains((String(constModelMessage) + String(mainBarUrl.suffix(3)))) {
            //: taskModel.zipFilePath = "\(StreamEffectCacheDefine.getChatVoicePath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            taskModel.zipFilePath = "\(StreamEffectCacheDefine.toDrop())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            //: } else if taskModel.zipFilePath.contains("Documents/User/record/") {
        } else if taskModel.zipFilePath.contains((String(notiHiddenName.suffix(5)) + "ents" + String(appManagerValue.suffix(5)) + "/rec" + String(dataFillEnableMsg.suffix(4)))) {
            //: taskModel.zipFilePath = "\(StreamEffectCacheDefine.getChatRecordPath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            taskModel.zipFilePath = "\(StreamEffectCacheDefine.underList())\((taskModel.zipFilePath as NSString).lastPathComponent)"
        }
...
extension ToSessionDelegate: URLSessionDataDelegate {
    // 开始
    ...
    func urlSession(_: URLSession, dataTask: URLSessionDataTask, didReceive _: URLResponse, completionHandler: @escaping (URLSession.ResponseDisposition) -> Void) {
        ...
        let totalBytesWritten = DescriptionReactiveCompatible.doPath(path: taskModel.zipFilePath)
    ...
    func urlSession(_: URLSession, dataTask: URLSessionDataTask, didReceive data: Data) {
        ...
        self.delegate?.program(model: taskModel)
    }
    ...
    func urlSession(_: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        if error != nil || errorCode != 200 {
            ...
            if let error = error, (error as NSError).userInfo[NSLocalizedDescriptionKey] as? String == (String(main_makeViewName)) {
                //: if taskModel.taskStatus == .pause {
                if taskModel.taskStatus == .pause {
                    //: pauseTask(url: task.taskDescription ?? "", isSuspend: true)
                    exhibit(url: task.taskDescription ?? "", isSuspend: true)
                    //: } else if taskModel.taskStatus == .cancel {
                } else if taskModel.taskStatus == .cancel {
                    //: self.cancelTask(url: task.taskDescription ?? "")
                    self.pick(url: task.taskDescription ?? "")
                }
                //: } else {
            } else {
                ...
                exhibit(url: task.taskDescription ?? "", isSuspend: false)
            }
            //: } else {
        } else {
            ...
            self.inputTask(url: task.taskDescription ?? "")
        }
...
extension ToSessionDelegate {
    ...
    func exhibit(url: String, isSuspend: Bool) {
        ...
        case .chatRecord:
            ...
            let status = isSuspend ? "\(BridgeValueConvertible.pause.rawValue)" : "\(BridgeValueConvertible.error.rawValue)"
            //: WCDBVoiceMsgTable.db_updateVoiceMsg(msgId: taskModel.taskId, status: status)
            ControlThen.addBy(msgId: taskModel.taskId, status: status)
            //: if isSuspend == false {
            if isSuspend == false {
                //: self.delegate?.voiceDownloadError(model: taskModel)
                self.delegate?.phonation(model: taskModel)
            }
    ...
    func pick(url: String) {
        ...
        case .chatRecord:
            ...
            ControlThen.perimeterLevel(msgId: taskModel.taskId)
            //: self.delegate?.voiceDownloadCancel(model: taskModel)
            self.delegate?.slowing(model: taskModel)
    ...
    func inputTask(url: String) {
        ...
        let size = DescriptionReactiveCompatible.doPath(path: taskModel.zipFilePath)
        //: if size > 0 {
        if size > 0 {
            ...
            if taskModel.taskTypeId == .chatRecord {
                ...
                ControlThen.addBy(msgId: taskModel.taskId, status: "\(BridgeValueConvertible.finish.rawValue)")
            }

            //: let filePath = "\(taskModel.zipFilePath).wav"
            let filePath = "\(taskModel.zipFilePath)" + (String(kPlayShowPath))
            ...
            self.natalDay(at: taskModel.zipFilePath, withKeyId: taskModel.taskId, withTypeId: taskModel.taskTypeId) { _ in
                ...
                case .chatRecord:
                    ...
                    self.delegate?.itemModel(model: taskModel)
            ...
            case .chatRecord:
                ...
                ControlThen.addBy(msgId: taskModel.taskId, status: "\(BridgeValueConvertible.expired.rawValue)")
                //: self.delegate?.voiceDownloadExpired(model: taskModel)
                self.delegate?.sex(model: taskModel)
    ...
    func natalDay(at path: String, withKeyId keyId: String, withTypeId typeId: BallBreakerValueConvertible, complete: @escaping (Error?) -> Void) {
        //: DispatchQueue.global().async {
        DispatchQueue.global().async {
            //: if let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
            if let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
                //: let newPath = "\(path).wav"
                let newPath = "\(path)" + (String(kPlayShowPath))
                ...
                if let handle = FileHandle(forWritingAtPath: newPath) {
                    ...
                    let unzippedData = data.gunzippedConstraint()
                    ...
                    if typeId == .chatRecord {
                        ...
                        ControlThen.addBy(msgId: keyId, filePath: newPath)
                    }
