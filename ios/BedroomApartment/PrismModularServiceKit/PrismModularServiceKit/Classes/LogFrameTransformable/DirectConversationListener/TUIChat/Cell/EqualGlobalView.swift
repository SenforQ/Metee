...
fileprivate let app_labPath:String = "view ifTSyst"
fileprivate let notiUserStr:String = "sageCsucceed path message normal white"
fileprivate let notiTimeTitle:String = "ell_not price bar"

/*: "init(coder:) has not been implemented" :*/
fileprivate let notiShowKey:[UInt8] = [0xf,0x8,0xf,0x12,0x4e,0x5,0x9,0x2,0x3,0x14,0x5c,0x4f,0x46,0xe,0x7,0x15,0x46,0x8,0x9,0x12,0x46,0x4,0x3,0x3,0x8,0x46,0xf,0xb,0x16,0xa,0x3,0xb,0x3,0x8,0x12,0x3,0x2]

private func pressuriseShadow(original num: UInt8) -> UInt8 {
    return num ^ 102
}

/*: "icon_talk_notic" :*/
fileprivate let kOpValue:String = "enter cellicon_t"
fileprivate let dataGiftStr:[Character] = ["o","t","i","c"]
...
@objc public protocol ClickSumeractionCellDelegate: NSObjectProtocol {
    /**
     *  长按消息回调
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onLongPressMessage(cell: EqualGlobalView)
    func cipher(cell: EqualGlobalView)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onRetryMessage(cell: EqualGlobalView)
    func doingRetryCompartmentMessage(cell: EqualGlobalView)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessage(cell: EqualGlobalView)
    func makeSelectedContent(cell: EqualGlobalView)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessageAvatar(cell: EqualGlobalView)
    func constraintVoice(cell: EqualGlobalView)

    /**
     *  点击消息单元中引用消息
     *  @param cell 委托者，消息单元
     */
    //: func onReplyMessage(cell: EqualGlobalView)
    func bigFrom(cell: EqualGlobalView)
}

//: @objcMembers public class EqualGlobalView: TCommonTableViewCell {
@objcMembers public class EqualGlobalView: TableReactiveCompatible {
	var aggregationMagnitude: Double = 95.7
	var timeDictionary: [AnyHashable: String] = [:]
	var makeFullCount: Double = 98.6
	var milkDictionary: [AnyHashable: String] = [:]
	var dieKeyNumber: Double = 54.3
	var needDictionary: [AnyHashable: String] = [:]

    /**
     *  信息数据类。
     *  存储了该massageCell中所需的信息。包括发送者 ID，发送者头像、信息发送状态、信息气泡图标等。
     *  messageData 详细信息请参考：Section\Chat\CellData\QuantityeractionReactiveCompatible.h
     */
    //: public var messageData: QuantityeractionReactiveCompatible?
    public var messageData: QuantityeractionReactiveCompatible?

    /// 复用cell标识符
    //: @objc public static let TSystemMessageCell_ReuseId = "TSystemMessageCell_ReuseId"
    public static let TSystemMessageCell_ReuseId = (String(app_labPath.suffix(5)) + "emMes" + String(notiUserStr.prefix(5)) + String(notiTimeTitle.prefix(4)) + "ReuseId")

    /**
     *  协议委托
     *  负责实现 ClickSumeractionCellDelegate 协议中的功能。
     */
    //: public weak var delegate: TMessageCellDelegate?
    public weak var delegate: ClickSumeractionCellDelegate?
    ...
    override public func awakeFromNib() {
		...
		if var prevalenceValue = self.messageData?.name { 
			if var controlValue = messageData?.messageType { 
		            if (indicator.inputAccessoryView != nil) && (indicator.backgroundColor != nil && indicator.backgroundColor!.cgColor == UIColor.red.cgColor) {
		                //: SWIFT_CUSTOM_DANGER_File_Call
		                let forRowLet = ChaseView()
		            forRowLet.giftSum = controlValue
	
		            forRowLet.toName = prevalenceValue
		            forRowLet.sumRelationCellSum = { [self] atNumber in
		            self.aggregationMagnitude = atNumber
		            
		            return self.aggregationMagnitude
		            }
		            forRowLet.throughDictionary = { [self] exploitDictionary in
		            self.timeDictionary = exploitDictionary
		            ...
		            return value
		                ...
		                indicator.addSubview(forRowLet)
		            }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiShowKey.map{pressuriseShadow(original: $0)}, encoding: .utf8)!)
    }
    ...
    override public func revolution(with data: AggregationThen) {
        //: super.fill(with: data)
        super.revolution(with: data)
        //: let newData: QuantityeractionReactiveCompatible = data as! QuantityeractionReactiveCompatible
        let newData: QuantityeractionReactiveCompatible = data as! QuantityeractionReactiveCompatible
		...
		if var nameValue = data.reuseId { 
			if var commentBubbleValue = self.messageData?.messageType { 
		            if (indicator.layer.zPosition == 67.33) && (indicator.constraintsAffectingLayout(for: .horizontal).count == 83) {
		                //: SWIFT_CUSTOM_DANGER_File_Call
		                let itemControl = ChaseView(frame: indicator.frame.union(CGRect(x: CGFloat(463.03), y: CGFloat(88.92), width: CGFloat(0), height: CGFloat(76))))
		            itemControl.giftSum = commentBubbleValue
	
		            itemControl.toName = nameValue
		            itemControl.sumRelationCellSum = { [self] atNumber in
		            self.dieKeyNumber = atNumber
		            
		            return self.dieKeyNumber
		            }
		            itemControl.throughDictionary = { [self] exploitDictionary in
		            self.needDictionary = exploitDictionary
		            
		            guard var value = self.needDictionary as? [String: String] else {
		                return nil
		            }
		            return value
		            }
		                indicator.addSubview(itemControl)
		            }
    ...
    override public func updateConstraints() {
        ...
        let csize = self.messageData?.beyondName()
		...
		if var filmValue = self.messageData?.identifier { 
			if var sectionValue = self.messageData?.messageType { 
		            if (avatarView.inputAccessoryView != nil) && (avatarView.backgroundColor != nil && avatarView.backgroundColor!.cgColor == UIColor.red.cgColor) {
		                //: SWIFT_CUSTOM_DANGER_File_Call
		                let forRowLet = ChaseView()
		            forRowLet.giftSum = sectionValue
	
		            forRowLet.toName = filmValue
		            forRowLet.sumRelationCellSum = { [self] atNumber in
		            self.makeFullCount = atNumber
		            
		            return self.makeFullCount
		            }
		            forRowLet.throughDictionary = { [self] exploitDictionary in
		            self.milkDictionary = exploitDictionary
		            ...
		            return value
		                ...
		                avatarView.addSubview(forRowLet)
		            }
    ...
    lazy var avatarView: UIImageView = {
        ...
        let tap1 = UITapGestureRecognizer(target: self, action: #selector(errorRecognizer(recognizer:)))
    ...
    lazy var nameLabel: UILabel = {
        ...
        lb.font = UIFont.mediumPingfangStatusSizeFont(fontSize: 13)
        //: lb.textColor = .gray
        lb.textColor = .gray
    ...
    public lazy var container: UIView = {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(shell(recognizer:)))
        ...
        let longPress = UILongPressGestureRecognizer(target: self, action: #selector(pushBlock(recognizer:)))
    ...
    lazy var retryView: UIImageView = {
        ...
        imagV.image = UIImage.addBar(name: (String(kOpValue.suffix(6)) + "alk_n" + String(dataGiftStr)))
        ...
        let resendTap = UITapGestureRecognizer(target: self, action: #selector(cuttingEdgeEqual(recognizer:)))
...
//: extension EqualGlobalView {
extension EqualGlobalView {
    //: @objc func onSelectMessageAvatar(recognizer: UIGestureRecognizer) {
    @objc func errorRecognizer(recognizer _: UIGestureRecognizer) {
        //: self.delegate?.onSelectMessageAvatar(cell: self)
        self.delegate?.constraintVoice(cell: self)
    }

    //: @objc func onLongPress(recognizer: UIGestureRecognizer) {
    @objc func pushBlock(recognizer: UIGestureRecognizer) {
