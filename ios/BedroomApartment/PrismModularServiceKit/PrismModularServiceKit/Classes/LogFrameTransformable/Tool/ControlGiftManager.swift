...
/*: "gifts" :*/
fileprivate let notiLengthName:String = "GIFTS"

/*: "vipGifts" :*/
fileprivate let app_barUrl:String = "let model backgroundvipGift"
fileprivate let user_addEqualMatchValue:[Character] = ["s"]

/*: "packGifts" :*/
fileprivate let userValueMsg:[Character] = ["p","a","c","k","G","i","f","t","s"]

/*: "giftSendItem" :*/
fileprivate let const_errorText:String = "guard center user translategiftSe"
...
class ControlGiftManager: NSObject {
    /// 是否展示背包红点
    ...
    private var cacheDict = [String: [MinutiaRepresentable: [Any]]]()
    // 数据锁
    ...
    private var giftType = FromWeltanschauungContentConvertible.normal

    //: static let share = TalkingChatGiftManager()
    static let share = ControlGiftManager()
...
extension ControlGiftManager {
    ...
    func behindReloadMessage(type: FromWeltanschauungContentConvertible = .normal, isRefresh: Bool = false, completion: @escaping () -> Void) {
        //: TalkingChatGiftManager.share.giftType = type
        ControlGiftManager.share.giftType = type
        //: if isRefresh == false {
        if isRefresh == false {
            //: let dataDict = cacheDict[type.rawValue]
            let dataDict = cacheDict[type.rawValue]
            //: let hotArr = dataDict?[GiftDataType.Hot] ?? [Any]()
            let hotArr = dataDict?[MinutiaRepresentable.Hot] ?? [Any]()
        ...
        if type == .groupChat {
            ...
            ControlGiftManager.share.happeningShow(completion: { isSucceed in
            ...
            ControlGiftManager.share.showViewCompletion(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            ControlGiftManager.share.cardCompletion(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            ControlGiftManager.share.collectionDown(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
    ...
    private func collectionDown(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = ControlGiftManager.share.giftType
        //: PictureRequestTool.req_ChatGiftList(type: type.rawValue) { succeed, result, errorModel in
        PictureRequestTool.feePressPull(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
                //: var dataDict = [GiftDataType: [Any]]()
                var dataDict = [MinutiaRepresentable: [Any]]()
                // hot礼物
                //: if let array: Array = data["gifts"] as? Array<Any> {
                if let array: Array = data[(notiLengthName.lowercased())] as? [Any] {
                    //: var hotArr = [Any]()
                    var hotArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<ModelHeadTransformable>.deserialize(from: array as? Array) {
                        //: hotArr.append(contentsOf: datas as [Any])
                        hotArr.append(contentsOf: datas as [Any])
                    }
                    //: dataDict[GiftDataType.Hot] = hotArr
                    dataDict[MinutiaRepresentable.Hot] = hotArr
                }

                // vip礼物
                //: if let array: Array = data["vipGifts"] as? Array<Any> {
                if let array: Array = data[(String(app_barUrl.suffix(7)) + String(user_addEqualMatchValue))] as? [Any] {
                    //: var vipArr = [Any]()
                    var vipArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<ModelHeadTransformable>.deserialize(from: array as? Array) {
                    ...
                    dataDict[MinutiaRepresentable.Vip] = vipArr
                }

                // Bags礼物
                //: if let array: Array = data["packGifts"] as? Array<Any> {
                if let array: Array = data[(String(userValueMsg))] as? [Any] {
                    //: var bagsArr = [Any]()
                    var bagsArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<ModelHeadTransformable>.deserialize(from: array as? Array) {
                    ...
                    dataDict[MinutiaRepresentable.Bags] = bagsArr
                }
                ...
                var firstModel = BeautyGiftHandyJSON()
                //: firstModel.num = 1
                firstModel.num = 1
                ...
                let sendItem: Array = data[(String(const_errorText.suffix(6)) + "ndItem")] as! [Any]
                ...
                for i in 0 ..< sendItem.count {
                    ...
                    if let configModel = JSONDeserializer<BeautyGiftHandyJSON>.deserializeFrom(dict: sendItem[i] as? [String: Any], designatedPath: nil) {
    ...
    private func happeningShow(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = ControlGiftManager.share.giftType
        //: PictureRequestTool.req_ChatRoomGiftList(type: type.rawValue) { succeed, result, errorModel in
        PictureRequestTool.listInput(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func full(params: [String: Any], completion: @escaping (_ data: [Any]?) -> Void) {
        //: PictureRequestTool.req_getChatRoomMembers(params: params, completion: { succeed, result, errorModel in
        PictureRequestTool.product(params: params, completion: { succeed, result, _ in
            //: if succeed {
            if succeed {
                ...
                for dict in data {
                    ...
                    if let memberModel = JSONDeserializer<ResistanceReactiveCompatible>.deserializeFrom(dict: dict as? [String: Any], designatedPath: nil) {
    ...
    private func showViewCompletion(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.live
        let type = FromWeltanschauungContentConvertible.live
        //: PictureRequestTool.req_LiveGiftList(type: type.rawValue) { succeed, result, errorModel in
        PictureRequestTool.minutia(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    private func cardCompletion(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.party
        let type = FromWeltanschauungContentConvertible.party
        //: PictureRequestTool.req_partyGiftList(type: type.rawValue) { succeed, result, errorModel in
        PictureRequestTool.half(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func queryFront(perPageCount: Int, dataType: MinutiaRepresentable) -> Int {
    ...
    func humanFace(dataType: MinutiaRepresentable) -> [Any] {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
    ...
    func withView() -> [Any] {
        ...
        if ControlGiftManager.share.giftType == .groupChat {
            //: return chatRoomSendItems
            return chatRoomSendItems
            //: } else if TalkingChatGiftManager.share.giftType == .live || TalkingChatGiftManager.share.giftType == .party {
        } else if ControlGiftManager.share.giftType == .live || ControlGiftManager.share.giftType == .party {
    ...
    func lagniappe(dataType: MinutiaRepresentable, dataArr: [Any]) {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
...
extension ControlGiftManager {
    ...
    @objc private func receiveEvent() {
        //: gift_clearMemoryCache(showRed: true)
        indexVoice(showRed: true)
    }
}
...
extension ControlGiftManager {
    ...
    func indexVoice(showRed: Bool = false) {
        //: TalkingChatGiftManager.share.showBagsRed = showRed
        ControlGiftManager.share.showBagsRed = showRed
        //: cacheDict.removeAll()
        cacheDict.removeAll()
    }
}
