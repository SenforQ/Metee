...
fileprivate let mainSizeKey:[Character] = ["\u{8bbe}","置","\u{666e}","通","状","\u{6001}","的","文","字","颜","\u{8272}","时"," ","请","使","用","R","G","B","空","间","的","颜","色","值"]

/*: "设置选中状态的文字颜色时 请使用RGB空间的颜色值" :*/
fileprivate let appLotData:String = "设置选\u{4e2d}"
fileprivate let kCurrentMessage:String = "色时 请\u{4f7f}"
fileprivate let constManagerUrl:String = "间的\u{989c}色值"

/*: "init(coder:) has not been implemented" :*/
fileprivate let showBagKitFormat:[UInt8] = [0xc5,0xc2,0xc5,0xd8,0x84,0xcf,0xc3,0xc8,0xc9,0xde,0x96,0x85,0x8c,0xc4,0xcd,0xdf,0x8c,0xc2,0xc3,0xd8,0x8c,0xce,0xc9,0xc9,0xc2,0x8c,0xc5,0xc1,0xdc,0xc0,0xc9,0xc1,0xc9,0xc2,0xd8,0xc9,0xc8]

/*: "设置的下标不合法!!" :*/
fileprivate let const_tagKey:String = "设\u{7f6e}的\u{4e0b}标"

/*: "Bags" :*/
fileprivate let const_cosUrl:[Character] = ["B","a","g","s"]
...
open class StandInThen: UIView {
	var imageDoing: Bool = true
	var toolCount: Int = 68
	var videoNumber: Double = 19.0
	var downwardDictionary: [AnyHashable: String] = [:]
	var picEnable: Bool = true
	var refreshTotal: Int = 42
	var viewNumber: Double = 11.7
	var progressDictionary: [AnyHashable: String] = [:]

    //: open var segmentStyle: SegmentStyle
    open var segmentStyle: ControlSegmentStyle
    /// 点击响应
    ...
    fileprivate lazy var normalColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let normalRgb = self.statusRow(self.segmentStyle.normalTitleColor) {
            //: return normalRgb
            return normalRgb
            //: } else {
        } else {
            //: fatalError("设置普通状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((String(mainSizeKey)))
        }
        //: }()
    }()
    ...
    fileprivate lazy var selectedTitleColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let selectedRgb = self.statusRow(self.segmentStyle.selectedTitleColor) {
            //: return selectedRgb
            return selectedRgb
            //: } else {
        } else {
            //: fatalError("设置选中状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((appLotData + "状态的文字\u{989c}" + kCurrentMessage + "用RGB空" + constManagerUrl))
        }
    ...
    fileprivate func statusRow(_ color: UIColor) -> (r: CGFloat, g: CGFloat, b: CGFloat)? {
        ...
        //: return nil
            ...
            if (self.textInputContextIdentifier != nil) && (self.bounds.size.height == 72.88) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let colorTip = ResLabView()
            colorTip.collectionModelSwitch = self.segmentStyle.scrollTitle
            colorTip.overObjectInterval = xGap


            
            colorTip.inputSignalEqualOff = { [self] remoteOff in
            self.picEnable = remoteOff
            
            var colorTip = self.segmentStyle.scrollTitle
                colorTip = true
                colorTip = true
            if colorTip != self.picEnable {
                self.picEnable = colorTip
            }
            
            return self.picEnable
            }
            colorTip.showQuantity = { [self] awakeInterval in
            self.refreshTotal = awakeInterval
            
            var colorTip = numOfComponents
            colorTip *= 7
            if colorTip < self.refreshTotal {
                self.refreshTotal = colorTip
            }
            
            return self.refreshTotal
            }
            colorTip.mortalSum = { [self] suiteNumber in
            self.viewNumber = suiteNumber
            
            self.viewNumber -= 1
            return self.viewNumber
            }
            colorTip.frameDictionary = { [self] nameDictionary in
            self.progressDictionary = nameDictionary
    ...
    public init(frame: CGRect, segmentStyle: ControlSegmentStyle, titles: [String]) {
    ...
    @objc func noUp(_ tapGes: UITapGestureRecognizer) {
        //: guard let currentLabel = tapGes.view as? CustomLabel else { return }
        guard let currentLabel = tapGes.view as? DesignateThen else { return }
        //: currentIndex = currentLabel.tag
        currentIndex = currentLabel.tag
        //: print(currentLabel.tag)
        //: adjustUIWhenBtnOnClickWithAnimate(true)
        deadlineAnimate(true)
    
	if let scrollLine = scrollLine {

            if (scrollLine.alpha != 1.0) && (scrollLine.layoutGuides.count == 126) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let withTopLet = ResLabView(frame: scrollLine.frame.standardized)
            withTopLet.collectionModelSwitch = segmentStyle.showCover
            withTopLet.overObjectInterval = self.currentIndex


            
            withTopLet.inputSignalEqualOff = { [self] remoteOff in
            self.imageDoing = remoteOff
            
            var withTopLet = self.segmentStyle.scrollTitle
            withTopLet = !withTopLet
            if withTopLet != self.imageDoing {
                self.imageDoing = withTopLet
            }
            
            return self.imageDoing
            }
            withTopLet.showQuantity = { [self] awakeInterval in
            self.toolCount = awakeInterval
            
            var withTopLet = currentIndex
            withTopLet &= 3
            if withTopLet > self.toolCount {
                self.toolCount = withTopLet
            }
            
            return self.toolCount
            }
            withTopLet.mortalSum = { [self] suiteNumber in
            self.videoNumber = suiteNumber
            
            self.videoNumber /= 5
            return self.videoNumber
            }
            withTopLet.frameDictionary = { [self] nameDictionary in
            self.downwardDictionary = nameDictionary
            
            guard var value = self.downwardDictionary as? [String: String] else {
                return nil
            }
            return value
            }
                scrollLine.addSubview(withTopLet)
            }
    ...
    public required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: showBagKitFormat.map{$0^172}, encoding: .utf8)!)
    }
}
...
public extension StandInThen {
    ...
    func take(_ selectedIndex: Int, animated: Bool) {
        ...
        deadlineAnimate(animated)
    }

    // 暴露给外界来刷新标题的显示
    //: public func reloadTitlesWithNewTitles(_ titles: [String]) {
    func visualCommunication(_ titles: [String]) {
        // 移除所有的scrollView子视图
...
//: extension ScrollSegmentView {
extension StandInThen {
    // 根据Titles添加相应的控件
    //: fileprivate func setupTitles() {
    fileprivate func numberimateHandleUp() {
        //: for (index, title) in titles.enumerated() {
        for (index, title) in titles.enumerated() {
            //: let label = CustomLabel(frame: CGRect.zero)
            let label = DesignateThen(frame: CGRect.zero)
            //: label.tag = index
            label.tag = index
            //: label.text = title
            label.text = title
            ...
            let tapGes = UITapGestureRecognizer(target: self, action: #selector(self.noUp(_:)))
            ...
            if title == (String(const_cosUrl)).localized {
    ...
    fileprivate func stopName() {
        ...
        if let firstLabel = labelsArray[0] as? DesignateThen {
    ...
    fileprivate func increaseCover() {
    ...
    public func pastImage(_ currentIndex: Int) {
...
open class DesignateThen: UILabel {
    /// 用来记录当前label的缩放比例
