...
fileprivate let constIndicatorContent:[UInt8] = [0xed,0xea,0xed,0xf0,0xac,0xe7,0xeb,0xe0,0xe1,0xf6,0xbe,0xad,0xa4,0xec,0xe5,0xf7,0xa4,0xea,0xeb,0xf0,0xa4,0xe6,0xe1,0xe1,0xea,0xa4,0xed,0xe9,0xf4,0xe8,0xe1,0xe9,0xe1,0xea,0xf0,0xe1,0xe0]

private func playerWrap(scale num: UInt8) -> UInt8 {
    return num ^ 132
}

/*: "icon_coinxxx" :*/
fileprivate let mainMediumStr:String = "icon_array text var let"
fileprivate let noti_labelPath:String = "withinxxx"

/*: "icon_gift_num_bg" :*/
fileprivate let notiTouchMsg:[Character] = ["i","c","o","n","_","g","i","f"]
fileprivate let user_makeTitle:String = "t_num_bgregular open show text"

/*: "x1" :*/
fileprivate let const_viewTextMsg:[Character] = ["x","1"]

/*: " x :*/
fileprivate let noti_contentUrl:[Character] = [" ","x"]
...
class TantamountView: UICollectionViewCell {
    //: var model = TalkingRoomGiftModel()
    var model = ModelHeadTransformable()
    //: let checked = false
    let checked = false
    //: let giftNum = 0
    let giftNum = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constIndicatorContent.map{playerWrap(scale: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var nameLB: UILabel = {
        ...
        lb.font = UIFont.pingfangCenter(type: .Regular, fontSize: 10)
        //: lb.textColor = .white
        lb.textColor = .white
    ...
    lazy var priceBtn: QuantityervalFirstButton = {
        //: let btn = TalkingButton.init()
        let btn = QuantityervalFirstButton()
        //: btn.setTitleColor(UIColor.white, for: .normal)
        btn.setTitleColor(UIColor.white, for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangFont(type: .Regular, fontSize: 10)
        btn.titleLabel?.font = UIFont.pingfangCenter(type: .Regular, fontSize: 10)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_coinxxx"), for: .normal)
        btn.setImage(UIImage.addBar(name: (String(mainMediumStr.prefix(5)) + noti_labelPath.replacingOccurrences(of: "with", with: "co"))), for: .normal)
    ...
    lazy var numBgView: UIImageView = {
        ...
        let imag = UIImage.addBar(name: (String(notiTouchMsg) + String(user_makeTitle.prefix(8))))
        //: let sizex = imag.size
        let sizex = imag.size
    ...
    lazy var numLb: UILabel = {
        ...
        lb.font = UIFont.pingfangCenter(type: .Regular, fontSize: 9)
        //: return lb
        return lb
        //: }()
    }()
...
extension TantamountView {
    /// 更改选中状态
    //: func setChecked(checked: Bool) {
    func describe(checked: Bool) {
        ...
        if checked {
            ...
            bgView.layer.borderColor = UIColor.outsideBecomeColor().cgColor
    ...
    func indexLargess(currenmodel: ModelHeadTransformable, giftType: MinutiaRepresentable) {
        //: model = currenmodel
        model = currenmodel
        //: giftIcon.setUrlImage(urlStr: model.imgPreview ?? "")
        giftIcon.step(urlStr: model.imgPreview ?? "")
        //: priceBtn.setTitle("\(model.price)", for: .normal)
        priceBtn.setTitle("\(model.price)", for: .normal)
        //: selectedImageView.setUrlImage(urlStr: model.cornerMark, placeImg: nil)
        selectedImageView.step(urlStr: model.cornerMark, placeImg: nil)
        ...
        let attributes = [NSAttributedString.Key.font: UIFont.pingfangCenter(type: .Regular, fontSize: 11),
...
extension TantamountView {
    ...
    func viewName() {
        ...
        bgView.backgroundColor = UIColor.alongDesign()
