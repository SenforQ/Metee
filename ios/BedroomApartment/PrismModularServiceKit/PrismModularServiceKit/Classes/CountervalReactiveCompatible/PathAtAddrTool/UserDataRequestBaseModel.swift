...
/*: "http://stat. :*/
fileprivate let data_addValue:String = "fill"
fileprivate let user_styleValue:String = "model self action stagettp:"

/*: .com" :*/
fileprivate let kPopTitle:String = "language none.com"

/*: "http://" :*/
fileprivate let show_coverLayerKey:[Character] = ["h","t","t","p",":","/","/"]

/*: "https://" :*/
fileprivate let dataUserTitle:String = "H"
fileprivate let notiMenuData:[Character] = ["t","t","p","s",":","/","/"]

/*: "http://m. :*/
fileprivate let k_equalTitle:String = "http:text view conversation perform"
fileprivate let mainAppearRequestData:[Character] = ["/","/","m","."]

/*: "http:// :*/
fileprivate let showNetworkUrl:String = "http://table image date text self"

/*: "-1" :*/
fileprivate let noti_edgeStr:String = "-1"

/*: "3,90" :*/
fileprivate let app_topCellTitle:String = "3,9view"

/*: "1990-01-01" :*/
fileprivate let k_seatValue:String = "1windowwindow0-01"
fileprivate let app_addFormat:[Character] = ["-","0","1"]

/*: "MM-dd-yyyy" :*/
fileprivate let k_fillId:String = "icon make item name giftMM-dd"
fileprivate let data_colorUrl:String = "sex info mode model-yyyy"

/*: "id" :*/
fileprivate let notiSucceedFormat:String = "iequal"
...
class AddRequestModel: NSObject {
    ...
    var requestType: FirstRequestType?
    ...
    override init() {
        ...
        self.requestServer = PathAtAddrTool.share.serverUrlStr
        //: self.showErrorStatusBar = true
        self.showErrorStatusBar = true
    }

    //: func paramsContainBinaryObject() -> Bool {
    func whetConstraint() -> Bool {
...
public struct PopErrorResponse {
    //: let errorCode: Int
    let errorCode: Int
    //: let errorMsg: String
    let errorMsg: String
...
struct StreamHandyJSON: HandyJSON {
    ...
    var CSConfig = CleanRegularHandyJSON() // 私聊官方客服提示消息模型
//    var jumpRandomVideo: Bool = false       // true表示可以跳转到随机视频匹配
    ...
    var reportDomain: String {
        ...
        if statDomain.isEmptyString {
            ...
            return (data_addValue.replacingOccurrences(of: "fill", with: "h") + String(user_styleValue.suffix(4)) + "//stat.") + "\(kErrId)" + (String(kPopTitle.suffix(4)))
            //: } else {
        } else {
            //: if !statDomain.hasPrefix("http://") && !statDomain.hasPrefix("https://") {
            if !statDomain.hasPrefix((String(show_coverLayerKey))) && !statDomain.hasPrefix((dataUserTitle.lowercased() + String(notiMenuData))) {
                //: return "http://".appending(statDomain)
                return (String(show_coverLayerKey)).appending(statDomain)
                //: }else {
            } else {
                //: return statDomain
                return statDomain
    ...
    var urlH5Domain: String {
        ...
        if urlDomain.isEmpty {
            ...
            return (String(k_equalTitle.prefix(5)) + String(mainAppearRequestData)) + "\(kErrId)" + (String(kPopTitle.suffix(4)))
            //: } else {
        } else {
            //: return "http://\(urlDomain)"
            return (String(showNetworkUrl.prefix(7))) + "\(urlDomain)"
        }
    }
...
public class KeyModelType: NSObject, HandyJSON {
    ...
    var chatPriceSettings: [CreateTransformable] = [] // 收费设置
    //: var videoPriceSettings: Array<ChatPriceModel> = []
    var videoPriceSettings: [CreateTransformable] = [] // 视频收费设置
    //: var voicePriceSettings: Array<ChatPriceModel> = []
    var voicePriceSettings: [CreateTransformable] = [] // 语音收费设置
    ...
    var vipSetting = [AddEqualTransformable]() // vip皮肤
    ...
    var tabPageBanner = [SeekSubMeasurable]() // V1.6.4 一级页面广告
    ...
    var intimateLimit = ScoreMomentModelType() // 1.7.6 各种操作的亲密度限制
...
@objcMembers public class VideoReplyReactiveCompatible: NSObject, HandyJSON {
    //: required public override init() {
    override public required init() {}
    ...
    var interest: [ViewTransformable]?
    //: var aboutMe: Array<UserSeleteTagModel>?
    var aboutMe: [ViewTransformable]?
    //: var partner: String?
    var partner: String?
    //: var full: Int?
    var full: Int?
    ...
    var gallery: [MTheoryTransformable]?
    //: var status: Int = -1
    ...
    var callPopupSetting = (app_topCellTitle.replacingOccurrences(of: "view", with: "0")) // 表示来电弹窗配置，如”3,90”表示3s后弹第一次，后续90s，返回”0,0”表示不弹窗
    //: var callTabSwitch = 0
    ...
    public lazy var age: String = {
        ...
        let birthdayDate = NSDate.pointFrom(self.birthday ?? (k_seatValue.replacingOccurrences(of: "window", with: "9") + String(app_addFormat)), dateFormat: (String(k_fillId.suffix(5)) + String(data_colorUrl.suffix(5))))
        //: guard let currentYear = NSDate.getCurrentDateComponents().year else { return "0" }
        guard let currentYear = NSDate.inputPath().year else { return "0" }
...
public struct ViewTransformable: HandyJSON, Equatable {
    //: var tag_id: String?
    var tag_id: String?
    //: var tag_name = ""
    var tag_name = ""
    ...
    func deadlineCurrent(compareTo: ViewTransformable) -> Bool {
        //: return
        return
...
struct KeyTransformable: HandyJSON {
    ...
    var aboutMe: [LikeTargetHandyJSON]!
    //: var interest: Array<UserTagTypeModel>!
    var interest: [LikeTargetHandyJSON]!
}

//: struct UserTagTypeModel: HandyJSON {
struct LikeTargetHandyJSON: HandyJSON {
    //: var name = ""
    var name = ""
    //: var name_ar = ""
    var name_ar = ""
    ...
    var list: [ViewTransformable]!
}

/// 官方客服提示消息模型
//: struct CSConfigModel: HandyJSON {
struct CleanRegularHandyJSON: HandyJSON {
...
struct AddEqualTransformable: HandyJSON {
...
struct ScoreMomentModelType: HandyJSON {
    //: var sendImg = 10
    var sendImg = 10 // 发送图片
    //: var sendVideo = 10
