...
/*: "add_header_cookies" :*/
fileprivate let noti_firstPlayerFormat:String = "add_self class"
fileprivate let userRequestResultContent:String = "_cookigreen quick play self path"
fileprivate let data_sharedUrl:[Character] = ["e","s"]
...
public extension UIImageView {
    ...
    internal func equalFinish(urlStr: String,
                              //: placeImg: UIImage = UIImage.placeImgSquare(),
                              placeImg: UIImage = UIImage.quadrateComment(),
    ...
    @objc func colorMake(urlStr: String) {
        //: let imgUrl = urlStr.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let imgUrl = urlStr.replacingOccurrences(of: showManagerMsg.frequenter6decrypt(), with: kErrId)
        //: self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.giftPlaceHolderImage())
        self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.plusAll())
    }
    ...
    @objc func pushTo(urlStr: String, completion: ((_ image: UIImage?) -> Void)? = nil) {
        // 允许请求携带cookie
        ...
        if var urlComponents = URLComponents(string: urlStr),
           ...
           let currURL = URL(string: PathAtAddrTool.share.serverUrlStr)
        {
        ...
        let downloader = ImageDownloader(name: (String(noti_firstPlayerFormat.prefix(4)) + "header" + String(userRequestResultContent.prefix(6)) + String(data_sharedUrl)))
        ...
        self.kf.setImage(with: URL(string: newImgUrl), placeholder: UIImage.quadrateComment(), options: optionsInfo) { result in
            //: if completion != nil {
            if completion != nil {
                //: let image = try? result.get().image
                let image = try? result.get().image
