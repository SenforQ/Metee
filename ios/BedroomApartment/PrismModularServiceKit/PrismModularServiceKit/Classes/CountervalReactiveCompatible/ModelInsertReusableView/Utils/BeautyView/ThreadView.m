...
NSString *StringFromHeatherMixtureData(Byte *data);


//: STDisplayShader
Byte constCenterUser_[] = {52, 15, 54, 7, 222, 180, 8, 137, 138, 122, 159, 169, 166, 162, 151, 175, 137, 158, 151, 154, 155, 168, 17};
...
- (instancetype)initWithFrame:(CGRect)frame context:(EAGLContext *)context
{
    ...
    if (self) {
        ...
        [self loadVertexShader:StringFromHeatherMixtureData(constCenterUser_)
                //: fragmentShader:@"STDisplayShader"
                fragmentShader:StringFromHeatherMixtureData(constCenterUser_)
              ...
              forProgram:(GLuint *)programPointer;
{
    ...
    *programPointer = glCreateProgram();
    ...
    if (![self compileShader:&vertexShader type:0x8B31 shaderString:dataResultNoti]) {
    ...
    if (![self compileShader:&fragShader type:0x8B30 shaderString:data_meetingApp]) {
...
Byte * HeatherMixtureDataToCache(Byte *data) {
    int chunk = data[0];
    int result = data[1];
    Byte audience = data[2];
    int galleryBegin = data[3];
    if (!chunk) return data + galleryBegin;
    for (int i = galleryBegin; i < galleryBegin + result; i++) {
    ...
    data[0] = 0;
    data[galleryBegin + result] = 0;
    return data + galleryBegin;
}

NSString *StringFromHeatherMixtureData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)HeatherMixtureDataToCache(data)];
}
