...
typedef struct {
    Byte launchBirth;
    Byte *numbererval;
    unsigned int viewVendor;
    bool to;
	int delay;
} GroupModeData;

NSString *StringFromGroupModeData(GroupModeData *data);


//: Width
GroupModeData noti_contextUser = (GroupModeData){87, (Byte []){0, 62, 51, 35, 63, 157}, 5, false, 217};


//: Height
GroupModeData userStreamBoxData = (GroupModeData){223, (Byte []){151, 186, 182, 184, 183, 171, 158}, 6, false, 53};


//: 请打开相机权限
GroupModeData data_sharedEraseConst = (GroupModeData){129, (Byte []){105, 46, 54, 103, 8, 18, 100, 61, 1, 102, 26, 57, 103, 29, 59, 103, 28, 2, 104, 24, 17, 106}, 21, false, 245};
...
@interface ViewCamera () <AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate>
...
- (void)setIExpectedFPS:(int)iExpectedFPS
{
    ...
    CGFloat fWidth = [[_dataOutput.videoSettings objectForKey:StringFromGroupModeData(&noti_contextUser)] floatValue];
    //: CGFloat fHeight = [[_dataOutput.videoSettings objectForKey:@"Height"] floatValue];
    CGFloat fHeight = [[_dataOutput.videoSettings objectForKey:StringFromGroupModeData(&userStreamBoxData)] floatValue];
...
- (CGRect)getScaleRect:(CGRect)rect scaleToFit:(BOOL)bScaleToFit
{
    ...
    if (self.dataOutput.videoSettings) {
        ...
        CGFloat fWidth = [[self.dataOutput.videoSettings objectForKey:StringFromGroupModeData(&noti_contextUser)] floatValue];
        //: CGFloat fHeight = [[self.dataOutput.videoSettings objectForKey:@"Height"] floatValue];
        CGFloat fHeight = [[self.dataOutput.videoSettings objectForKey:StringFromGroupModeData(&userStreamBoxData)] floatValue];
...
- (BOOL)judgeCameraAuthorization
{
    ...
    if (authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            UIAlertController *alertView = [UIAlertController alertControllerWithTitle:@""
                                                                               //: message:@"请打开相机权限"
                                                                               message:StringFromGroupModeData(&data_sharedEraseConst)
...
Byte *GroupModeDataToByte(GroupModeData *data) {
    if (data->to) return data->numbererval;
    for (int i = 0; i < data->viewVendor; i++) {
        data->numbererval[i] ^= data->launchBirth;
    }
    data->numbererval[data->viewVendor] = 0;
    data->to = true;
	if (data->viewVendor >= 1) {
		data->delay = data->numbererval[0];
	}
    return data->numbererval;
}

NSString *StringFromGroupModeData(GroupModeData *data) {
    return [NSString stringWithUTF8String:(char *)GroupModeDataToByte(data)];
}
