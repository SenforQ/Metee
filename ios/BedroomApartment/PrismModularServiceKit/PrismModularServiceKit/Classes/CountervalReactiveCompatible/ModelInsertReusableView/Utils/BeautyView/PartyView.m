...
//: model
Byte show_pathNoti[] = {97, 5, 81, 8, 232, 87, 121, 96, 28, 30, 19, 20, 27, 231};


//: M_SenseME_Face_Video_5.3.4
Byte showTitleConst_[] = {10, 26, 34, 13, 217, 245, 203, 53, 212, 16, 54, 41, 138, 43, 61, 49, 67, 76, 81, 67, 43, 35, 61, 36, 63, 65, 67, 61, 52, 71, 66, 67, 77, 61, 19, 12, 17, 12, 18, 189};


//: FaceActionEvent_Interval_12_frame
Byte userClickMain_[] = {39, 33, 27, 9, 19, 142, 13, 148, 203, 43, 70, 72, 74, 38, 72, 89, 78, 84, 83, 42, 91, 74, 83, 89, 68, 46, 83, 89, 74, 87, 91, 70, 81, 68, 22, 23, 68, 75, 87, 70, 82, 74, 239};


//: hasFace
Byte constPlayerShow[] = {17, 7, 40, 4, 64, 57, 75, 30, 57, 59, 61, 153};


//: FaceActionEvent_Interval_1_second
Byte mainSaveAssetNoti[] = {55, 33, 22, 13, 34, 58, 254, 94, 108, 197, 251, 97, 16, 48, 75, 77, 79, 43, 77, 94, 83, 89, 88, 47, 96, 79, 88, 94, 73, 51, 88, 94, 79, 92, 96, 75, 86, 73, 27, 73, 93, 79, 77, 89, 88, 78, 234};


//: key.Notification.Tap.To.Modify.Exposure.Point
Byte noti_targetK[] = {19, 45, 55, 11, 137, 96, 127, 222, 15, 109, 188, 52, 46, 66, 247, 23, 56, 61, 50, 47, 50, 44, 42, 61, 50, 56, 55, 247, 29, 42, 57, 247, 29, 56, 247, 22, 56, 45, 50, 47, 66, 247, 14, 65, 57, 56, 60, 62, 59, 46, 247, 25, 56, 50, 55, 61, 85};


//: camera_focus_red
Byte k_sessionShow_[] = {63, 16, 8, 11, 255, 54, 205, 187, 209, 177, 91, 91, 89, 101, 93, 106, 89, 87, 94, 103, 91, 109, 107, 87, 106, 93, 92, 158};
...
//
//#import "EffectCleanReactiveCompatible.h"
//#import "BeginLooksModel.h"
//ST_MOBILE
...
@interface PartyView ()<ExampleDelegate,UIGestureRecognizerDelegate>
{
...
//: @property (nonatomic, strong) SeekClickReactiveCompatible * lichelper;
@property (nonatomic, strong) SeekClickReactiveCompatible * lichelper;
//: @property (nonatomic, strong) SuiteVoiceReactiveCompatible * ParamUtil;
@property (nonatomic, strong) SuiteVoiceReactiveCompatible * ParamUtil;
...
- (instancetype)init {
    ...
    if (self) {
        ...
        //: self.lichelper = [SeekClickReactiveCompatible share];
        self.lichelper = [SeekClickReactiveCompatible share];
        //: self.ParamUtil = [[SuiteVoiceReactiveCompatible alloc]init];
        self.ParamUtil = [[SuiteVoiceReactiveCompatible alloc]init];
        //: _currentDropCount = 5;
        ...
        if ([self.lichelper areaColorDesign]) {
            ...
            //: [[EffectCleanReactiveCompatible share] getLocalModel];
            [[EffectCleanReactiveCompatible share] viewModel];
            ...
            [NSNotificationCenter.defaultCenter addObserver:self
                                                   ...
                                                   selector:@selector(handleTapNoti:)
                                                       ...
                                                       name:StringFromActivityData(noti_targetK)
                                                     //: object:nil];
                                                     object:nil];
...
- (instancetype)initWihtVideoConversation:(NSString*)sessionPreset {
    ...
    if (self = [super init]) {
        ...
        //: } else {
            ...
            //: [SeekClickReactiveCompatible.share checkRemoteLicInfoWithCompletion:^(BOOL x) {
            [SeekClickReactiveCompatible.share notActionWithCompletion:^(BOOL x) {
                //: if (x) {
                if (x) {
                    //: [self initFilterView:sessionPreset];
                    [self initFilterView:sessionPreset];
...
- (void)setCachFilter {
    ...
    //: if ([EffectCleanReactiveCompatible share].filterSuite.filterModelPath.length > 0) {
    if ([EffectCleanReactiveCompatible share].filterSuite.filterModelPath.length > 0) {
        //: BeginLooksModel* model = [[EffectCleanReactiveCompatible share] getFilterDataModel];
        BeginLooksModel* model = [[EffectCleanReactiveCompatible share] laughEnable];
        //: if (model) {
        if (model) {
...
- (void)setupCameraAndPreview {
    ...
    _stCamera = [[ViewCamera alloc] initWithDevicePosition:AVCaptureDevicePositionFront
...
- (void)resetPreviewWith:(CGFloat)width height:(CGFloat)height {
    ...
    if (self.glPreview.superview == nil) {
        ...
        @synchronized (self) {
           ...
           _glPreview = [[ThreadView alloc] initWithFrame:previewRect context:previewContext];
        }
...
- (void)setupHandle {
    ...
    //初始化检测模块句柄
    //: NSString *strModelPath = [ColorDelimitateMacroDefine.getModelFileName stringByAppendingPathComponent:@"M_SenseME_Face_Video_5.3.4"];
    NSString *strModelPath = [ColorDelimitateMacroDefine.blockResume stringByAppendingPathComponent:StringFromActivityData(showTitleConst_)];
    //: strModelPath = [strModelPath stringByAppendingPathExtension:@"model"];
    strModelPath = [strModelPath stringByAppendingPathExtension:StringFromActivityData(show_pathNoti)];
...
- (UIImageView *)focusImageView {
    ...
    if (!_focusImageView) {
        ...
        _focusImageView.image = [UIImage imageNamed:StringFromActivityData(k_sessionShow_)];
...
- (void)receiveFilterNotice:(NSNotification*) notice {
    //: if ([notice.object isKindOfClass:[BeginLooksModel class]]) {
    if ([notice.object isKindOfClass:[BeginLooksModel class]]) {
        //: BeginLooksModel* model = (BeginLooksModel*)notice.object;
        BeginLooksModel* model = (BeginLooksModel*)notice.object;
        //: [self handleFilterChanged:model];
        [self handleFilterChanged:model];
    }
...
//: - (void)handleFilterChanged:(BeginLooksModel *)model {
- (void)handleFilterChanged:(BeginLooksModel *)model {
...
- (void)setupBeautyParam {
    ...
    //: GlobalEntiretycostModel *beautyValueCache = EffectCleanReactiveCompatible.share.currentBeautySuite;
    GlobalEntiretycostModel *beautyValueCache = EffectCleanReactiveCompatible.share.textStatus;

    // 设置美白参数
    //: [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_WHITEN_STRENGTH value:beautyValueCache.fWhitenStrength];
    [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_WHITEN_STRENGTH value:beautyValueCache.fWhitenStrength];
    // 设置默认红润参数
    //: [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_REDDEN_STRENGTH value:beautyValueCache.fReddenStrength];
    [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_REDDEN_STRENGTH value:beautyValueCache.fReddenStrength];
    // 设置默认磨皮参数
    //: [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SMOOTH_STRENGTH value:beautyValueCache.fSmoothStrength];
    [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SMOOTH_STRENGTH value:beautyValueCache.fSmoothStrength];
    // 设置默认大眼参数
    //: [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ENLARGE_EYE_RATIO value:beautyValueCache.fEnlargeEyeStrength];
    [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ENLARGE_EYE_RATIO value:beautyValueCache.fEnlargeEyeStrength];
    // 设置默认瘦脸参数
    //: [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_FACE_RATIO value:beautyValueCache.fShrinkFaceStrength];
    [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_FACE_RATIO value:beautyValueCache.fShrinkFaceStrength];
    // 设置小脸参数
    //: [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_JAW_RATIO value:beautyValueCache.fShrinkJawStrength];
    [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_JAW_RATIO value:beautyValueCache.fShrinkJawStrength];

    //瘦脸型
    //: [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_THIN_FACE_SHAPE_RATIO value:beautyValueCache.fThinFaceShapeStrength];
    [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_THIN_FACE_SHAPE_RATIO value:beautyValueCache.fThinFaceShapeStrength];
    //窄脸
    //: [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_NARROW_FACE_STRENGTH value:beautyValueCache.fNarrowFaceStrength];
    [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_NARROW_FACE_STRENGTH value:beautyValueCache.fNarrowFaceStrength];
    //圆眼
    //: [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ROUND_EYE_RATIO value:beautyValueCache.fRoundEyeStrength];
    [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ROUND_EYE_RATIO value:beautyValueCache.fRoundEyeStrength];
    //下巴
    //: [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_CHIN_LENGTH_RATIO value:beautyValueCache.fChinStrength];
    [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_CHIN_LENGTH_RATIO value:beautyValueCache.fChinStrength];
    //额头
    //: [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_HAIRLINE_HEIGHT_RATIO value:beautyValueCache.fHairLineStrength];
    [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_HAIRLINE_HEIGHT_RATIO value:beautyValueCache.fHairLineStrength];
    //瘦鼻翼
    //: [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NARROW_NOSE_RATIO value:beautyValueCache.fNarrowNoseStrength];
    [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NARROW_NOSE_RATIO value:beautyValueCache.fNarrowNoseStrength];
    //长鼻
    //: [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NOSE_LENGTH_RATIO value:beautyValueCache.fLongNoseStrength];
    [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NOSE_LENGTH_RATIO value:beautyValueCache.fLongNoseStrength];
    //嘴形
    //: [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_MOUTH_SIZE_RATIO value:[self rangeMappedValueFrom:beautyValueCache.fMouthStrength]];
    [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_MOUTH_SIZE_RATIO value:[self rangeMappedValueFrom:beautyValueCache.fMouthStrength]];
    //缩人中
    //: [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PHILTRUM_LENGTH_RATIO value:[self rangeMappedValueFrom:beautyValueCache.fPhiltrumStrength]];
    [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PHILTRUM_LENGTH_RATIO value:[self rangeMappedValueFrom:beautyValueCache.fPhiltrumStrength]];

    //设置对比度参数
    //: [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_CONTRAST_STRENGTH value:[self rangeMappedValueFrom:beautyValueCache.fContrastStrength]];
    [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_CONTRAST_STRENGTH value:[self rangeMappedValueFrom:beautyValueCache.fContrastStrength]];

    //设置饱和度参数
    //: [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SATURATION_STRENGTH value:[self rangeMappedValueFrom:beautyValueCache.fSaturationStrength]];
    [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SATURATION_STRENGTH value:[self rangeMappedValueFrom:beautyValueCache.fSaturationStrength]];

    //锐化参数
    //: [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHARPEN_STRENGTH value:[self rangeMappedValueFrom:beautyValueCache.fSharpenStrength]];
    [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHARPEN_STRENGTH value:[self rangeMappedValueFrom:beautyValueCache.fSharpenStrength]];

    //: [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_APPLE_MUSLE_RATIO value:[self rangeMappedValueFrom:beautyValueCache.fAppleMusleStrength]];
    [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_APPLE_MUSLE_RATIO value:[self rangeMappedValueFrom:beautyValueCache.fAppleMusleStrength]];

    //: [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PROFILE_RHINOPLASTY_RATIO value:beautyValueCache.fProfileRhinoplastyStrength];
    [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PROFILE_RHINOPLASTY_RATIO value:beautyValueCache.fProfileRhinoplastyStrength];

    //: [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_DISTANCE_RATIO value:beautyValueCache.fEyeDistanceStrength];
    [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_DISTANCE_RATIO value:beautyValueCache.fEyeDistanceStrength];

    //: [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_ANGLE_RATIO value:beautyValueCache.fEyeAngleStrength];
    [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_ANGLE_RATIO value:beautyValueCache.fEyeAngleStrength];

    //: [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_OPEN_CANTHUS_RATIO value:beautyValueCache.fOpenCanthusStrength];
    [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_OPEN_CANTHUS_RATIO value:beautyValueCache.fOpenCanthusStrength];

    //: [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_BRIGHT_EYE_RATIO value:beautyValueCache.fBrightEyeStrength];
    [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_BRIGHT_EYE_RATIO value:beautyValueCache.fBrightEyeStrength];
    //: [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_DARK_CIRCLES_RATIO value:beautyValueCache.fRemoveDarkCirclesStrength];
    [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_DARK_CIRCLES_RATIO value:beautyValueCache.fRemoveDarkCirclesStrength];
    //: [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_NASOLABIAL_FOLDS_RATIO value:beautyValueCache.fRemoveNasolabialFoldsStrength];
    [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_NASOLABIAL_FOLDS_RATIO value:beautyValueCache.fRemoveNasolabialFoldsStrength];
    //: [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_WHITE_TEETH_RATIO value:beautyValueCache.fWhiteTeethStrength];
    [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_WHITE_TEETH_RATIO value:beautyValueCache.fWhiteTeethStrength];
    //: [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_CHEEKBONE_RATIO value:beautyValueCache.fShrinkCheekboneStrength];
    [self.ParamUtil careWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_CHEEKBONE_RATIO value:beautyValueCache.fShrinkCheekboneStrength];
...
void infoPoint(st_mobile_animal_face_t *src, int faceCount, st_mobile_animal_face_t *dst) {
...
void formatAngle(st_mobile_animal_face_t *src, int faceCount) {
    //: if (faceCount > 0) {
    if (faceCount > 0) {
        //: for (int i = 0; i < faceCount; ++i) {
        for (int i = 0; i < faceCount; ++i) {
...
- (void)callbackDelegateWithSampleBuffer:(CMSampleBufferRef)originalSampleBuffer
    ...
    if ( originalSampleBuffer && resultPixelBufffer ) {
        ...
        id<ErrorService> tmpDelegate = self.delegate;
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    if (_hDetector) {
        ...
        if (self.iCurrentAction > 0) {
            ...
            if (_faceFrameCount == 12) { // 第12帧时
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:StringFromActivityData(userClickMain_) object:nil userInfo: @{StringFromActivityData(constPlayerShow): @(hasFace)}];
            }
            ...
            if (_faceFrameCount%18 == 0) { // 每秒发送一次
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:StringFromActivityData(mainSaveAssetNoti) object:nil userInfo: @{StringFromActivityData(constPlayerShow): @(hasFace)}];
            }
        }
    }
    ...
    if (faceCount > 0) {
        ...
        infoPoint(_detectResult1, faceCount, newDetectResult1);
    }
    ...
    dispatch_async(self.renderQueue, ^{
       ...
       // ST_MOBILE 以下为滤镜部分
        ...
        if (faceCount > 0) {
            ...
            formatAngle(newDetectResult1, faceCount);
        }
        //: if (callBackFlag) {
        if (callBackFlag) {
            //: @synchronized (self) {
...
Byte * ActivityDataToCache(Byte *data) {
    int nameCanvas = data[0];
    int portraitDelay = data[1];
    Byte mobile = data[2];
    int flowerStore = data[3];
    if (!nameCanvas) return data + flowerStore;
    for (int i = flowerStore; i < flowerStore + portraitDelay; i++) {
    ...
    data[0] = 0;
    data[flowerStore + portraitDelay] = 0;
    return data + flowerStore;
}
