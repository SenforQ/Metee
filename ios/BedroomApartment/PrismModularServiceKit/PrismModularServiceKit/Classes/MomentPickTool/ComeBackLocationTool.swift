...
/*: "allowList" :*/
fileprivate let notiToKey:[Character] = ["a","l","l","o","w"]
fileprivate let main_numberColorUrl:[Character] = ["L","i","s","t"]

/*: "denyList" :*/
fileprivate let dataTrackStr:[Character] = ["d","e","n","y","L","i","s","t"]

/*: "America" :*/
fileprivate let user_appIndexUrl:String = "as forAmerica"

/*: "US" :*/
fileprivate let kActionRequestMakeUrl:[Character] = ["U","S"]
...
class ComeBackLocationTool: NSObject {
    //: var areaArr = [TalkingAreaModel]()
    var areaArr = [NoManSLandAreaModel]() /// 注册允许国家列表（白名单）
    //: var blackListAreaArr = [TalkingAreaModel]()
    var blackListAreaArr = [NoManSLandAreaModel]() /// 注册不允许国家列表（黑名单）

    //: static let share = TalkingLocationTool()
    static let share = ComeBackLocationTool()
...
extension ComeBackLocationTool {
    ...
    private func behindMake() {
        ...
        SuiteReactiveCompatible.index { succeed, result, _ in
            ...
            if succeed {
                ...
                let allowArray: Array = dict[(String(notiToKey) + String(main_numberColorUrl))] as! [Any]
                //: let denyListArray: Array =  dict["denyList"] as! Array<Any>
                let denyListArray: Array = dict[(String(dataTrackStr))] as! [Any]

                //: if let datas = Array<TalkingAreaModel>.deserialize(from: allowArray as? Array) {
                if let datas = Array<NoManSLandAreaModel>.deserialize(from: allowArray as? Array) {
                    //: self.areaArr.append(contentsOf: (datas as? [TalkingAreaModel])!)
                    self.areaArr.append(contentsOf: (datas as? [NoManSLandAreaModel])!)
                }
                //: if let blackdatas = Array<TalkingAreaModel>.deserialize(from: denyListArray as? Array) {
                if let blackdatas = Array<NoManSLandAreaModel>.deserialize(from: denyListArray as? Array) {
                    //: self.blackListAreaArr.append(contentsOf: (blackdatas as? [TalkingAreaModel])!)
                    self.blackListAreaArr.append(contentsOf: (blackdatas as? [NoManSLandAreaModel])!)
                }
            }
        }
    }
    ...
    static func dark() -> Bool {
        ...
        let simInfo = UIDevice.digitizerInfo()
        //: let arr = TalkingLocationTool.share.blackListAreaArr
        let arr = ComeBackLocationTool.share.blackListAreaArr
        //: for model in arr {
        for model in arr {
    ...
    func thoroughCountry() -> NoManSLandAreaModel {
        //: let simInfo = UIDevice.getDeviceSimInfo()
        let simInfo = UIDevice.digitizerInfo()
        //: let arr = TalkingLocationTool.share.areaArr
        let arr = ComeBackLocationTool.share.areaArr
        //: for model in arr {
        for model in arr {
            //: if simInfo.contains(model.countryCode.uppercased()) || simInfo.contains(model.countryCode.lowercased()) {
            if simInfo.contains(model.countryCode.uppercased()) || simInfo.contains(model.countryCode.lowercased()) {
        ...
        return NoManSLandAreaModel(areaCode: "1", areaName: (String(user_appIndexUrl.suffix(7))), countryCode: "US", url: "")
    }
}

//: class TalkingAreaModel: HandyJSON {
class NoManSLandAreaModel: HandyJSON {
    //: var areaCode = ""
    var areaCode = ""
    //: var countryCode = ""
    var countryCode = ""
