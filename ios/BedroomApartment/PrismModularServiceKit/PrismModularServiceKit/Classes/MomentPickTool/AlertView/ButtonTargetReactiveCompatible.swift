...
fileprivate let kMomentValue:String = "btn_shvar frame process"
fileprivate let dataCenterId:String = "extra name cell model ofrtBac"

/*: "btn_alertBackground_false" :*/
fileprivate let app_giftUserFormat:String = "view withbtn_"
fileprivate let appTabUrl:String = "atopgro"
fileprivate let const_measurementKey:String = "FALSE"
...
// __CLOSE_PRINT__
//
//  ButtonTargetReactiveCompatible.swift
//  AbroadTalking
//
...
public enum ConcatenateMaskType {
    //: case color
    case color
    //: case effect
    case effect
...
public class ButtonTargetReactiveCompatible: NSObject {
    /// 通用回调
...
public extension ButtonTargetReactiveCompatible {
    /// 适配器回调,用于给适配器参数赋值
    //: public typealias ConfigAlert = ((_ config: ShowAlertConfig) -> Void)
    typealias ConfigAlert = (_ config: PlayColorAlertConfig) -> Void
    ...
    internal class func doState(title: String? = nil,
                                //: message: String?  = nil,
                                message: String? = nil,
                                ...
                                config: PlayColorAlertConfig? = nil)
    {
        //: customAlert(title: title,
        equalAlert(title: title,
                   //: message: message,
                   message: message,
    ...
    @objc internal class func noSave(title: String? = nil,
    ...
    class func equalAlert(title: String? = nil,
                          ...
                          config: PlayColorAlertConfig? = nil)
    {
        ...
        //: ColorDelimitateMacroDefine.getWindow().addSubview(alertView)
        ColorDelimitateMacroDefine.beforeText().addSubview(alertView)
    ...
    class func obnubilateBlockAlert() {
        //: ColorDelimitateMacroDefine.getWindow().subviews.forEach { (view) in
        ColorDelimitateMacroDefine.beforeText().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: ConnectBeautyReactiveCompatible.self) {
    ...
    class func cookieChange(title: String, isTouch: Bool) {
        //: ColorDelimitateMacroDefine.getWindow().subviews.forEach { (view) in
        ColorDelimitateMacroDefine.beforeText().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: ConnectBeautyReactiveCompatible.self) {
                //: let cunrenView: TalkingAlertView = view as! TalkingAlertView
                let cunrenView: ConnectBeautyReactiveCompatible = view as! ConnectBeautyReactiveCompatible
                //: cunrenView.changeRightBtn(title: title, isTouch: isTouch)
                cunrenView.nowTouch(title: title, isTouch: isTouch)
            }
        }
    }
}
...
public class PlayColorAlertConfig {
    /// 执行动画时间
    ...
    public var maskType: ConcatenateMaskType = .color
    /// 阴影
    ...
    public var titleFont: UIFont = .pingfangCenter(type: .Medium, fontSize: 18)
    /// alert标题字体颜色
    //: public var titleColor  = UIColor.appTitleColor()
    public var titleColor = UIColor.census()
    /// alert信息字体
    //: public var textFont: UIFont = .pingfangFont(type: .Medium, fontSize: 16)
    public var textFont: UIFont = .pingfangCenter(type: .Medium, fontSize: 16)
    //: public var textColor  = UIColor.appTitleColor()
    public var textColor = UIColor.census()
    ...
    public var buttonFont: UIFont = .pingfangCenter(type: .Medium, fontSize: 16)
    /// alert按钮字体颜色
    //: public var leftColor  = UIColor.appThemeColor()
    public var leftColor = UIColor.outsideBecomeColor()
    //: public var rightColor = UIColor.white
    public var rightColor = UIColor.white
    //: public var rightBackgroundImage = "btn_show_alertBackground"
    public var rightBackgroundImage = (String(kMomentValue.prefix(6)) + "ow_ale" + String(dataCenterId.suffix(5)) + "kground")

    //: public var rightBackgroundImagefalse = "btn_alertBackground_false"
    public var rightBackgroundImagefalse = (String(app_giftUserFormat.suffix(4)) + "alertB" + appTabUrl.replacingOccurrences(of: "top", with: "ck") + "und_" + const_measurementKey.lowercased())
    //: public var rightFont: UIFont = .pingfangFont(type: .Medium, fontSize: 16)
    public var rightFont: UIFont = .pingfangCenter(type: .Medium, fontSize: 16)
