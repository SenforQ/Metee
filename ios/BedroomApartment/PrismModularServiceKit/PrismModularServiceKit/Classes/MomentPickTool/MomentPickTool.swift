...
/*: "Finish" :*/
fileprivate let show_viewUrl:String = "if gift gift right dailyFinish"

/*: "Cancel" :*/
fileprivate let show_modelIndexStr:[Character] = ["C"]
fileprivate let showStatusName:[Character] = ["a","n","c","e","l"]

/*: "btn_photo_choice_pre" :*/
fileprivate let userTitleData:String = "btn_phcell value input"
fileprivate let noti_contactBlockTitleKey:String = "share"
fileprivate let noti_giftData:String = "interval itemhoice_pre"

/*: "btn_photo_add_nor" :*/
fileprivate let kOfMessage:[Character] = ["b","t","n","_","p","h","o","t","o"]
fileprivate let kTabStatusStr:[Character] = ["_","a"]
fileprivate let k_requestTitle:String = "self colordd_nor"

/*: "( :*/
fileprivate let kUserLabStr:[Character] = ["("]

/*: )" :*/
fileprivate let mainAttributeMessage:String = "nose"

/*: "/tmp" :*/
fileprivate let k_measurementAddIfMessage:[Character] = ["/","t","m","p"]

/*: "Export session failed" :*/
fileprivate let notiSaveFormat:String = "Export setime record finish add"
fileprivate let kKitValue:String = "imageio"

/*: "Export canceled" :*/
fileprivate let main_toMsg:String = "frame icon modelExport"
fileprivate let userTopFlowKey:[Character] = [" ","c","a","n","c","e","l","e","d"]

/*: "Successful!" :*/
fileprivate let notiViewName:String = "Succemodel dismiss interval"

/*: "Unsupported video formats" :*/
fileprivate let kColorSectionMessage:String = "view model backgroundUnsup"
fileprivate let user_resourceText:String = "view text text datavideo fo"
fileprivate let dataDailyStr:[Character] = ["r","m","a","t","s"]

/*: "yyyy-MM-dd-HH:mm:ss-SSS" :*/
fileprivate let dataGiftMessage:String = "yyyy-collection name leading"
fileprivate let kMomentName:String = "d-HH:video title input"

/*: "/tmp/video- :*/
fileprivate let show_nameValue:String = "title lab make/tmp/"

/*: - :*/
fileprivate let app_styleTitle:[Character] = ["-"]

/*: .mp4" :*/
fileprivate let noti_sizeTitle:String = "button let path case.mp4"
...
class MomentPickTool: NSObject {
    ...
    class func infoVideo(allowImgCrop: Bool = false, needCircleCrop: Bool = false, maxCount: Int, allowPhoto: Bool, allowVideo: Bool) -> TZImagePickerController {
...
//        imagePicker?.preferredLanguage = "en"
        ...
        if allowImgCrop {
            ...
            var height = notiEnabledRecordUrl * 488 / 375
            //: if needCircleCrop == true {
            if needCircleCrop == true { // 切圆图
                //: height = ScreenWidth
                height = notiEnabledRecordUrl
            }
            //: imagePicker?.cropRect = CGRect(x: 0, y: (ScreenHeight-height)/2, width: ScreenWidth, height: height)
            imagePicker?.cropRect = CGRect(x: 0, y: (main_versionEnterTimeStr - height) / 2, width: notiEnabledRecordUrl, height: height)
        }
        //: if allowVideo {
        if allowVideo {
        ...
        imagePicker?.doneBtnTitleStr = (String(show_viewUrl.suffix(6))).localized
        //: imagePicker?.cancelBtnTitleStr = "Cancel".localized
        imagePicker?.cancelBtnTitleStr = (String(show_modelIndexStr) + String(showStatusName)).localized

        //: imagePicker?.barItemTextFont = UIFont.pingfangFont(type: .Regular, fontSize: 15)
        imagePicker?.barItemTextFont = UIFont.pingfangCenter(type: .Regular, fontSize: 15)
        //: imagePicker?.barItemTextColor = UIColor.appValueColor()
        imagePicker?.barItemTextColor = UIColor.oversee()
        ...
        imagePicker?.naviTitleFont = UIFont.pingfangCenter(type: .Medium, fontSize: 17)
        //: imagePicker?.naviTitleColor = UIColor.appTitleColor()
        imagePicker?.naviTitleColor = UIColor.census()
        ...
        imagePicker?.iconThemeColor = UIColor.outsideBecomeColor()

        //: imagePicker?.allowPickingOriginalPhoto = false
        imagePicker?.allowPickingOriginalPhoto = false
        //: imagePicker?.photoSelImage = UIImage.BundleImageNamed(name: "btn_photo_choice_pre")
        imagePicker?.photoSelImage = UIImage.addBar(name: (String(userTitleData.prefix(6)) + "oto_" + noti_contactBlockTitleKey.replacingOccurrences(of: "share", with: "c") + String(noti_giftData.suffix(9))))
        //: imagePicker?.takePictureImage = UIImage.BundleImageNamed(name: "btn_photo_add_nor")
        imagePicker?.takePictureImage = UIImage.addBar(name: (String(kOfMessage) + String(kTabStatusStr) + String(k_requestTitle.suffix(6))))
        ...
        imagePicker?.photoPickerPageDidRefreshStateBlock = { (_: UICollectionView?,
                ...
                doneButton?.titleLabel?.font = UIFont.pingfangCenter(type: .Medium, fontSize: 14)
                ...
                if text.count > 0 && Int(text)! > 0 {
                    ...
                    doneButton?.setTitle((String(show_viewUrl.suffix(6))).localized + "(\(text))", for: .normal)
                    //: } else {
                } else {
                    //: doneButton?.setTitle("Finish".localized, for: .normal)
                    doneButton?.setTitle((String(show_viewUrl.suffix(6))).localized, for: .normal)
                }
                //: return
        }
        ...
        imagePicker?.photoPickerPageDidLayoutSubviewsBlock = { (_: UICollectionView?,
                ...
                doneButton?.backgroundColor = UIColor.outsideBecomeColor()
                //: let text = numberLabel?.text ?? "0"
                let text = numberLabel?.text ?? "0"
                //: if text.count > 0 && Int(text)! > 0 {
                if text.count > 0 && Int(text)! > 0 {
        ...
        imagePicker?.photoPreviewPageDidLayoutSubviewsBlock = { (_: UICollectionView?,
                ...
                selectButton?.frame = CGRect(x: notiEnabledRecordUrl - 5 - (selectButton?.frame.size.width)!, y: (selectButton?.frame.origin.y)!, width: (selectButton?.frame.size.width)!, height: (selectButton?.frame.size.height)!)
                //: let text = numberLabel?.text ?? "0"
                let text = numberLabel?.text ?? "0"
                //: if text.count > 0 && Int(text)! > 0 {
                if text.count > 0 && Int(text)! > 0 {
    ...
    class func tvCompletion(asset: PHAsset?, completion: @escaping (_ filePath: URL?) -> Void) {
        ...
        PHImageManager.default().requestAVAsset(forVideo: asset!, options: options) { (asset: AVAsset?, _: AVAudioMix?, _) in
            ...
            if asset!.isKind(of: AVURLAsset.self) {
                ...
                if !FileManager.default.fileExists(atPath: NSHomeDirectory().appending((String(k_measurementAddIfMessage)))) {
                    //: do {
                    do {
                        //: try FileManager.default.createDirectory(atPath: NSHomeDirectory().appending("/tmp"), withIntermediateDirectories: true)
                        try FileManager.default.createDirectory(atPath: NSHomeDirectory().appending((String(k_measurementAddIfMessage))), withIntermediateDirectories: true)
                        //: } catch {
                    } catch {}
                }
                ...
                exportSession?.exportAsynchronously(completionHandler: {
                    ...
                    case .failed:
                        ...
                        printLog(message: (String(notiSaveFormat.prefix(9)) + kKitValue.replacingOccurrences(of: "image", with: "ss") + "n failed"))
                    //: case .cancelled:
                    case .cancelled:
                        //: printLog(message: "Export canceled")
                        printLog(message: (String(main_toMsg.suffix(6)) + String(userTopFlowKey)))
                    //: case .completed:
                    case .completed:
                        //: printLog(message: "Successful!")
                        printLog(message: (String(notiViewName.prefix(5)) + "ssful!"))
                        //: DispatchQueue.main.async {
                        DispatchQueue.main.async {
                ...
                printLog(message: (String(kColorSectionMessage.suffix(5)) + "ported " + String(user_resourceText.suffix(8)) + String(dataDailyStr)))
            }
        }
    }
    ...
    private class func afterBindTitle() -> String {
        //: let formater = DateFormatter()
        let formater = DateFormatter()
        //: formater.dateFormat = "yyyy-MM-dd-HH:mm:ss-SSS"
        formater.dateFormat = (String(dataGiftMessage.prefix(5)) + "MM-d" + String(kMomentName.prefix(5)) + "mm:ss-SSS")
        //: let outputPath = NSHomeDirectory().appending("/tmp/video-\(formater.string(from: Date()))-\(Int.random(in: 0...10000000)).mp4")
        let outputPath = NSHomeDirectory().appending((String(show_nameValue.suffix(5)) + "video-") + "\(formater.string(from: Date()))-\(Int.random(in: 0 ... 10_000_000))" + (String(noti_sizeTitle.suffix(4))))
        //: return outputPath
        return outputPath
    }
}