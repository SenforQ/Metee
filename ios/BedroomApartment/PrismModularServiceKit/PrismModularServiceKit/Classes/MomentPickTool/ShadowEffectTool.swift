...
fileprivate let appGameTitle:String = "path scale stairs sharedAppRe"
fileprivate let kToTitle:String = "esVeinput error"
fileprivate let userInfoId:[Character] = ["r","s","i","o","n","K","e","y"]

/*: ".zip" :*/
fileprivate let kClickKeyStr:String = "kit label data beauty.zip"

/*: "pic" :*/
fileprivate let user_whiteName:[Character] = ["p","i","c"]

/*: "nor" :*/
fileprivate let constInputName:String = "ndatar"

/*: "svga" :*/
fileprivate let kEqualVideoUrl:[Character] = ["s","v","g","a"]

/*: "mp3" :*/
fileprivate let dataRegularModeSearchPath:String = "minfo"

/*: "model" :*/
fileprivate let constAppearData:String = "modearray"

/*: "language" :*/
fileprivate let noti_viewStr:String = "equal"
fileprivate let dataMakeId:String = "anbuttonage"

/*: "@2x.png" :*/
fileprivate let kAddPath:String = "action name row label@2x.png"

/*: "png" :*/
fileprivate let const_topTableName:[Character] = ["p","n","g"]
...
private let main_noUrl = (String(appGameTitle.suffix(5)) + "sourc" + String(kToTitle.prefix(4)) + String(userInfoId))

//: enum SVGAType: String {
enum NumbereractionSendable: String {
...
class ShadowEffectTool: NSObject {
    //: static let `default` = SVGAEffectTool()
    static let `default` = ShadowEffectTool()
    ...
    private lazy var AppResPath: String = {
        ...
        let res_Dir = (docuPath as NSString).appendingPathComponent(show_barText)
        //: let version = Defaults.string(forKey: AppResourcesVersionKey) ?? ""
        let version = kBarUrl.string(forKey: main_noUrl) ?? ""
        //: if version == AppVersion, FileManager.default.fileExists(atPath: res_Dir) {
        if version == app_domainId, FileManager.default.fileExists(atPath: res_Dir) {
            //: return res_Dir
            return res_Dir
        }
        //: let path = PodspecBundle.bundle.path(forResource: PodspecName, ofType: ".zip") ?? ""
        let path = ToPodspecBundle.bundle.path(forResource: show_barText, ofType: (String(kClickKeyStr.suffix(4)))) ?? ""
        ...
        let zipSuc = SSZipArchive.unzipFile(atPath: path,
                                            ...
                                            password: show_barText,
                                            //: progressHandler: nil)
                                            progressHandler: nil)
        //: if zipSuc {
        if zipSuc {
            //: Defaults.set(AppVersion, forKey: AppResourcesVersionKey)
            kBarUrl.set(app_domainId, forKey: main_noUrl)
            //: return res_Dir
            return res_Dir
        }
        //: return ""
    ...
    private lazy var imageResPath: String = //: return (AppResPath as NSString).appendingPathComponent("pic")
        (AppResPath as NSString).appendingPathComponent((String(user_whiteName)))
    //: }()

    //: private lazy var languageImageResPath: String = {
    private lazy var languageImageResPath: String = {
        //: let language = LanguageManager.shared.currLanguage
        let language = AddBeautyThen.shared.currLanguage
    ...
    private lazy var norImageResPath: String = {
        ...
        return (imageResPath as NSString).appendingPathComponent((constInputName.replacingOccurrences(of: "data", with: "o")))
        //: }()
    }()
    ...
    private lazy var svgaResPath: String = //: return (AppResPath as NSString).appendingPathComponent("svga")
        (AppResPath as NSString).appendingPathComponent((String(kEqualVideoUrl)))
    //: }()
    ...
    private lazy var mp3ResPath: String = //: return (AppResPath as NSString).appendingPathComponent("mp3")
        (AppResPath as NSString).appendingPathComponent((dataRegularModeSearchPath.replacingOccurrences(of: "info", with: "p3")))
    //: }()
    ...
    public lazy var modelResPath: String = //: return (AppResPath as NSString).appendingPathComponent("model")
        (AppResPath as NSString).appendingPathComponent((constAppearData.replacingOccurrences(of: "array", with: "l")))
    //: }()
    ...
    public lazy var languagePath: String = //: return (AppResPath as NSString).appendingPathComponent("language")
        (AppResPath as NSString).appendingPathComponent((noti_viewStr.replacingOccurrences(of: "equal", with: "l") + dataMakeId.replacingOccurrences(of: "button", with: "gu")))
    //: }()
}

//: extension SVGAEffectTool {
extension ShadowEffectTool {
    /// 直接获取图片
    //: func getImage(name: String) -> UIImage? {
    func dataUser(name: String) -> UIImage? {
        //: let languagePath = (languageImageResPath as NSString).appendingPathComponent(name).appending("@2x.png")
        let languagePath = (languageImageResPath as NSString).appendingPathComponent(name).appending((String(kAddPath.suffix(7))))
        ...
        let path = (norImageResPath as NSString).appendingPathComponent(name).appending((String(kAddPath.suffix(7))))
    ...
    func toType(name: String, type: String = (dataRegularModeSearchPath.replacingOccurrences(of: "info", with: "p3"))) -> String {
