...
fileprivate let k_roundName:[UInt8] = [0xcc,0xcb,0xcc,0xd1,0x8d,0xc6,0xca,0xc1,0xc0,0xd7,0x9f,0x8c,0x85,0xcd,0xc4,0xd6,0x85,0xcb,0xca,0xd1,0x85,0xc7,0xc0,0xc0,0xcb,0x85,0xcc,0xc8,0xd5,0xc9,0xc0,0xc8,0xc0,0xcb,0xd1,0xc0,0xc1]

private func userMake(value num: UInt8) -> UInt8 {
    return num ^ 165
}

/*: "FFD855" :*/
fileprivate let data_keyName:[Character] = ["F","F","D","8","5","5"]
...
class DescriptionCropOverlay: UIView {
    ...
    var defaultWidth: CGFloat = 0.0 {
        ...
        didSet {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.2) {
                ...
                let minX = self.leftSilder.frame.origin.x + self.defaultWidth - noti_tableFormat
                //: self.rightSlider.frame = CGRect(x: minX,
                self.rightSlider.frame = CGRect(x: minX,
                                                //: y: self.rightSlider.frame.origin.y,
                                                y: self.rightSlider.frame.origin.y,
                                                //: width: kSliderWidth,
                                                width: noti_tableFormat,
                ...
                self.wait(startX: self.videoStartX)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_roundName.map{userMake(value: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var indicatorView: UIView = {
        ...
        indicator.backgroundColor = UIColor(hex: (String(data_keyName)))
        //    indicator.alpha = 0.5
    ...
    private lazy var leftSilder: UIView = {
        ...
        leftSilder.frame = CGRect(x: show_styleTitle, y: 0, width: noti_tableFormat, height: self.frame.size.height)
    ...
    private lazy var rightSlider: UIView = {
        ...
        rightSlider.frame = CGRect(x: self.frame.size.width - noti_tableFormat, y: 0, width: noti_tableFormat, height: self.frame.size.height)
    ...
    private lazy var leftCoverView: UIView = {
        ...
        leftCoverView.backgroundColor = UIColor.appearToRgba(0, 0, 0, 0.6)
        //: leftCoverView.frame = CGRect(x: 0, y: 0, width: LeftMargin, height: self.frame.size.height)
        leftCoverView.frame = CGRect(x: 0, y: 0, width: show_styleTitle, height: self.frame.size.height)
    ...
    private lazy var rightCoverView: UIView = {
        ...
        rightCoverView.backgroundColor = UIColor.appearToRgba(0, 0, 0, 0.6)
...
extension DescriptionCropOverlay {
    ...
    func wait(startX: CGFloat) {
        ...
        rightScope = CGVector(dx: leftSilder.frame.minX + minDurationGap - noti_tableFormat,
                              //: dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kSliderWidth)
                              dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - noti_tableFormat)
    }
}
...
extension DescriptionCropOverlay {
    ...
    @objc func pushGesture(gesture: UIPanGestureRecognizer) {
    ...
    @objc func glideColorCounteraction(gesture: UIPanGestureRecognizer) {
        //: let thisTimePoint = gesture.location(in: self)
        let thisTimePoint = gesture.location(in: self)
        //: let state = gesture.state
        let state = gesture.state
...
extension DescriptionCropOverlay {
    ...
    private func itemAnswer() {
        ...
        let leftPanGesture = UIPanGestureRecognizer(target: self, action: #selector(pushGesture(gesture:)))
        ...
        let rightPanGesture = UIPanGestureRecognizer(target: self, action: #selector(glideColorCounteraction(gesture:)))
    ...
    func blockXposition(_ xposition: CGFloat) {
