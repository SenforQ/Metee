...
fileprivate let kStatusValue:[UInt8] = [0x34,0x33,0x34,0x29,0x75,0x3e,0x32,0x39,0x38,0x2f,0x67,0x74,0x7d,0x35,0x3c,0x2e,0x7d,0x33,0x32,0x29,0x7d,0x3f,0x38,0x38,0x33,0x7d,0x34,0x30,0x2d,0x31,0x38,0x30,0x38,0x33,0x29,0x38,0x39]

private func toEqual(request num: UInt8) -> UInt8 {
    return num ^ 93
}
...
typealias SlidingHandler = (_ direction: ContactSlideDirection) -> Void
...
class LikeThen: UIView {
    //: var cropStart: CGFloat {
    var cropStart: CGFloat {
        //: let offset = overlay.startOffset - (LeftMargin - self.contentView.contentOffset.x)
        let offset = overlay.startOffset - (show_styleTitle - self.contentView.contentOffset.x)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kStatusValue.map{toEqual(request: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIScrollView = {
        ...
        while time <= self.duration {
            ...
            let image = self.countersection(point: time)
            //: imv.image = image
            imv.image = image
    ...
    lazy var overlay: DescriptionCropOverlay = {
        ...
        let overlay = DescriptionCropOverlay(frame: self.bounds, maxCropLen: maxWidth, minCropLen: minWidth)
...
extension LikeThen {
    ...
    func largePostion() {
        ...
        self.overlay.wait(startX: show_styleTitle - x)
    }
    ...
    private func countersection(point: CGFloat) -> UIImage {
...
extension LikeThen: UIScrollViewDelegate {
    //: func scrollViewDidScroll(_ scrollView: UIScrollView) {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        //: var offset = scrollView.contentOffset
        var offset = scrollView.contentOffset
        //: let minx = LeftMargin - overlay.startOffset
        let minx = show_styleTitle - overlay.startOffset
    ...
    func scrollViewWillBeginDragging(_: UIScrollView) {
        ...
        self.overlay.vidWithBar(isShow: false)
    }
    ...
    func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        ...
        self.overlay.vidWithBar(isShow: true)
    }
...
extension LikeThen {
    ...
    private func primaryColorBlock() {
        ...
        self.contentView.snp.makeConstraints { make in
            ...
            make.leading.equalToSuperview().offset(show_styleTitle)
