...
fileprivate let kValueStr:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "btnJQbEAB_etihw_kcab_shadow" :*/
fileprivate let constShareTitle:[Character] = ["b","t","n","J","Q","b","E","A","B","_","e","t"]
fileprivate let notiUserUrl:[Character] = ["i","h","w","_","k","c","a","b","_","s"]
fileprivate let appAddStr:String = "HADOW"

/*: "Edit" :*/
fileprivate let dataRegularValue:String = "eye let autoEdit"

/*: "Finish" :*/
fileprivate let app_modeSharedGiftMsg:[Character] = ["F"]
fileprivate let main_hideMsg:String = "inirowh"

/*: "btnYDo2AS_oediv_play" :*/
fileprivate let notiMyLabelFormat:String = "in return false outside blockbtnYDo"
fileprivate let kUpLanguageValue:String = "oedidismiss"

/*: "Cancel" :*/
fileprivate let notiTotalStr:String = "self bottom self translation nameCancel"

/*: "bgQkRTbF_oediv_toast" :*/
fileprivate let const_tableMsg:String = "bgQkRTraw table to"
fileprivate let app_hideData:String = "div_tlet self data"

/*: "The video length should be %@-%@ seconds. Please intercept it" :*/
fileprivate let showRequestStr:[UInt8] = [0x95,0xa9,0xa4,0xe1,0xb7,0xa8,0xa5,0xa4,0xae,0xe1,0xad,0xa4,0xaf,0xa6,0xb5,0xa9,0xe1,0xb2,0xa9,0xae,0xb4,0xad,0xa5,0xe1,0xa3,0xa4,0xe1,0xe4,0x81,0xec,0xe4,0x81,0xe1,0xb2,0xa4,0xa2,0xae,0xaf,0xa5,0xb2,0xef,0xe1,0x91,0xad,0xa4,0xa0,0xb2,0xa4,0xe1,0xa8,0xaf,0xb5,0xa4,0xb3,0xa2,0xa4,0xb1,0xb5,0xe1,0xa8,0xb5]
...
// __CLOSE_PRINT__
//
//  StyleControlViewController.swift
//  AbroadTalking
//
...
typealias VideoEditBlock = (_ videoInfo: ScrubbedTransformable?) -> Void

//: let kMinVideoDuration: CGFloat = 5.0
let main_eventName: CGFloat = 5.0

//: class TalkingVideoEditorController: TalkingBaseViewController {
class StyleControlViewController: ScoreViewController {
	var dataDoing: Bool = true
	var mudraSum: Int = 73
	var caboodleArray: [AnyHashable] = []
	var beggarMyNeighborPolicyDictionary: [AnyHashable: String] = [:]
	var contentOpen: Bool = false
	var bigTotal: Int = 84
	var bottomRemindArray: [AnyHashable] = []
    ...
    private var slider: LikeThen?
    //: private var videoInfo = TalkingVideoInfoModel()
    private var videoInfo = ScrubbedTransformable() // 编辑结果模型
    //: private var currStyle = TalkingVideoEditorStyle.normal
    private var currStyle = ReadTrackEditorStyle.normal // 当前样式
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if ActionSocketDelegate.shared.isShared == true { // 正在音视频通话，静音播放
		...
		if var viewValue = slider?.framesInScreen { 
			if var labelValue = slider?.framesInScreen { 
		            if (cancelBtn.layer.contentsRect.size.width != 1) && (cancelBtn.convert(CGRect(x: 0, y: 0, width: 0, height: CGFloat(67)), from: cancelBtn.superview).origin.x == 45.76) {
		                //: SWIFT_CUSTOM_DANGER_File_Call
		                let maxLegal = ScrubView()
		            maxLegal.laboratoryOff = shouldSaveCropedVideoToPhotoLibrary
		            maxLegal.enableInputInterval = labelValue
		            maxLegal.indexDoing = { [self] upwardOpen in
		            self.contentOpen = upwardOpen
		            
		            var maxLegal = isCompress
		            maxLegal = false
		            if maxLegal != self.contentOpen {
		                self.contentOpen = maxLegal
		            }
		            
		            return self.contentOpen
		            }
		            maxLegal.contextCount = { [self] playerManagerCheckMagnitude in
		            self.bigTotal = playerManagerCheckMagnitude
		            
		            var maxLegal = viewValue
		                maxLegal += 1
		                if maxLegal >= 0 {
		                    maxLegal = maxLegal - 1
		                }
		            if maxLegal < self.bigTotal {
		                self.bigTotal = maxLegal
		            }
		            
		            return self.bigTotal
		            }
		            maxLegal.viewArray = { [self] beautyEventArray in
		            self.bottomRemindArray = beautyEventArray
		            
		            guard var value = self.bottomRemindArray as? [String] else {
		                return nil
		            }
		            return value
		            }
		            maxLegal.winnowDictionary = { [self] wayDictionary in
		            self.sourceDictionary = wayDictionary
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        self.playBtn.isHidden = false
            ...
            if (!playBtn.transform.isIdentity) && (playBtn.layoutMargins.top == 14.82) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let receiveLet = ScrubView(frame: playBtn.bounds.intersection(CGRect(x: CGFloat(0), y: CGFloat(0), width: CGFloat(223.16), height: CGFloat(76))))
            receiveLet.laboratoryOff = videoInfo.needCut
            receiveLet.enableInputInterval = useVideoView.giftView.keyborHeight
            receiveLet.indexDoing = { [self] upwardOpen in
            self.dataDoing = upwardOpen
            
            var receiveLet = playing
            receiveLet = true
            if receiveLet != self.dataDoing {
                self.dataDoing = receiveLet
            }
            
            return self.dataDoing
            }
            receiveLet.contextCount = { [self] playerManagerCheckMagnitude in
            self.mudraSum = playerManagerCheckMagnitude
            
            var receiveLet = useVideoView.giftView.keyborHeight
            receiveLet &+= 1
            if receiveLet < self.mudraSum {
                self.mudraSum = receiveLet
            }
            
            return self.mudraSum
            }
            receiveLet.viewArray = { [self] beautyEventArray in
            self.caboodleArray = beautyEventArray
            
            guard var value = self.caboodleArray as? [String] else {
                return nil
            }
            return value
            }
            receiveLet.winnowDictionary = { [self] wayDictionary in
            self.beggarMyNeighborPolicyDictionary = wayDictionary
            
            guard var value = self.beggarMyNeighborPolicyDictionary as? [String: String] else {
                return nil
            }
            return value
            }
                playBtn.addSubview(receiveLet)
            }

	}

    //: init(style: TalkingVideoEditorStyle = .normal, maxDuration: CGFloat, videoURL: URL, completionHandler: @escaping VideoEditBlock) {
    init(style: ReadTrackEditorStyle = .normal, maxDuration: CGFloat, videoURL: URL, completionHandler: @escaping VideoEditBlock) {
        ...
        imgWidth = (notiEnabledRecordUrl - 2 * padding) / (framesInScreen * 1.0)
        //: imgHeight = 50
        imgHeight = 50
        //: duration = 0
        duration = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kValueStr.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var backBtn: UIButton = {
        ...
        v.setImage(UIImage.addBar(name: (String(constShareTitle) + String(notiUserUrl) + appAddStr.lowercased())), for: .normal)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = AddBeautyThen.shared.direction == .rightToLeft ? .right : .left
        //: v.addTarget(self, action: #selector(backAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(instanceShared), for: .touchUpInside)
    ...
    private lazy var editBtn: UIButton = {
        ...
        v.setTitle((String(dataRegularValue.suffix(4))).localized, for: .normal)
        //: v.setTitleColor(UIColor.appThemeColor(), for: .normal)
        v.setTitleColor(UIColor.outsideBecomeColor(), for: .normal)
        //: v.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 14)
        v.titleLabel?.font = UIFont.nogMain(fontSize: 14)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = AddBeautyThen.shared.direction == .rightToLeft ? .right : .left
        //: v.isHidden = true
        v.isHidden = true
//        v.addTarget(self, action: #selector(editAction(sender:)), for: .touchUpInside)
    ...
    private lazy var bottomDoneBtn: UIButton = {
        ...
        v.setTitle((String(app_modeSharedGiftMsg) + main_hideMsg.replacingOccurrences(of: "row", with: "s")).localized, for: .normal)
        //: v.setTitleColor(UIColor.white, for: .normal)
        v.setTitleColor(UIColor.white, for: .normal)
        //: v.titleLabel?.font = UIFont.pingfangMediumFont(fontSize: 14)
        v.titleLabel?.font = UIFont.mediumPingfangStatusSizeFont(fontSize: 14)
        //: v.layer.cornerRadius = 14.0
        v.layer.cornerRadius = 14.0
        //: v.layer.masksToBounds = true
        v.layer.masksToBounds = true
        //: v.backgroundColor = UIColor.appThemeColor()
        v.backgroundColor = UIColor.outsideBecomeColor()
        //: v.addTarget(self, action: #selector(bottomDoneAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(appearCurrent), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: return v
        return v
    ...
    private lazy var playBtn: UIButton = {
        ...
        v.setImage(UIImage.addBar(name: (String(notiMyLabelFormat.suffix(6)) + "2AS_" + kUpLanguageValue.replacingOccurrences(of: "dismiss", with: "v") + "_play")), for: .normal)
        //: v.addTarget(self, action: #selector(playAction(sender:)), for: .touchUpInside)
        v.addTarget(self, action: #selector(senderDestroy(sender:)), for: .touchUpInside)
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        //: return v
        return v
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        v.setTitle((String(notiTotalStr.suffix(6))).localized, for: .normal)
    ...
    private lazy var confirmBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(lensHoodAction), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: v.snp.makeConstraints { make in
        v.snp.makeConstraints { make in
    ...
    private lazy var toastImageView: UIImageView = {
        ...
        let v = UIImageView(image: UIImage.addBar(name: (String(const_tableMsg.prefix(6)) + "bF_oe" + String(app_hideData.prefix(5)) + "oast")))
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        ...
        lab.text = String(bytes: showRequestStr.map{$0^193}, encoding: .utf8)!.fast(minDuration, Int(maxduration))
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.numberOfLines = 0
        lab.numberOfLines = 0
    ...
    lazy var useVideoView: MuzzleVelocityView = {
        //: let v = TalkingSendIntimateInfoView(frame: CGRect.zero, type: .intimateVideo)
        let v = MuzzleVelocityView(frame: CGRect.zero, type: .intimateVideo)
        //: v.isHidden = true
        v.isHidden = true
        //: self.view.addSubview(v)
        self.view.addSubview(v)
...
extension StyleControlViewController {
    /// 返回按钮事件
    //: @objc func backAction() {
    @objc func instanceShared() {
        ...
        ViewSaveListener.shared.toVideo()
    ...
    private func objectMake(animated _: Bool = true) {
    ...
    @objc func asEffect() {
        ...
        self.previewLayer?.frame = CGRect(x: 26, y: 26, width: notiEnabledRecordUrl - 2 * 26, height: main_versionEnterTimeStr - 46 - 127 - constNameValue)
        //: imgWidth = ScreenWidth / framesInScreen
        imgWidth = notiEnabledRecordUrl / framesInScreen

        //: if slider == nil {
        if slider == nil {
            //: let frame = CGRect(x: 0, y: ScreenHeight, width: ScreenWidth, height: CGFloat(imgHeight))
            let frame = CGRect(x: 0, y: main_versionEnterTimeStr, width: notiEnabledRecordUrl, height: CGFloat(imgHeight))
            // 创建预览区域
            ...
            slider = LikeThen(frame: frame, url: self.videoURL!, imgWidth: imgWidth, maxDuration: maxduration, minVideoDuration: main_eventName)
            ...
            slider?.snp.makeConstraints { make in
                ...
                make.bottom.equalToSuperview().offset(-60 - constNameValue)
...
//                self.playing = false
            ...
            slider?.slidingBlock = { [weak self] (direction: ContactSlideDirection) in
                //: guard let self = self else { return }
                guard let self = self else { return }
...
//                        self.playBtn.isHidden = true
    ...
    @objc func appearCurrent() {
        ...
        if self.videoEditHandler != nil {
            ...
            ViewSaveListener.shared.addGreet(videoInfo: videoInfo, containerView: self.view, saveDocuments: true) { [weak self] outputPath, coverPath in
                //: guard let self = self else { return }
                guard let self = self else { return }
                //: self.popCurrentViewController()
                self.objectMake()
    ...
    @objc func senderDestroy(sender: UIButton) {
...
extension StyleControlViewController {
    ...
    private func workInValue() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(digitizerCreate), name: NSNotification.Name.AVPlayerItemDidPlayToEndTime, object: item)
        ...
        self.playerTimer = self.player?.addPeriodicTimeObserver(forInterval: step, queue: DispatchQueue.main, using: { [weak self] time in
            ...
            if self.slider != nil, self.slider!.isHidden == false {
                ...
                self.slider?.overlay.blockXposition(xposition)
            }
            //: }) as AnyObject?
        }) as AnyObject?
    ...
    private func lightTotal() {
        ...
        self.botBar.snp.makeConstraints { make in
            ...
            make.height.equalTo(48 + constNameValue)
        }
    ...
    private func funcSub() {
        // 监听应用进入后台
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(didEnterBackground),
                                               selector: #selector(backgroundWill),
